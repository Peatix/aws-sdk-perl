...
package TestFromYaml;
  use Moo;
  use Types::Standard qw(Str HashRef Bool ArrayRef Maybe);
  ...
  has todo_reason => (is => 'ro', isa => Maybe[Str], lazy => 1, default => sub { shift->_contents->{ todo } });
  has is_todo => (is => 'ro', isa => Bool, lazy => 1, default => sub { defined shift->todo_reason });

  has service => (is => 'ro', isa => Maybe[Str], lazy => 1, default => sub { shift->_contents->{ service } });
  has method  => (is => 'ro', isa => Maybe[Str], lazy => 1, default => sub { shift->_contents->{ call } });

  has exception => (is => 'ro', isa => Maybe[Str], lazy => 1, default => sub {
    my $self = shift; 
    exists $self->_contents->{ exception } and $self->_contents->{ exception } == 1 
  });

  has tests => (is => 'ro', isa => Maybe[ArrayRef], lazy => 1, default => sub {
    my $self = shift; 
