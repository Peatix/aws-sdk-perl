...
subtest 'presign_put with Japanese filename matches boto3 path' => sub {
    # Use AWS SDK to generate the same pre-signed URL, so we know the pre-signed
    # URL generated by the Paws::S3::Presigner module is correct.
    #
    # The expected path below was generated with boto3 1.43.69 /
    # botocore 1.43.69:
    #
    #   import boto3
    ...
    # "アップロード/請求書 2024年度.pdf"
    my $key = "\x{30A2}\x{30C3}\x{30D7}\x{30ED}\x{30FC}\x{30C9}"
            . '/'
            . "\x{8ACB}\x{6C42}\x{66F8} 2024\x{5E74}\x{5EA6}.pdf";
    my $file_size = 1048576;

    my $boto3_path =
        '/%E3%82%A2%E3%83%83%E3%83%97%E3%83%AD%E3%83%BC%E3%83%89'
      . '/%E8%AB%8B%E6%B1%82%E6%9B%B8%202024%E5%B9%B4%E5%BA%A6.pdf';
    ...
    my $u = URI->new($url);
    is($u->host, 'my-bucket.s3.ap-northeast-1.amazonaws.com',
       'host matches boto3 virtual-hosted-style host');
    is($u->path, $boto3_path,
       'Japanese key path is UTF-8 percent-encoded identically to boto3');
    is($u->query_param('X-Amz-SignedHeaders'), 'content-length;host',
       'file size is signed as the content-length header, as boto3 does');
    ...
    is($u->query_param('X-Amz-Signature'), $expected_sig,
       'golden signature over the Japanese key and file size');
};
