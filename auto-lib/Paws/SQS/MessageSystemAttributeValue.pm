...
package Paws::SQS::MessageSystemAttributeValue;
  use Moose;
...
Paws::SQS::MessageSystemAttributeValue
...
As an example, if Att1 is expected to be a Paws::SQS::MessageSystemAttributeValue object:

  $service_obj->Method(Att1 => { BinaryListValues => $value, ..., StringValue => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SQS::MessageSystemAttributeValue object:
...
types, the C<Value> attribute has the same restrictions on the content
as the message body. For more information, see C< SendMessage.>
