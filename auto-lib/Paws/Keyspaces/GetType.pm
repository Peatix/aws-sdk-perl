
package Paws::Keyspaces::GetType;
  use Moose;
  has KeyspaceName => (is => 'ro', isa => 'Str', traits => ['NameInRequest'], request_name => 'keyspaceName' , required => 1);
  has TypeName => (is => 'ro', isa => 'Str', traits => ['NameInRequest'], request_name => 'typeName' , required => 1);

  use MooseX::ClassAttribute;

  class_has _api_call => (isa => 'Str', is => 'ro', default => 'GetType');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::Keyspaces::GetTypeResponse');
  class_has _result_key => (isa => 'Str', is => 'ro');
1;

### main pod documentation begin ###

=head1 NAME

Paws::Keyspaces::GetType - Arguments for method GetType on L<Paws::Keyspaces>

=head1 DESCRIPTION

This class represents the parameters used for calling the method GetType on the
L<Amazon Keyspaces|Paws::Keyspaces> service. Use the attributes of this class
as arguments to method GetType.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to GetType.

=head1 SYNOPSIS

    my $cassandra = Paws->service('Keyspaces');
    my $GetTypeResponse = $cassandra->GetType(
      KeyspaceName => 'MyKeyspaceName',
      TypeName     => 'MyTypeName',

    );

    # Results:
    my $DirectParentTypes     = $GetTypeResponse->DirectParentTypes;
    my $DirectReferringTables = $GetTypeResponse->DirectReferringTables;
    my $FieldDefinitions      = $GetTypeResponse->FieldDefinitions;
    my $KeyspaceArn           = $GetTypeResponse->KeyspaceArn;
    my $KeyspaceName          = $GetTypeResponse->KeyspaceName;
    my $LastModifiedTimestamp = $GetTypeResponse->LastModifiedTimestamp;
    my $MaxNestingDepth       = $GetTypeResponse->MaxNestingDepth;
    my $Status                = $GetTypeResponse->Status;
    my $TypeName              = $GetTypeResponse->TypeName;

    # Returns a L<Paws::Keyspaces::GetTypeResponse> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/cassandra/GetType>

=head1 ATTRIBUTES


=head2 B<REQUIRED> KeyspaceName => Str

The name of the keyspace that contains this type.



=head2 B<REQUIRED> TypeName => Str

The formatted name of the type. For example, if the name of the type
was created without double quotes, Amazon Keyspaces saved the name in
lower-case characters. If the name was created in double quotes, you
must use double quotes to specify the type name.




=head1 SEE ALSO

This class forms part of L<Paws>, documenting arguments for method GetType in L<Paws::Keyspaces>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

