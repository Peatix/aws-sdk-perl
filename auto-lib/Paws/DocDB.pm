package Paws::DocDB;
  ...
  sub FailoverGlobalCluster {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DocDB::FailoverGlobalCluster', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub SwitchoverGlobalCluster {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DocDB::SwitchoverGlobalCluster', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
infrastructure with no impact to your workloadE<rsquo>s performance.
...
authorized to copy or restore the manual cluster snapshot. If C<all> is
included in the list of values for the C<restore> attribute, then the
...
Each argument is described in detail in: L<Paws::DocDB::FailoverGlobalCluster>

Returns: a L<Paws::DocDB::FailoverGlobalClusterResult> instance
...
disaster in the primary region. Failing over can result in a loss of
write transaction data that wasn't replicated to the chosen secondary
before the failover event occurred. However, the recovery process that
promotes a DB instance on the chosen seconday DB cluster to be the
primary writer DB instance guarantees that the data is in a
...
snapshot. Use the value C<all> to make the manual cluster snapshot
public, which means that it can be copied or restored by all Amazon Web
Services accounts. Do not add the C<all> value for any manual cluster
...
of authorized Amazon Web Services account IDs for the C<ValuesToAdd>
...
Each argument is described in detail in: L<Paws::DocDB::SwitchoverGlobalCluster>

Returns: a L<Paws::DocDB::SwitchoverGlobalClusterResult> instance
