...
package Paws::Neptune::GlobalClusterMember;
  use Moose;
...
Paws::Neptune::GlobalClusterMember
...
As an example, if Att1 is expected to be a Paws::Neptune::GlobalClusterMember object:

  $service_obj->Method(Att1 => { DBClusterArn => $value, ..., Readers => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Neptune::GlobalClusterMember object:
...
has read-write capability) for the Neptune global database with which
it is associated.
