...
package Paws::SecurityHub::VpcInfoPeeringOptionsDetails;
  use Moose;
...
Paws::SecurityHub::VpcInfoPeeringOptionsDetails
...
As an example, if Att1 is expected to be a Paws::SecurityHub::VpcInfoPeeringOptionsDetails object:

  $service_obj->Method(Att1 => { AllowDnsResolutionFromRemoteVpc => $value, ..., AllowEgressFromLocalVpcToRemoteClassicLink => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SecurityHub::VpcInfoPeeringOptionsDetails object:
