...
package Paws::SecurityHub::AwsRdsDbClusterSnapshotDbClusterSnapshotAttribute;
  use Moose;
...
Paws::SecurityHub::AwsRdsDbClusterSnapshotDbClusterSnapshotAttribute
...
As an example, if Att1 is expected to be a Paws::SecurityHub::AwsRdsDbClusterSnapshotDbClusterSnapshotAttribute object:

  $service_obj->Method(Att1 => { AttributeName => $value, ..., AttributeValues => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SecurityHub::AwsRdsDbClusterSnapshotDbClusterSnapshotAttribute object:
...
named C<restore> refers to the list of Amazon Web Services accounts
...
C<AttributeName> field is set to C<restore>, then this element returns
a list of IDs of the Amazon Web Services accounts that are authorized
to copy or restore the manual DB cluster snapshot. If a value of C<all>
