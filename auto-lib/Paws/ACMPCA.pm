...
Amazon Web Services Private CA throttles API requests at different
rates depending on the operation. Throttling means that Amazon Web
Services Private CA rejects an otherwise valid request because the
request exceeds the operation's quota for the number of requests per
second. When a request is throttled, Amazon Web Services Private CA
...
To see an up-to-date list of your Amazon Web Services Private CA
quotas, or to request a quota increase, log into your Amazon Web
Services account and visit the Service Quotas
...
is used to issue a certificate. The IssueCertificate
...
status of the CA is C<PENDING_CERTIFICATE>).
...
against the Amazon Web Services Private CA policy. For more
...
C<CREATING> - Amazon Web Services Private CA is creating your private
...
the ARNs of the certificates that you issue by using Amazon Web
Services Private CA.
...
updated approximately 30 minutes after a certificate is revoked. If for
any reason the CRL update fails, Amazon Web Services Private CA
attempts makes further attempts every 15 minutes. With Amazon
CloudWatch, you can create alarms for the metrics C<CRLGenerated> and
C<MisconfiguredCRLBucket>. For more information, see Supported
CloudWatch Metrics
...
on input by its Amazon Resource Name (ARN). You specify the tag by
...
C<CreateCertificateAuthority> action and explicitly allow tagging. For
