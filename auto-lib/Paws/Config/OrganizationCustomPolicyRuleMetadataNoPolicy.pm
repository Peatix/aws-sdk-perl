...
package Paws::Config::OrganizationCustomPolicyRuleMetadataNoPolicy;
  use Moose;
  ...
  has PolicyRuntime => (is => 'ro', isa => 'Str');
...
Paws::Config::OrganizationCustomPolicyRuleMetadataNoPolicy
...
As an example, if Att1 is expected to be a Paws::Config::OrganizationCustomPolicyRuleMetadataNoPolicy object:

  $service_obj->Method(Att1 => { DebugLogDeliveryAccounts => $value, ..., TagValueScope => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Config::OrganizationCustomPolicyRuleMetadataNoPolicy object:
...
Web Services resource, and organization trigger types that trigger
...
a rule. For Config Custom Policy rules, Config supports change
