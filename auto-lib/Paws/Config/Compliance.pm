...
a result of C<NON_COMPLIANT>, up to a maximum number.
...
Config returns the C<INSUFFICIENT_DATA> value when no evaluation
results are available for the Amazon Web Services resource or Config
rule.

For the C<Compliance> data type, Config supports only C<COMPLIANT>,
C<NON_COMPLIANT>, and C<INSUFFICIENT_DATA> values. Config does not
