...
package Paws::CUR::ReportDefinition;
  ...
  has ReportStatus => (is => 'ro', isa => 'Paws::CUR::ReportStatus');
...
S3 bucket, additional artifacts, and schema elements in the definition.
...
The Amazon resource name of the billing view. The C<BillingViewArn> is
needed to create Amazon Web Services Cost and Usage Report for each
billing group maintained in the Amazon Web Services Billing Conductor
service. The C<BillingViewArn> for a billing group can be constructed
as:
C<arn:aws:billing::payer-account-id:billingview/billing-group-primary-account-id>
...
=head2 ReportStatus => L<Paws::CUR::ReportStatus>
