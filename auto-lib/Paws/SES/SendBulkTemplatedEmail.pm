...
package Paws::SES::SendBulkTemplatedEmail;
  ...
  has DefaultTemplateData => (is => 'ro', isa => 'Str', required => 1);
...
=head1 SYNOPSIS
    ...
    my $SendBulkTemplatedEmailResponse = $email->SendBulkTemplatedEmail(
      DefaultTemplateData => 'MyTemplateData',
      Destinations        => [
        {
          Destination => {
          ...
          ReplacementTemplateData => 'MyTemplateData',    # max: 262144
        },
        ...
      ],
      Source               => 'MyAddress',
...
=head2 B<REQUIRED> DefaultTemplateData => Str
...
C<Destination> receive the same version of the email. You can specify
up to 50 C<Destination> objects within a C<Destinations> array.
...
C<ReturnPath> parameter. The C<ReturnPath> parameter is never
