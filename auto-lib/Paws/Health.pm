package Paws::Health;
  ...
  sub DescribeEntityAggregatesForOrganization {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Health::DescribeEntityAggregatesForOrganization', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
in the Health Dashboard (https://health.aws.amazon.com/health/home).
You can use the API operations to get information about events that
might affect your Amazon Web Services services and resources.

You must have a Business, Enterprise On-Ramp, or Enterprise Support
plan from Amazon Web Services Support
(http://aws.amazon.com/premiumsupport/) to use the Health API. If you
...
receive a C<SubscriptionRequiredException> error.
...
active-passive configuration. You can use the high availability
endpoint example to determine which Amazon Web Services Region is
active, so that you can get the latest information from the API. For
more information, see Accessing the Health API
...
or get notified of security vulnerabilities. Use the organizational
view API operations to enable this feature and return event
...
different types of Health events, see Event
...
=item [OrganizationEntityAccountFilters => ArrayRef[L<Paws::Health::EntityAccountFilter>]]
...
=head2 DescribeEntityAggregatesForOrganization
...
Each argument is described in detail in: L<Paws::Health::DescribeEntityAggregatesForOrganization>

Returns: a L<Paws::Health::DescribeEntityAggregatesForOrganizationResponse> instance
...
service, and so on, as returned by DescribeEvents
...
affected entities, use the DescribeAffectedEntitiesForOrganization
...
the C<entityValues> parameter, Health might return public events that
...
to that resource or service. To get events that are specific to a
service, use the C<services> parameter in the C<filter> object. For
...
metadata for each event appears in the EventType
...
For more information, see Aggregating Health events
(https://docs.aws.amazon.com/health/latest/ug/aggregate-events.html) in
the I<Health User Guide>.
...
For more information, see Deleting a Service-Linked Role
...
information, see Aggregating Health events
(https://docs.aws.amazon.com/health/latest/ug/aggregate-events.html) in
the I<Health User Guide>.
...
=head2 DescribeAllAffectedEntitiesForOrganization(sub { },[Locale => Str, MaxResults => Int, NextToken => Str, OrganizationEntityAccountFilters => ArrayRef[L<Paws::Health::EntityAccountFilter>], OrganizationEntityFilters => ArrayRef[L<Paws::Health::EventAccountFilter>]])

=head2 DescribeAllAffectedEntitiesForOrganization([Locale => Str, MaxResults => Int, NextToken => Str, OrganizationEntityAccountFilters => ArrayRef[L<Paws::Health::EntityAccountFilter>], OrganizationEntityFilters => ArrayRef[L<Paws::Health::EventAccountFilter>]])
