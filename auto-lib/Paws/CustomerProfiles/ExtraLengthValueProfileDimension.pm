...
package Paws::CustomerProfiles::ExtraLengthValueProfileDimension;
  use Moose;
...
Paws::CustomerProfiles::ExtraLengthValueProfileDimension
...
As an example, if Att1 is expected to be a Paws::CustomerProfiles::ExtraLengthValueProfileDimension object:

  $service_obj->Method(Att1 => { DimensionType => $value, ..., Values => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CustomerProfiles::ExtraLengthValueProfileDimension object:
