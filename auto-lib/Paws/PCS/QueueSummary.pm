...
As an example, if Att1 is expected to be a Paws::PCS::QueueSummary object:

  $service_obj->Method(Att1 => { Arn => $value, ..., Status => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::PCS::QueueSummary object:
...
The object returned by the C<ListQueues> API action.
