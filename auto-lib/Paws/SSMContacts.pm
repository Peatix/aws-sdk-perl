package Paws::SSMContacts;
  ...
  sub CreateRotation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::CreateRotation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub CreateRotationOverride {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::CreateRotationOverride', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DeleteRotation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::DeleteRotation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub DeleteRotationOverride {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::DeleteRotationOverride', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetRotation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::GetRotation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetRotationOverride {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::GetRotationOverride', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListPageResolutions {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::ListPageResolutions', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListPreviewRotationShifts {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::ListPreviewRotationShifts', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListRotationOverrides {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::ListRotationOverrides', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListRotations {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::ListRotations', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListRotationShifts {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::ListRotationShifts', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub UpdateRotation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMContacts::UpdateRotation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllPageResolutions {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListPageResolutions(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListPageResolutions(@_, NextToken => $next_result->NextToken);
        push @{ $result->PageResolutions }, @{ $next_result->PageResolutions };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'PageResolutions') foreach (@{ $result->PageResolutions });
        $result = $self->ListPageResolutions(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'PageResolutions') foreach (@{ $result->PageResolutions });
    }
  ...
  sub ListAllPreviewRotationShifts {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListPreviewRotationShifts(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListPreviewRotationShifts(@_, NextToken => $next_result->NextToken);
        push @{ $result->RotationShifts }, @{ $next_result->RotationShifts };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'RotationShifts') foreach (@{ $result->RotationShifts });
        $result = $self->ListPreviewRotationShifts(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'RotationShifts') foreach (@{ $result->RotationShifts });
    }
  ...
  sub ListAllRotationOverrides {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListRotationOverrides(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListRotationOverrides(@_, NextToken => $next_result->NextToken);
        push @{ $result->RotationOverrides }, @{ $next_result->RotationOverrides };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'RotationOverrides') foreach (@{ $result->RotationOverrides });
        $result = $self->ListRotationOverrides(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'RotationOverrides') foreach (@{ $result->RotationOverrides });
    }
  ...
  sub ListAllRotations {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListRotations(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListRotations(@_, NextToken => $next_result->NextToken);
      ...
      while ($result->NextToken) {
        ...
        $result = $self->ListRotations(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'Rotations') foreach (@{ $result->Rotations });
    }
  ...
  sub ListAllRotationShifts {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListRotationShifts(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListRotationShifts(@_, NextToken => $next_result->NextToken);
        push @{ $result->RotationShifts }, @{ $next_result->RotationShifts };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'RotationShifts') foreach (@{ $result->RotationShifts });
        $result = $self->ListRotationShifts(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'RotationShifts') foreach (@{ $result->RotationShifts });
    }
  ...
  sub operations { qw/AcceptPage ActivateContactChannel CreateContact CreateContactChannel CreateRotation CreateRotationOverride DeactivateContactChannel DeleteContact DeleteContactChannel DeleteRotation DeleteRotationOverride DescribeEngagement DescribePage GetContact GetContactChannel GetContactPolicy GetRotation GetRotationOverride ListContactChannels ListContacts ListEngagements ListPageReceipts ListPageResolutions ListPagesByContact ListPagesByEngagement ListPreviewRotationShifts ListRotationOverrides ListRotations ListRotationShifts ListTagsForResource PutContactPolicy SendActivationCode StartEngagement StopEngagement TagResource UntagResource UpdateContact UpdateContactChannel UpdateRotation / }
...
=head2 CreateRotation

=over

=item ContactIds => ArrayRef[Str|Undef]

=item Name => Str

=item Recurrence => L<Paws::SSMContacts::RecurrenceSettings>
...
Each argument is described in detail in: L<Paws::SSMContacts::CreateRotation>

Returns: a L<Paws::SSMContacts::CreateRotationResult> instance

Creates a rotation in an on-call schedule.


=head2 CreateRotationOverride

=over

=item EndTime => Str

=item NewContactIds => ArrayRef[Str|Undef]
...
Each argument is described in detail in: L<Paws::SSMContacts::CreateRotationOverride>

Returns: a L<Paws::SSMContacts::CreateRotationOverrideResult> instance
...
Each argument is described in detail in: L<Paws::SSMContacts::DeleteRotation>

Returns: a L<Paws::SSMContacts::DeleteRotationResult> instance
...
=head2 DeleteRotationOverride

=over

=item RotationId => Str

=item RotationOverrideId => Str


=back

Each argument is described in detail in: L<Paws::SSMContacts::DeleteRotationOverride>

Returns: a L<Paws::SSMContacts::DeleteRotationOverrideResult> instance
...
Each argument is described in detail in: L<Paws::SSMContacts::GetRotation>

Returns: a L<Paws::SSMContacts::GetRotationResult> instance

Retrieves information about an on-call rotation.


=head2 GetRotationOverride
...
Each argument is described in detail in: L<Paws::SSMContacts::GetRotationOverride>

Returns: a L<Paws::SSMContacts::GetRotationOverrideResult> instance
...
Each argument is described in detail in: L<Paws::SSMContacts::ListPageResolutions>

Returns: a L<Paws::SSMContacts::ListPageResolutionsResult> instance
...
hierarchy of I<escalation plan E<gt> on-call schedule E<gt> contact>.
...
=head2 ListPreviewRotationShifts
...
=item [Overrides => ArrayRef[L<Paws::SSMContacts::PreviewOverride>]]

=item [RotationStartTime => Str]
...
Each argument is described in detail in: L<Paws::SSMContacts::ListPreviewRotationShifts>

Returns: a L<Paws::SSMContacts::ListPreviewRotationShiftsResult> instance

Returns a list of shifts based on rotation configuration parameters.

The Incident Manager primarily uses this operation to populate the
B<Preview> calendar. It is not typically run by end users.


=head2 ListRotationOverrides
...
Each argument is described in detail in: L<Paws::SSMContacts::ListRotationOverrides>

Returns: a L<Paws::SSMContacts::ListRotationOverridesResult> instance
...
=item [RotationNamePrefix => Str]


=back

Each argument is described in detail in: L<Paws::SSMContacts::ListRotations>

Returns: a L<Paws::SSMContacts::ListRotationsResult> instance

Retrieves a list of on-call rotations.


=head2 ListRotationShifts
...
Each argument is described in detail in: L<Paws::SSMContacts::ListRotationShifts>

Returns: a L<Paws::SSMContacts::ListRotationShiftsResult> instance
...
C<ActivateChannel> operation. Incident Manager can't engage a contact
...
Each argument is described in detail in: L<Paws::SSMContacts::UpdateRotation>

Returns: a L<Paws::SSMContacts::UpdateRotationResult> instance
...
=head2 ListAllPageResolutions(sub { },PageId => Str, [NextToken => Str])

=head2 ListAllPageResolutions(PageId => Str, [NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - PageResolutions, passing the object as the first parameter, and the string 'PageResolutions' as the second parameter 

If not, it will return a a L<Paws::SSMContacts::ListPageResolutionsResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllPreviewRotationShifts(sub { },EndTime => Str, Members => ArrayRef[Str|Undef], Recurrence => L<Paws::SSMContacts::RecurrenceSettings>, TimeZoneId => Str, [MaxResults => Int, NextToken => Str, Overrides => ArrayRef[L<Paws::SSMContacts::PreviewOverride>], RotationStartTime => Str, StartTime => Str])

=head2 ListAllPreviewRotationShifts(EndTime => Str, Members => ArrayRef[Str|Undef], Recurrence => L<Paws::SSMContacts::RecurrenceSettings>, TimeZoneId => Str, [MaxResults => Int, NextToken => Str, Overrides => ArrayRef[L<Paws::SSMContacts::PreviewOverride>], RotationStartTime => Str, StartTime => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - RotationShifts, passing the object as the first parameter, and the string 'RotationShifts' as the second parameter 

If not, it will return a a L<Paws::SSMContacts::ListPreviewRotationShiftsResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllRotationOverrides(sub { },EndTime => Str, RotationId => Str, StartTime => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllRotationOverrides(EndTime => Str, RotationId => Str, StartTime => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - RotationOverrides, passing the object as the first parameter, and the string 'RotationOverrides' as the second parameter 

If not, it will return a a L<Paws::SSMContacts::ListRotationOverridesResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllRotations(sub { },[MaxResults => Int, NextToken => Str, RotationNamePrefix => Str])

=head2 ListAllRotations([MaxResults => Int, NextToken => Str, RotationNamePrefix => Str])
...
If not, it will return a a L<Paws::SSMContacts::ListRotationsResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllRotationShifts(sub { },EndTime => Str, RotationId => Str, [MaxResults => Int, NextToken => Str, StartTime => Str])

=head2 ListAllRotationShifts(EndTime => Str, RotationId => Str, [MaxResults => Int, NextToken => Str, StartTime => Str])
...
If not, it will return a a L<Paws::SSMContacts::ListRotationShiftsResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
