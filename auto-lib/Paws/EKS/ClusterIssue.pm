...
As an example, if Att1 is expected to be a Paws::EKS::ClusterIssue object:

  $service_obj->Method(Att1 => { Code => $value, ..., ResourceIds => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::EKS::ClusterIssue object:
