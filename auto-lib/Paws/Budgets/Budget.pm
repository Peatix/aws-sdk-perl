...
package Paws::Budgets::Budget;
  use Moose;
  has AutoAdjustData => (is => 'ro', isa => 'Paws::Budgets::AutoAdjustData');
  ...
  has FilterExpression => (is => 'ro', isa => 'Paws::Budgets::Expression');
...
As an example, if Att1 is expected to be a Paws::Budgets::Budget object:
  ...
  $service_obj->Method(Att1 => { AutoAdjustData => $value, ..., TimeUnit => $value  });
...
=head2 AutoAdjustData => L<Paws::Budgets::AutoAdjustData>
...
Plans utilization or coverage budgets default to C<100>. This is the
...
C<:> and C<\> characters, and the "/action/" substring, aren't allowed
in C<BudgetName>.
...
The cost filters, such as C<Region>, C<Service>, C<LinkedAccount>,
C<Tag>, or C<CostCategory>, that are applied to a budget.

Amazon Web Services Budgets supports the following services as a
C<Service> filter for RI budgets:
...
=head2 FilterExpression => L<Paws::Budgets::Expression>
...
supports both monthly and quarterly C<TimeUnit>.
...
For quarterly budgets, provide four quarters of C<PlannedBudgetLimits>
...
C<key> is the start of the quarter, C<UTC> in epoch seconds.
...
C<PlannedBudgetLimits> also contain C<BudgetLimit> representing the
current month or quarter limit present in C<PlannedBudgetLimits>. This
only applies to budgets that are created with C<PlannedBudgetLimits>.
Budgets that are created without C<PlannedBudgetLimits> only contain
C<BudgetLimit>. They don't contain C<PlannedBudgetLimits>.


=head2 TimePeriod => L<Paws::Budgets::TimePeriod>

The period of time that's covered by a budget. You set the start date
and end date. The start date must come before the end date. The end
date must come before C<06/15/87 00:00 UTC>.
...
budget on January 24, 2018, chose C<DAILY>, and didn't set a start
date, Amazon Web Services set your start date to C<01/24/18 00:00 UTC>.
If you chose C<MONTHLY>, Amazon Web Services set your start date to
C<01/01/18 00:00 UTC>. If you didn't specify an end date, Amazon Web
Services set your end date to C<06/15/87 00:00 UTC>. The defaults are
