...
package Paws::Route53RecoveryControlConfig::AssertionRule;
  use Moose;
...
Paws::Route53RecoveryControlConfig::AssertionRule
...
As an example, if Att1 is expected to be a Paws::Route53RecoveryControlConfig::AssertionRule object:

  $service_obj->Method(Att1 => { AssertedControls => $value, ..., WaitPeriodMs => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Route53RecoveryControlConfig::AssertionRule object:
...
states must be ON, so that at least two Amazon Web Services Regions
