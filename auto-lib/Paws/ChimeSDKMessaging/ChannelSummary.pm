...
package Paws::ChimeSDKMessaging::ChannelSummary;
  use Moose;
...
Paws::ChimeSDKMessaging::ChannelSummary
...
As an example, if Att1 is expected to be a Paws::ChimeSDKMessaging::ChannelSummary object:

  $service_obj->Method(Att1 => { ChannelArn => $value, ..., Privacy => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ChimeSDKMessaging::ChannelSummary object:
...
The time at which the last persistent message visible to the caller in
a channel was sent.
