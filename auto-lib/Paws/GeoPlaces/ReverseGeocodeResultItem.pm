...
package Paws::GeoPlaces::ReverseGeocodeResultItem;
  use Moose;
...
Paws::GeoPlaces::ReverseGeocodeResultItem
...
As an example, if Att1 is expected to be a Paws::GeoPlaces::ReverseGeocodeResultItem object:

  $service_obj->Method(Att1 => { AccessPoints => $value, ..., Title => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::GeoPlaces::ReverseGeocodeResultItem object:
...
The returned location from the C<Reverse Geocode> action.
