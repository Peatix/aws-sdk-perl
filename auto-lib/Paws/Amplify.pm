...
and web apps. Amplify Hosting provides a continuous delivery and
...
=item [CacheConfig => L<Paws::Amplify::CacheConfig>]

=item [ComputeRoleArn => Str]
...
interface (CLI). This API isnE<rsquo>t available to Amplify Gen 2
...
=item [Backend => L<Paws::Amplify::Backend>]
...
=item [EnableSkewProtection => Bool]
...
The maximum duration between the C<CreateDeployment> call and the
C<StartDeployment> call cannot exceed 8 hours. If the duration exceeds
8 hours, the C<StartDeployment> call and the associated C<Job> will
fail.
...
=item [CertificateSettings => L<Paws::Amplify::CertificateSettings>]
