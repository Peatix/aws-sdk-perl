...
package Paws::Pipes::EcsResourceRequirement;
  use Moose;
...
Paws::Pipes::EcsResourceRequirement
...
As an example, if Att1 is expected to be a Paws::Pipes::EcsResourceRequirement object:

  $service_obj->Method(Att1 => { Type => $value, ..., Value => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Pipes::EcsResourceRequirement object:
...
If the C<InferenceAccelerator> type is used, the C<value> matches the
C<deviceName> for an InferenceAccelerator specified in a task
definition.
