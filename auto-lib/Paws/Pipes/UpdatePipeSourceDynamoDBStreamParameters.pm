...
package Paws::Pipes::UpdatePipeSourceDynamoDBStreamParameters;
  use Moose;
...
Paws::Pipes::UpdatePipeSourceDynamoDBStreamParameters
...
As an example, if Att1 is expected to be a Paws::Pipes::UpdatePipeSourceDynamoDBStreamParameters object:

  $service_obj->Method(Att1 => { BatchSize => $value, ..., ParallelizationFactor => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Pipes::UpdatePipeSourceDynamoDBStreamParameters object:
