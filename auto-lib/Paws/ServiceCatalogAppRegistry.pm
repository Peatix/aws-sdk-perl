package Paws::ServiceCatalogAppRegistry;
  ...
  sub GetAssociatedResource {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalogAppRegistry::GetAssociatedResource', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetConfiguration {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalogAppRegistry::GetConfiguration', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAttributeGroupsForApplication {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalogAppRegistry::ListAttributeGroupsForApplication', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub PutConfiguration {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalogAppRegistry::PutConfiguration', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub SyncResource {
    my $self = shift;
  ...
  sub ListAllAttributeGroupsForApplication {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListAttributeGroupsForApplication(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->ListAttributeGroupsForApplication(@_, nextToken => $next_result->nextToken);
        push @{ $result->attributeGroupsDetails }, @{ $next_result->attributeGroupsDetails };
      }
      return $result;
    } else {
      while ($result->nextToken) {
        $callback->($_ => 'attributeGroupsDetails') foreach (@{ $result->attributeGroupsDetails });
        $result = $self->ListAttributeGroupsForApplication(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'attributeGroupsDetails') foreach (@{ $result->attributeGroupsDetails });
    }
...
Amazon Web Services Service Catalog AppRegistry enables organizations
to understand the application context of their Amazon Web Services
resources. AppRegistry provides a repository of your applications,
their resources, and the application metadata that you use within your
enterprise.
...
the C<OPTIONS> parameter set to C<APPLY_APPLICATION_TAG>.
...
C<AWSServiceCatalogAppRegistryFullAccess> policy. For more information,
see AWSServiceCatalogAppRegistryFullAccess
(https://docs.aws.amazon.com/servicecatalog/latest/arguide/full.html)
...
C<resource-groups:AssociateResource>

=item *

C<cloudformation:UpdateStack>
...
in the I<Resource Groups Tagging API Reference>.
...
been associated with an application using the C<APPLY_APPLICATION_TAG>
...
C<resource-groups:DisassociateResource>
...
exact same application is returned or a C<ResourceNotFoundException> is
thrown, avoiding the ABA addressing problem.


=head2 GetAssociatedResource
...
=item [ResourceTagStatus => ArrayRef[Str|Undef]]


=back

Each argument is described in detail in: L<Paws::ServiceCatalogAppRegistry::GetAssociatedResource>

Returns: a L<Paws::ServiceCatalogAppRegistry::GetAssociatedResourceResponse> instance
...
Each argument is described in detail in: L<Paws::ServiceCatalogAppRegistry::GetConfiguration>

Returns: a L<Paws::ServiceCatalogAppRegistry::GetConfigurationResponse> instance

Retrieves a C<TagKey> configuration from an account.
...
=head2 ListAttributeGroupsForApplication
...
Each argument is described in detail in: L<Paws::ServiceCatalogAppRegistry::ListAttributeGroupsForApplication>

Returns: a L<Paws::ServiceCatalogAppRegistry::ListAttributeGroupsForApplicationResponse> instance
...
=head2 PutConfiguration

=over

=item Configuration => L<Paws::ServiceCatalogAppRegistry::AppRegistryConfiguration>


=back

Each argument is described in detail in: L<Paws::ServiceCatalogAppRegistry::PutConfiguration>
...
Specifically, the resourceE<rsquo>s AppRegistry system tags sync with
its associated application. We remove the resource's AppRegistry system
...
=head2 ListAllAttributeGroupsForApplication(sub { },Application => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllAttributeGroupsForApplication(Application => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - attributeGroupsDetails, passing the object as the first parameter, and the string 'attributeGroupsDetails' as the second parameter 

If not, it will return a a L<Paws::ServiceCatalogAppRegistry::ListAttributeGroupsForApplicationResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
