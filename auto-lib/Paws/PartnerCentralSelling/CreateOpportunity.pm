...
package Paws::PartnerCentralSelling::CreateOpportunity;
  use Moose;
  ...
  class_has _api_call => (isa => 'Str', is => 'ro', default => 'CreateOpportunity');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::PartnerCentralSelling::CreateOpportunityResponse');
...
Paws::PartnerCentralSelling::CreateOpportunity - Arguments for method CreateOpportunity on L<Paws::PartnerCentralSelling>

=head1 DESCRIPTION

This class represents the parameters used for calling the method CreateOpportunity on the
L<Partner Central Selling API|Paws::PartnerCentralSelling> service. Use the attributes of this class
as arguments to method CreateOpportunity.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to CreateOpportunity.

=head1 SYNOPSIS

    my $partnercentral-selling = Paws->service('PartnerCentralSelling');
    my $CreateOpportunityResponse = $partnercentral -selling->CreateOpportunity(
      Catalog     => 'MyCatalogIdentifier',
      ClientToken => 'MyCreateOpportunityRequestClientTokenString',
      Customer    => {
        Account => {
      ...
      Origin                       => 'AWS Referral',        # OPTIONAL
      PartnerOpportunityIdentifier =>
        'MyCreateOpportunityRequestPartnerOpportunityIdentifierString'
      ,                                                      # OPTIONAL
      PrimaryNeedsFromAws => [
      ...
      Project => {
        ...
        RelatedOpportunityIdentifier => 'MyOpportunityIdentifier',    # OPTIONAL
        SalesActivities              => [
    ...
    # Results:
    my $Id               = $CreateOpportunityResponse->Id;
    my $LastModifiedDate = $CreateOpportunityResponse->LastModifiedDate;
    my $PartnerOpportunityIdentifier =
      $CreateOpportunityResponse->PartnerOpportunityIdentifier;

   # Returns a L<Paws::PartnerCentralSelling::CreateOpportunityResponse> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/partnercentral-selling/CreateOpportunity>
...
determines which environment the opportunity is created in. Use C<AWS>
...
Universally Unique Identifier (UUID) as the C<ClientToken>. You can use
standard libraries from most programming languages to generate this. If
you use the same client token, the API returns the following error:
...
Specifies customer details associated with the C<Opportunity>.
...
with C<Catalog: AWS>, this field must only be C<Partner Referral>.
However, when using C<Catalog: Sandbox>, you can set this field to
C<AWS Referral> to simulate Amazon Web Services referral creation. This
...
An object that contains project details for the C<Opportunity>.
...
This class forms part of L<Paws>, documenting arguments for method CreateOpportunity in L<Paws::PartnerCentralSelling>
