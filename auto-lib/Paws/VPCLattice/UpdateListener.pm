
package Paws::VPCLattice::UpdateListener;
  use Moose;
  has DefaultAction => (is => 'ro', isa => 'Paws::VPCLattice::RuleAction', traits => ['NameInRequest'], request_name => 'defaultAction', required => 1);
  has ListenerIdentifier => (is => 'ro', isa => 'Str', traits => ['ParamInURI'], uri_name => 'listenerIdentifier', required => 1);
  has ServiceIdentifier => (is => 'ro', isa => 'Str', traits => ['ParamInURI'], uri_name => 'serviceIdentifier', required => 1);

  use MooseX::ClassAttribute;

  class_has _api_call => (isa => 'Str', is => 'ro', default => 'UpdateListener');
  class_has _api_uri  => (isa => 'Str', is => 'ro', default => '/services/{serviceIdentifier}/listeners/{listenerIdentifier}');
  class_has _api_method  => (isa => 'Str', is => 'ro', default => 'PATCH');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::VPCLattice::UpdateListenerResponse');
1;

### main pod documentation begin ###

=head1 NAME

Paws::VPCLattice::UpdateListener - Arguments for method UpdateListener on L<Paws::VPCLattice>

=head1 DESCRIPTION

This class represents the parameters used for calling the method UpdateListener on the
L<Amazon VPC Lattice|Paws::VPCLattice> service. Use the attributes of this class
as arguments to method UpdateListener.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to UpdateListener.

=head1 SYNOPSIS

    my $vpc-lattice = Paws->service('VPCLattice');
    my $UpdateListenerResponse = $vpc -lattice->UpdateListener(
      DefaultAction => {
        FixedResponse => {
          StatusCode => 1,    # min: 100, max: 599

        },    # OPTIONAL
        Forward => {
          TargetGroups => [
            {
              TargetGroupIdentifier =>
                'MyTargetGroupIdentifier',    # min: 17, max: 2048
              Weight => 1,                    # max: 999; OPTIONAL
            },
            ...
          ],    # min: 1, max: 10

        },    # OPTIONAL
      },
      ListenerIdentifier => 'MyListenerIdentifier',
      ServiceIdentifier  => 'MyServiceIdentifier',

    );

    # Results:
    my $Arn           = $UpdateListenerResponse->Arn;
    my $DefaultAction = $UpdateListenerResponse->DefaultAction;
    my $Id            = $UpdateListenerResponse->Id;
    my $Name          = $UpdateListenerResponse->Name;
    my $Port          = $UpdateListenerResponse->Port;
    my $Protocol      = $UpdateListenerResponse->Protocol;
    my $ServiceArn    = $UpdateListenerResponse->ServiceArn;
    my $ServiceId     = $UpdateListenerResponse->ServiceId;

    # Returns a L<Paws::VPCLattice::UpdateListenerResponse> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/vpc-lattice/UpdateListener>

=head1 ATTRIBUTES


=head2 B<REQUIRED> DefaultAction => L<Paws::VPCLattice::RuleAction>

The action for the default rule.



=head2 B<REQUIRED> ListenerIdentifier => Str

The ID or ARN of the listener.



=head2 B<REQUIRED> ServiceIdentifier => Str

The ID or ARN of the service.




=head1 SEE ALSO

This class forms part of L<Paws>, documenting arguments for method UpdateListener in L<Paws::VPCLattice>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

