...
package Paws::Lightsail::Blueprint;
  use Moose;
  has AppCategory => (is => 'ro', isa => 'Str', request_name => 'appCategory', traits => ['NameInRequest']);
...
As an example, if Att1 is expected to be a Paws::Lightsail::Blueprint object:
  ...
  $service_obj->Method(Att1 => { AppCategory => $value, ..., VersionCode => $value  });
...
The ID for the virtual private server image (C<app_wordpress_x_x> or
C<app_lamp_x_x>).
...
The group name of the blueprint (C<amazon-linux>).
...
The friendly name of the blueprint (C<Amazon Linux>).
...
The type of the blueprint (C<os> or C<app>).
