...
package Paws::SageMakerRuntime::InvokeEndpointWithResponseStream;
  use Moose;
  ...
  class_has _api_call => (isa => 'Str', is => 'ro', default => 'InvokeEndpointWithResponseStream');
  class_has _api_uri  => (isa => 'Str', is => 'ro', default => '/endpoints/{EndpointName}/invocations-response-stream');
  class_has _api_method  => (isa => 'Str', is => 'ro', default => 'POST');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::SageMakerRuntime::InvokeEndpointWithResponseStreamOutput');
1;
...
Paws::SageMakerRuntime::InvokeEndpointWithResponseStream - Arguments for method InvokeEndpointWithResponseStream on L<Paws::SageMakerRuntime>

=head1 DESCRIPTION

This class represents the parameters used for calling the method InvokeEndpointWithResponseStream on the
L<Amazon SageMaker Runtime|Paws::SageMakerRuntime> service. Use the attributes of this class
as arguments to method InvokeEndpointWithResponseStream.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to InvokeEndpointWithResponseStream.

=head1 SYNOPSIS

    my $runtime.sagemaker = Paws->service('SageMakerRuntime');
    my $InvokeEndpointWithResponseStreamOutput =
      $runtime . sagemaker->InvokeEndpointWithResponseStream(
      Body                    => 'BlobBodyBlob',
      ...
      SessionId               => 'MySessionIdHeader',                 # OPTIONAL
      TargetContainerHostname => 'MyTargetContainerHostnameHeader',   # OPTIONAL
      TargetVariant           => 'MyTargetVariantHeader',             # OPTIONAL
      );

    # Results:
    my $Body        = $InvokeEndpointWithResponseStreamOutput->Body;
    my $ContentType = $InvokeEndpointWithResponseStreamOutput->ContentType;
    my $CustomAttributes =
...
# Returns a L<Paws::SageMakerRuntime::InvokeEndpointWithResponseStreamOutput> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/runtime.sagemaker/InvokeEndpointWithResponseStream>
...
C<InvokeEndpointWithResponseStream> action. Instead, you can create one
by using the C< InvokeEndpoint > action. In your request, you specify
C<NEW_SESSION> for the C<SessionId> request parameter. The response to
that request provides the session ID for the C<NewSessionId> response
parameter.
...
This class forms part of L<Paws>, documenting arguments for method InvokeEndpointWithResponseStream in L<Paws::SageMakerRuntime>
