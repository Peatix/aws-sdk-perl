package Paws::Pricing;
  ...
  sub GetPriceListFileUrl {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Pricing::GetPriceListFileUrl', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetProducts {
    my $self = shift;
  ...
  sub ListPriceLists {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Pricing::ListPriceLists', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllPriceLists {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListPriceLists(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListPriceLists(@_, NextToken => $next_result->NextToken);
        push @{ $result->PriceLists }, @{ $next_result->PriceLists };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'PriceLists') foreach (@{ $result->PriceLists });
        $result = $self->ListPriceLists(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'PriceLists') foreach (@{ $result->PriceLists });
    }
  ...
  sub operations { qw/DescribeServices GetAttributeValues GetPriceListFileUrl GetProducts ListPriceLists / }
...
You can use the Amazon Web Services Price List to do the following:
...
for all Amazon Web Services services, then C<GetServices> with a
service code to retrieve the attribute names for that service. After
you have the service code and attribute names, you can use
C<GetAttributeValues> to see what values are available for an
attribute. With the service code and an attribute name and value, you
can use C<GetProducts> to find specific products that you're interested
in, such as an C<AmazonEC2> instance, with a C<Provisioned IOPS>
C<volumeType>.
...
=head2 GetPriceListFileUrl

=over

=item FileFormat => Str

=item PriceListArn => Str


=back

Each argument is described in detail in: L<Paws::Pricing::GetPriceListFileUrl>

Returns: a L<Paws::Pricing::GetPriceListFileUrlResponse> instance
...
This URL is based on the C<PriceListArn> and C<FileFormat> that you
...
=head2 ListPriceLists

=over

=item CurrencyCode => Str

=item EffectiveDate => Str
...
Each argument is described in detail in: L<Paws::Pricing::ListPriceLists>

Returns: a L<Paws::Pricing::ListPriceListsResponse> instance
...
authorized to view, given a C<ServiceCode>, C<CurrencyCode>, and an
C<EffectiveDate>. Use without a C<RegionCode> filter to list Price List
references from all available Amazon Web Services Regions. Use with a
C<RegionCode> filter to get the Price List reference that's specific to
a specific Amazon Web Services Region. You can use the C<PriceListArn>
...
=head2 GetAllProducts(sub { },ServiceCode => Str, [Filters => ArrayRef[L<Paws::Pricing::Filter>], FormatVersion => Str, MaxResults => Int, NextToken => Str])

=head2 GetAllProducts(ServiceCode => Str, [Filters => ArrayRef[L<Paws::Pricing::Filter>], FormatVersion => Str, MaxResults => Int, NextToken => Str])
...
=head2 ListAllPriceLists(sub { },CurrencyCode => Str, EffectiveDate => Str, ServiceCode => Str, [MaxResults => Int, NextToken => Str, RegionCode => Str])

=head2 ListAllPriceLists(CurrencyCode => Str, EffectiveDate => Str, ServiceCode => Str, [MaxResults => Int, NextToken => Str, RegionCode => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - PriceLists, passing the object as the first parameter, and the string 'PriceLists' as the second parameter 

If not, it will return a a L<Paws::Pricing::ListPriceListsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
