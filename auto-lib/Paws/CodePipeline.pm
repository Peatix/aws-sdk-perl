package Paws::CodePipeline;
  ...
  sub ListDeployActionExecutionTargets {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodePipeline::ListDeployActionExecutionTargets', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListRuleExecutions {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodePipeline::ListRuleExecutions', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListRuleTypes {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodePipeline::ListRuleTypes', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub OverrideStageCondition {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodePipeline::OverrideStageCondition', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub PollForJobs {
    my $self = shift;
  ...
  sub RollbackStage {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodePipeline::RollbackStage', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllDeployActionExecutionTargets {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListDeployActionExecutionTargets(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->ListDeployActionExecutionTargets(@_, nextToken => $next_result->nextToken);
      ...
      while ($result->nextToken) {
        ...
        $result = $self->ListDeployActionExecutionTargets(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'targets') foreach (@{ $result->targets });
    }
  ...
  sub ListAllRuleExecutions {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListRuleExecutions(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->ListRuleExecutions(@_, nextToken => $next_result->nextToken);
        push @{ $result->ruleExecutionDetails }, @{ $next_result->ruleExecutionDetails };
      }
      return $result;
    } else {
      while ($result->nextToken) {
        $callback->($_ => 'ruleExecutionDetails') foreach (@{ $result->ruleExecutionDetails });
        $result = $self->ListRuleExecutions(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'ruleExecutionDetails') foreach (@{ $result->ruleExecutionDetails });
    }
...
This is the CodePipeline API Reference. This guide provides
descriptions of the actions and data types for CodePipeline. Some
functionality for your pipeline can only be configured through the API.
For more information, see the CodePipeline User Guide
...
B<Using the API to integrate with CodePipeline>
...
=head2 ListDeployActionExecutionTargets

=over

=item ActionExecutionId => Str

=item [Filters => ArrayRef[L<Paws::CodePipeline::TargetFilter>]]
...
Each argument is described in detail in: L<Paws::CodePipeline::ListDeployActionExecutionTargets>

Returns: a L<Paws::CodePipeline::ListDeployActionExecutionTargetsOutput> instance
...
=item [Filter => L<Paws::CodePipeline::PipelineExecutionFilter>]
...
=head2 ListRuleExecutions

=over

=item PipelineName => Str

=item [Filter => L<Paws::CodePipeline::RuleExecutionFilter>]
...
Each argument is described in detail in: L<Paws::CodePipeline::ListRuleExecutions>

Returns: a L<Paws::CodePipeline::ListRuleExecutionsOutput> instance
...
=item [RuleOwnerFilter => Str]


=back

Each argument is described in detail in: L<Paws::CodePipeline::ListRuleTypes>

Returns: a L<Paws::CodePipeline::ListRuleTypesOutput> instance
...
=head2 OverrideStageCondition

=over

=item ConditionType => Str
...
Each argument is described in detail in: L<Paws::CodePipeline::OverrideStageCondition>
...
owner field. If the action type contains C<AWS> or C<ThirdParty> in the
...
=item TargetPipelineExecutionId => Str


=back

Each argument is described in detail in: L<Paws::CodePipeline::RollbackStage>

Returns: a L<Paws::CodePipeline::RollbackStageOutput> instance
...
=item [SourceRevisions => ArrayRef[L<Paws::CodePipeline::SourceRevisionOverride>]]

=item [Variables => ArrayRef[L<Paws::CodePipeline::PipelineVariable>]]
...
=head2 ListAllDeployActionExecutionTargets(sub { },ActionExecutionId => Str, [Filters => ArrayRef[L<Paws::CodePipeline::TargetFilter>], MaxResults => Int, NextToken => Str, PipelineName => Str])

=head2 ListAllDeployActionExecutionTargets(ActionExecutionId => Str, [Filters => ArrayRef[L<Paws::CodePipeline::TargetFilter>], MaxResults => Int, NextToken => Str, PipelineName => Str])
...
If not, it will return a a L<Paws::CodePipeline::ListDeployActionExecutionTargetsOutput> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllPipelineExecutions(sub { },PipelineName => Str, [Filter => L<Paws::CodePipeline::PipelineExecutionFilter>, MaxResults => Int, NextToken => Str])

=head2 ListAllPipelineExecutions(PipelineName => Str, [Filter => L<Paws::CodePipeline::PipelineExecutionFilter>, MaxResults => Int, NextToken => Str])
...
=head2 ListAllRuleExecutions(sub { },PipelineName => Str, [Filter => L<Paws::CodePipeline::RuleExecutionFilter>, MaxResults => Int, NextToken => Str])

=head2 ListAllRuleExecutions(PipelineName => Str, [Filter => L<Paws::CodePipeline::RuleExecutionFilter>, MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - ruleExecutionDetails, passing the object as the first parameter, and the string 'ruleExecutionDetails' as the second parameter 

If not, it will return a a L<Paws::CodePipeline::ListRuleExecutionsOutput> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
