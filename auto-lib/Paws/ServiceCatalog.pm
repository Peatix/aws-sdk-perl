package Paws::ServiceCatalog;
  ...
  sub NotifyProvisionProductEngineWorkflowResult {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalog::NotifyProvisionProductEngineWorkflowResult', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub NotifyTerminateProvisionedProductEngineWorkflowResult {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalog::NotifyTerminateProvisionedProductEngineWorkflowResult', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub NotifyUpdateProvisionedProductEngineWorkflowResult {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ServiceCatalog::NotifyUpdateProvisionedProductEngineWorkflowResult', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
C<PrincipalARN> association is included in the share.

The C<PortfolioID>, C<PrincipalARN>, and C<PrincipalType> parameters
are required.

You can associate a maximum of 10 Principals with a portfolio using
C<PrincipalType> as C<IAM_PATTERN>.
...
accounts. For a user in a recipient account who is I<not> an Service
...
is a concern, then Service Catalog recommends using C<PrincipalType> as
C<IAM>. With this configuration, the C<PrincipalARN> must already exist
...
=item [SourceConnection => L<Paws::ServiceCatalog::SourceConnection>]
...
=item [IncludeProvisioningArtifactParameters => Bool]
...
The C<PrincipalType> and C<PrincipalARN> must match the
C<AssociatePrincipalWithPortfolio> call request details. For example,
to disassociate an association created with a C<PrincipalARN> of
C<PrincipalType> IAM you must use the C<PrincipalType> IAM when calling
...
C<IAM>, the same principal will still have access to the portfolio if
it matches one of the associated principals of type C<IAM_PATTERN>. To
fully remove access for a principal, verify all the associated
Principals of type C<IAM_PATTERN>, and then ensure you disassociate any
C<IAM_PATTERN> principals that match the principal whose access you are
removing.
...
C<organizations:EnableAWSServiceAccess> on your behalf so that your
...
C<UPDATE_ROLLBACK_COMPLETE>, C<IMPORT_COMPLETE>, and
...
after you call C<UpdateProvisionedProduct> for the provisioned product.
...
CloudFormation stack must have the C<IMPORT_COMPLETE> status before you
import another.
...
accepted by this account. By specifying the C<PortfolioShareType>, you
...
in the I<Service Catalog User Guide>.
...
Lists all C<PrincipalARN>s and corresponding C<PrincipalType>s
...
=head2 NotifyProvisionProductEngineWorkflowResult
...
=item WorkflowToken => Str

=item [FailureReason => Str]

=item [Outputs => ArrayRef[L<Paws::ServiceCatalog::RecordOutput>]]

=item [ResourceIdentifier => L<Paws::ServiceCatalog::EngineWorkflowResourceIdentifier>]


=back

Each argument is described in detail in: L<Paws::ServiceCatalog::NotifyProvisionProductEngineWorkflowResult>

Returns: a L<Paws::ServiceCatalog::NotifyProvisionProductEngineWorkflowResultOutput> instance

Notifies the result of the provisioning engine execution.


=head2 NotifyTerminateProvisionedProductEngineWorkflowResult
...
Each argument is described in detail in: L<Paws::ServiceCatalog::NotifyTerminateProvisionedProductEngineWorkflowResult>

Returns: a L<Paws::ServiceCatalog::NotifyTerminateProvisionedProductEngineWorkflowResultOutput> instance

Notifies the result of the terminate engine execution.


=head2 NotifyUpdateProvisionedProductEngineWorkflowResult
...
Each argument is described in detail in: L<Paws::ServiceCatalog::NotifyUpdateProvisionedProductEngineWorkflowResult>

Returns: a L<Paws::ServiceCatalog::NotifyUpdateProvisionedProductEngineWorkflowResultOutput> instance
...
template launches an CloudFormation stack and its underlying resources.
...
or disable C<TagOptions> sharing or Principal sharing for an existing
portfolio share.
