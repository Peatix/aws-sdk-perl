package Paws::MWAA;
  ...
  sub InvokeRestApi {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::MWAA::InvokeRestApi', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub operations { qw/CreateCliToken CreateEnvironment CreateWebLoginToken DeleteEnvironment GetEnvironment InvokeRestApi ListEnvironments ListTagsForResource PublishMetrics TagResource UntagResource UpdateEnvironment / }
...
C<api.airflow.{region}.amazonaws.com> - This endpoint is used for
...
C<env.airflow.{region}.amazonaws.com> - This endpoint is used to
...
(https://docs.aws.amazon.com/general/latest/gr/mwaa.html) in the
...
=item [AirflowConfigurationOptions => L<Paws::MWAA::AirflowConfigurationOptions>]

=item [AirflowVersion => Str]

=item [EndpointManagement => Str]
...
=item [MaxWebservers => Int]

=item [MaxWorkers => Int]

=item [MinWebservers => Int]
...
=item [StartupScriptS3ObjectVersion => Str]

=item [StartupScriptS3Path => Str]
...
Creating an Apache Airflow web login token
...
=item [Body => L<Paws::MWAA::RestApiRequestBody>]

=item [QueryParameters => L<Paws::MWAA::Document>]


=back

Each argument is described in detail in: L<Paws::MWAA::InvokeRestApi>

Returns: a L<Paws::MWAA::InvokeRestApiResponse> instance
...
B<Internal only>. Publishes environment health metrics to Amazon
CloudWatch.
