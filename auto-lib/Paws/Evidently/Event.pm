...
As an example, if Att1 is expected to be a Paws::Evidently::Event object:

  $service_obj->Method(Att1 => { Data => $value, ..., Type => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Evidently::Event object:
...
C<aws.evidently.evaluation> specifies an evaluation event, which
determines which feature variation that a user sees.
C<aws.evidently.custom> specifies a custom event, which generates
