...
package Paws::Notifications::SourceEventMetadataSummary;
  use Moose;
...
Paws::Notifications::SourceEventMetadataSummary
...
As an example, if Att1 is expected to be a Paws::Notifications::SourceEventMetadataSummary object:

  $service_obj->Method(Att1 => { EventOriginRegion => $value, ..., Source => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Notifications::SourceEventMetadataSummary object:
...
Contains metadata about the event that caused the C<NotificationEvent>.
For other specific values, see C<sourceEventMetadata>.
