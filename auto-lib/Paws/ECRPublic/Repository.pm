...
package Paws::ECRPublic::Repository;
  use Moose;
...
As an example, if Att1 is expected to be a Paws::ECRPublic::Repository object:

  $service_obj->Method(Att1 => { CreatedAt => $value, ..., RepositoryUri => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ECRPublic::Repository object:
