...
As an example, if Att1 is expected to be a Paws::ECRPublic::Layer object:

  $service_obj->Method(Att1 => { LayerAvailability => $value, ..., MediaType => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ECRPublic::Layer object:
