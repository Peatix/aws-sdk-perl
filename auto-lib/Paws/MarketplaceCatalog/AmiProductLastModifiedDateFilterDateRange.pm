...
package Paws::MarketplaceCatalog::AmiProductLastModifiedDateFilterDateRange;
  use Moose;
...
Paws::MarketplaceCatalog::AmiProductLastModifiedDateFilterDateRange
...
As an example, if Att1 is expected to be a Paws::MarketplaceCatalog::AmiProductLastModifiedDateFilterDateRange object:

  $service_obj->Method(Att1 => { AfterValue => $value, ..., BeforeValue => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::MarketplaceCatalog::AmiProductLastModifiedDateFilterDateRange object:
