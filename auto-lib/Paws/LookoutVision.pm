package Paws::LookoutVision;
  ...
  sub DescribeModelPackagingJob {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::LookoutVision::DescribeModelPackagingJob', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub DescribeProject {
    my $self = shift;
  ...
  sub ListModelPackagingJobs {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::LookoutVision::ListModelPackagingJobs', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListModels {
    my $self = shift;
  ...
  sub StartModelPackagingJob {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::LookoutVision::StartModelPackagingJob', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub StopModel {
    my $self = shift;
  ...
  sub ListAllModelPackagingJobs {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListModelPackagingJobs(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListModelPackagingJobs(@_, NextToken => $next_result->NextToken);
        push @{ $result->ModelPackagingJobs }, @{ $next_result->ModelPackagingJobs };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'ModelPackagingJobs') foreach (@{ $result->ModelPackagingJobs });
        $result = $self->ListModelPackagingJobs(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'ModelPackagingJobs') foreach (@{ $result->ModelPackagingJobs });
    }
  ...
  sub operations { qw/CreateDataset CreateModel CreateProject DeleteDataset DeleteModel DeleteProject DescribeDataset DescribeModel DescribeModelPackagingJob DescribeProject DetectAnomalies ListDatasetEntries ListModelPackagingJobs ListModels ListProjects ListTagsForResource StartModel StartModelPackagingJob StopModel TagResource UntagResource UpdateDatasetEntries / }
...
(C<ModelVersion>) is in the C<Models> array.
...
=head2 DescribeModelPackagingJob
...
Each argument is described in detail in: L<Paws::LookoutVision::DescribeModelPackagingJob>

Returns: a L<Paws::LookoutVision::DescribeModelPackagingJobResponse> instance

Describes an Amazon Lookout for Vision model packaging job.

This operation requires permissions to perform the
C<lookoutvision:DescribeModelPackagingJob> operation.

For more information, see I<Using your Amazon Lookout for Vision model
...
For more information, see I<Detecting anomalies in an image> in the
...
Each argument is described in detail in: L<Paws::LookoutVision::ListModelPackagingJobs>

Returns: a L<Paws::LookoutVision::ListModelPackagingJobsResponse> instance

Lists the model packaging jobs created for an Amazon Lookout for Vision
project.

This operation requires permissions to perform the
C<lookoutvision:ListModelPackagingJobs> operation.
...
The C<ListModels> operation is eventually consistent. Recent calls to
C<CreateModel> might take a while to appear in the response from
C<ListProjects>.
...
are in the AWS Region in which you call C<ListProjects>.

The C<ListProjects> operation is eventually consistent. Recent calls to
C<CreateProject> and C<DeleteProject> might take a while to appear in
the response from C<ListProjects>.
...
=head2 StartModelPackagingJob

=over

=item Configuration => L<Paws::LookoutVision::ModelPackagingConfiguration>
...
Each argument is described in detail in: L<Paws::LookoutVision::StartModelPackagingJob>

Returns: a L<Paws::LookoutVision::StartModelPackagingJobResponse> instance
...
C<Status> is C<SUCCEEDED>.

To deploy the component to the target device, use the component name
and component version with the AWS IoT Greengrass CreateDeployment
...
C<lookoutvision:StartModelPackagingJob>
...
C<greengrass:CreateComponentVersion>

=item *

C<greengrass:DescribeComponent>

=item *

(Optional) C<greengrass:TagResource>. Only required if you want to tag
the component.
...
To update an existing JSON Line, use the C<source-ref> field to
...
The images you reference in the C<source-ref> field of a JSON line,
...
=head2 ListAllModelPackagingJobs(sub { },ProjectName => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllModelPackagingJobs(ProjectName => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - ModelPackagingJobs, passing the object as the first parameter, and the string 'ModelPackagingJobs' as the second parameter 

If not, it will return a a L<Paws::LookoutVision::ListModelPackagingJobsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
