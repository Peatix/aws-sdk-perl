# Generated by default/object.tt
package Paws::Connect::EvaluationForm;
  use Moose;
  has CreatedBy => (is => 'ro', isa => 'Str', required => 1);
  has CreatedTime => (is => 'ro', isa => 'Str', required => 1);
  has Description => (is => 'ro', isa => 'Str');
  has EvaluationFormArn => (is => 'ro', isa => 'Str', required => 1);
  has EvaluationFormId => (is => 'ro', isa => 'Str', required => 1);
  has EvaluationFormVersion => (is => 'ro', isa => 'Int', required => 1);
  has Items => (is => 'ro', isa => 'ArrayRef[Paws::Connect::EvaluationFormItem]', required => 1);
  has LastModifiedBy => (is => 'ro', isa => 'Str', required => 1);
  has LastModifiedTime => (is => 'ro', isa => 'Str', required => 1);
  has Locked => (is => 'ro', isa => 'Bool', required => 1);
  has ScoringStrategy => (is => 'ro', isa => 'Paws::Connect::EvaluationFormScoringStrategy');
  has Status => (is => 'ro', isa => 'Str', required => 1);
  has Tags => (is => 'ro', isa => 'Paws::Connect::TagMap');
  has Title => (is => 'ro', isa => 'Str', required => 1);

1;

### main pod documentation begin ###

=head1 NAME

Paws::Connect::EvaluationForm

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::Connect::EvaluationForm object:

  $service_obj->Method(Att1 => { CreatedBy => $value, ..., Title => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Connect::EvaluationForm object:

  $result = $service_obj->Method(...);
  $result->Att1->CreatedBy

=head1 DESCRIPTION

Information about the evaluation form.

=head1 ATTRIBUTES


=head2 B<REQUIRED> CreatedBy => Str

The Amazon Resource Name (ARN) of the user who created the evaluation
form.


=head2 B<REQUIRED> CreatedTime => Str

The timestamp for when the evaluation form was created.


=head2 Description => Str

The description of the evaluation form.


=head2 B<REQUIRED> EvaluationFormArn => Str

The Amazon Resource Name (ARN) for the evaluation form resource.


=head2 B<REQUIRED> EvaluationFormId => Str

The unique identifier for the evaluation form.


=head2 B<REQUIRED> EvaluationFormVersion => Int

A version of the evaluation form.


=head2 B<REQUIRED> Items => ArrayRef[L<Paws::Connect::EvaluationFormItem>]

Items that are part of the evaluation form. The total number of
sections and questions must not exceed 100 each. Questions must be
contained in a section.


=head2 B<REQUIRED> LastModifiedBy => Str

The Amazon Resource Name (ARN) of the user who last updated the
evaluation form.


=head2 B<REQUIRED> LastModifiedTime => Str

The timestamp for when the evaluation form was last updated.


=head2 B<REQUIRED> Locked => Bool

The flag indicating whether the evaluation form is locked for changes.


=head2 ScoringStrategy => L<Paws::Connect::EvaluationFormScoringStrategy>

A scoring strategy of the evaluation form.


=head2 B<REQUIRED> Status => Str

The status of the evaluation form.


=head2 Tags => L<Paws::Connect::TagMap>

The tags used to organize, track, or control access for this resource.
For example, { "Tags": {"key1":"value1", "key2":"value2"} }.


=head2 B<REQUIRED> Title => Str

A title of the evaluation form.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::Connect>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

