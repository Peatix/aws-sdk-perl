package Paws::STS;
  ...
  sub AssumeRoot {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::STS::AssumeRoot', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub operations { qw/AssumeRole AssumeRoleWithSAML AssumeRoleWithWebIdentity AssumeRoot DecodeAuthorizationMessage GetAccessKeyInfo GetCallerIdentity GetFederationToken GetSessionToken / }
...
=item [ProvidedContexts => ArrayRef[L<Paws::STS::ProvidedContext>]]
...
=item [TransitiveTagKeys => ArrayRef[Str|Undef]]
...
Typically, you use C<AssumeRole> within your account or for
cross-account access. For a comparison of C<AssumeRole> with other API
operations that produce temporary credentials, see Requesting Temporary
Security Credentials
(https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html)
and Compare STS credentials
...
role that is being assumed. For more information, see Session Policies
(https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html#policies_session)
in the I<IAM User Guide>.

When you create a role, you create two policies: a role trust policy
that specifies I<who> can assume the role, and a permissions policy
that specifies I<what> can be done with the role. You specify the
...
C<AssumeRole> for the ARN of the role in the other account.
...
Attach a policy to the user that allows the user to call C<AssumeRole>
...
You can do either because the roleE<rsquo>s trust policy acts as an IAM
...
C<AssumeRoleWithSAML> with the other API operations that produce
...
one hour. When you use the C<AssumeRole> API operation to assume a
role, you can specify the duration of your role session with the
C<DurationSeconds> parameter. You can specify a parameter value of up
to 43200 seconds (12 hours), depending on the maximum session duration
setting for your role. However, if you assume a role using role
chaining and provide a C<DurationSeconds> parameter value greater than
...
following exception: you cannot call the STS C<GetFederationToken> or
...
Calling C<AssumeRoleWithSAML> does not require the use of Amazon Web
...
Calling C<AssumeRoleWithSAML> can result in an entry in your CloudTrail
logs. The entry includes the value in the C<NameID> element of the SAML
assertion. We recommend that you use a C<NameIDType> that is not
...
Amazon Web Services account that represents your identity provider. You
must also create an IAM role that specifies this SAML provider in its
trust policy.
...
such as Google or Amazon Cognito federated identities
...
Developer Guide (http://aws.amazon.com/sdkforios/) and the Amazon Web
Services SDK for Android Developer Guide
(http://aws.amazon.com/sdkforandroid/) to uniquely identify a user. You
...
(https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-identity.html)
in I<Amazon Cognito Developer Guide>.

Calling C<AssumeRoleWithWebIdentity> does not require the use of Amazon
...
C<AssumeRoleWithWebIdentity> with the other API operations that produce
temporary credentials, see Requesting Temporary Security Credentials
(https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html)
and Compare STS credentials
(https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_sts-comparison.html)
...
value for your role, see Update the maximum session duration for a role
...
C<AssumeRoleWithWebIdentity> can be used to make API calls to any
Amazon Web Services service with the following exception: you cannot
call the STS C<GetFederationToken> or C<GetSessionToken> API
operations.
...
Calling C<AssumeRoleWithWebIdentity> can result in an entry in your
...
(http://aws.amazon.com/sdkforios/) and Amazon Web Services SDK for
Android Developer Guide (http://aws.amazon.com/sdkforandroid/). These
...
=item TargetPrincipal => Str

=item TaskPolicyArn => L<Paws::STS::PolicyDescriptorType>
...
Each argument is described in detail in: L<Paws::STS::AssumeRoot>

Returns: a L<Paws::STS::AssumeRootResponse> instance
...
user must be granted permissions through an IAM policy
(https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html)
to request the C<DecodeAuthorizationMessage>
(C<sts:DecodeAuthorizationMessage>) action.
...
IDs beginning with C<AKIA> are long-term credentials for an IAM user or
the Amazon Web Services account root user. Access key IDs beginning
with C<ASIA> are temporary credentials that are created using STS
operations. If the account in the response belongs to you, you can sign
in as the root user and review your root user access keys. Then, you
...
access to the C<sts:GetCallerIdentity> action, you can still perform
this operation. Permissions are not required because the same
...
access key ID, a secret access key, and a security token) for a user. A
typical use is in a proxy application that gets temporary security
credentials on behalf of distributed applications inside a corporate
network.

You must call the C<GetFederationToken> operation using the long-term
...
C<GetFederationToken> with the other API operations that produce
temporary credentials, see Requesting Temporary Security Credentials
(https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html)
and Compare STS credentials
(https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_sts-comparison.html)
...
Although it is possible to call C<GetFederationToken> using the
security credentials of an Amazon Web Services account root user rather
than an IAM user that you create for the purpose of a proxy
...
C<department> tag keys. Assume that the user that you are federating
has the C<Department>=C<Marketing> tag and you pass the
...
C<GetSessionToken> if you want to use MFA to protect programmatic calls
to specific Amazon Web Services API operations like Amazon EC2
C<StopInstances>.

MFA-enabled IAM users must call C<GetSessionToken> and submit an MFA
...
For a comparison of C<GetSessionToken> with the other API operations
...
purpose of the C<sts:GetSessionToken> operation is to authenticate the
...
The credentials that C<GetSessionToken> returns are based on
...
Although it is possible to call C<GetSessionToken> using the security
credentials of an Amazon Web Services account root user rather than an
IAM user, we do not recommend it. If C<GetSessionToken> is called using
