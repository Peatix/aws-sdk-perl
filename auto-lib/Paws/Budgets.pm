package Paws::Budgets;
  ...
  sub DescribeBudgetNotificationsForAccount {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Budgets::DescribeBudgetNotificationsForAccount', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListTagsForResource {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Budgets::ListTagsForResource', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub TagResource {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Budgets::TagResource', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub UntagResource {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Budgets::UntagResource', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub UpdateBudget {
    my $self = shift;
  ...
  sub DescribeAllBudgetNotificationsForAccount {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->DescribeBudgetNotificationsForAccount(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->DescribeBudgetNotificationsForAccount(@_, NextToken => $next_result->NextToken);
        push @{ $result->BudgetNotificationsForAccount }, @{ $next_result->BudgetNotificationsForAccount };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'BudgetNotificationsForAccount') foreach (@{ $result->BudgetNotificationsForAccount });
        $result = $self->DescribeBudgetNotificationsForAccount(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'BudgetNotificationsForAccount') foreach (@{ $result->BudgetNotificationsForAccount });
    }
...
=item [ResourceTags => ArrayRef[L<Paws::Budgets::ResourceTag>]]
...
in the syntax at one time. Either C<FilterExpression> and C<Metrics> or
C<CostFilters> and C<CostTypes>, not both or a different combination.
We recommend using C<FilterExpression> and C<Metrics> as they provide
more flexible and powerful filtering capabilities. The Request Syntax
section shows the C<FilterExpression>/C<Metrics> syntax.
...
=item [ShowFilterExpression => Bool]
...
=head2 DescribeBudgetNotificationsForAccount
...
Each argument is described in detail in: L<Paws::Budgets::DescribeBudgetNotificationsForAccount>

Returns: a L<Paws::Budgets::DescribeBudgetNotificationsForAccountResponse> instance
...
Each argument is described in detail in: L<Paws::Budgets::ListTagsForResource>

Returns: a L<Paws::Budgets::ListTagsForResourceResponse> instance
...
Each argument is described in detail in: L<Paws::Budgets::TagResource>

Returns: a L<Paws::Budgets::TagResourceResponse> instance
...
Each argument is described in detail in: L<Paws::Budgets::UntagResource>

Returns: a L<Paws::Budgets::UntagResourceResponse> instance
...
C<calculatedSpend> drops to zero until Amazon Web Services has new
...
=head2 DescribeAllBudgetNotificationsForAccount(sub { },AccountId => Str, [MaxResults => Int, NextToken => Str])

=head2 DescribeAllBudgetNotificationsForAccount(AccountId => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - BudgetNotificationsForAccount, passing the object as the first parameter, and the string 'BudgetNotificationsForAccount' as the second parameter 

If not, it will return a a L<Paws::Budgets::DescribeBudgetNotificationsForAccountResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 DescribeAllBudgets(sub { },AccountId => Str, [MaxResults => Int, NextToken => Str, ShowFilterExpression => Bool])

=head2 DescribeAllBudgets(AccountId => Str, [MaxResults => Int, NextToken => Str, ShowFilterExpression => Bool])
