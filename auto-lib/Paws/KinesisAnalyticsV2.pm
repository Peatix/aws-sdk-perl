package Paws::KinesisAnalyticsV2;
  ...
  sub DescribeApplicationOperation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::KinesisAnalyticsV2::DescribeApplicationOperation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListApplicationOperations {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::KinesisAnalyticsV2::ListApplicationOperations', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllApplicationOperations {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListApplicationOperations(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListApplicationOperations(@_, NextToken => $next_result->NextToken);
        push @{ $result->ApplicationOperationInfoList }, @{ $next_result->ApplicationOperationInfoList };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'ApplicationOperationInfoList') foreach (@{ $result->ApplicationOperationInfoList });
        $result = $self->ListApplicationOperations(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'ApplicationOperationInfoList') foreach (@{ $result->ApplicationOperationInfoList });
    }
  ...
  sub ListAllApplicationVersions {
    ...
    if (not defined $callback) {
      while ($next_result->NextToken) {
        ...
        push @{ $result->ApplicationVersionSummaries }, @{ $next_result->ApplicationVersionSummaries };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'ApplicationVersionSummaries') foreach (@{ $result->ApplicationVersionSummaries });
...
=head2 DescribeApplicationOperation
...
Each argument is described in detail in: L<Paws::KinesisAnalyticsV2::DescribeApplicationOperation>

Returns: a L<Paws::KinesisAnalyticsV2::DescribeApplicationOperationResponse> instance
...
=head2 ListApplicationOperations
...
Each argument is described in detail in: L<Paws::KinesisAnalyticsV2::ListApplicationOperations>

Returns: a L<Paws::KinesisAnalyticsV2::ListApplicationOperationsResponse> instance
...
You can roll back an application only if it is in the C<UPDATING>,
C<AUTOSCALING>, or C<RUNNING> statuses.
...
After creating an application, you must exclusively call this operation
...
is stopped, unless C<Force> is set to C<true>.
...
application. For more information, see Using Tagging
(https://docs.aws.amazon.com/kinesisanalytics/latest/java/how-tagging.html).
...
=item [RuntimeEnvironmentUpdate => Str]
...
Managed Service for Apache Flink updates the C<ApplicationVersionId>
...
=head2 ListAllApplicationOperations(sub { },ApplicationName => Str, [Limit => Int, NextToken => Str, Operation => Str, OperationStatus => Str])

=head2 ListAllApplicationOperations(ApplicationName => Str, [Limit => Int, NextToken => Str, Operation => Str, OperationStatus => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - ApplicationOperationInfoList, passing the object as the first parameter, and the string 'ApplicationOperationInfoList' as the second parameter 

If not, it will return a a L<Paws::KinesisAnalyticsV2::ListApplicationOperationsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllApplicationVersions(sub { },ApplicationName => Str, [Limit => Int, NextToken => Str])

=head2 ListAllApplicationVersions(ApplicationName => Str, [Limit => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - ApplicationVersionSummaries, passing the object as the first parameter, and the string 'ApplicationVersionSummaries' as the second parameter 

If not, it will return a a L<Paws::KinesisAnalyticsV2::ListApplicationVersionsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
