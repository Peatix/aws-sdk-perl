...
package Paws::B2bi::CreateCapability;
  use Moose;
  ...
  class_has _api_call => (isa => 'Str', is => 'ro', default => 'CreateCapability');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::B2bi::CreateCapabilityResponse');
...
Paws::B2bi::CreateCapability - Arguments for method CreateCapability on L<Paws::B2bi>

=head1 DESCRIPTION

This class represents the parameters used for calling the method CreateCapability on the
L<AWS B2B Data Interchange|Paws::B2bi> service. Use the attributes of this class
as arguments to method CreateCapability.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to CreateCapability.

=head1 SYNOPSIS

    my $b2bi = Paws->service('B2bi');
    my $CreateCapabilityResponse = $b2bi->CreateCapability(
      Configuration => {
        Edi => {
          InputLocation => {
    ...
    # Results:
    my $CapabilityArn = $CreateCapabilityResponse->CapabilityArn;
    my $CapabilityId  = $CreateCapabilityResponse->CapabilityId;
    my $Configuration = $CreateCapabilityResponse->Configuration;
    my $CreatedAt     = $CreateCapabilityResponse->CreatedAt;
    my $InstructionsDocuments =
      $CreateCapabilityResponse->InstructionsDocuments;
    my $Name = $CreateCapabilityResponse->Name;
    my $Type = $CreateCapabilityResponse->Type;

    # Returns a L<Paws::B2bi::CreateCapabilityResponse> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/b2bi/CreateCapability>
...
Specifies the type of the capability. Currently, only C<edi> is
supported.
...
This class forms part of L<Paws>, documenting arguments for method CreateCapability in L<Paws::B2bi>
