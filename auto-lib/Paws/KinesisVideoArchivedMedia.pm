package Paws::KinesisVideoArchivedMedia;
  ...
  sub GetImages {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::KinesisVideoArchivedMedia::GetImages', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetAllImages {
    ...
    my $result = $self->GetImages(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->GetImages(@_, NextToken => $next_result->NextToken);
      ...
      while ($result->NextToken) {
        ...
        $result = $self->GetImages(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'Images') foreach (@{ $result->Images });
    }
  ...
  sub operations { qw/GetClip GetDASHStreamingSessionURL GetHLSStreamingSessionURL GetImages GetMediaForFragmentList ListFragments / }
...
Services Pricing (https://aws.amazon.com/pricing/). Charges for
...
the token with the same measures that you use with your Amazon Web
...
For pricing information, see Amazon Kinesis Video Streams Pricing
(https://aws.amazon.com/kinesis/video-streams/pricing/) and Amazon Web
Services Pricing (https://aws.amazon.com/pricing/). Charges for both
HLS sessions and outgoing Amazon Web Services data apply.
...
video and, optionally, AAC encoded audio. Specifically, the codec ID of
track 1 should be C<V_MPEG/ISO/AVC> (for H.264) or C<V_MPEG/ISO/HEVC>
(for H.265). Optionally, the codec ID of track 2 should be C<A_AAC>.
For audio only streaming, the codec ID of track 1 should be C<A_AAC>.
...
changes are supported. After the first media fragment is made available
in a streaming session, fragments can contain CPD changes for each
track. Therefore, the fragments in a session can have a different
resolution, bit rate, or other information in the CPD without
interrupting playback. However, any change made in the track number or
track codec format can return an error when those different media
fragments are loaded. For example, streaming will fail if the fragments
in the stream change from having only video to having both audio and
video, or if an AAC audio track is changed to an ALAW audio track. For
...
C<x-amz-RequestId> HTTP header E<ndash> if you want to report an issue
to Amazon Web Services, the support team can better diagnose the
...
=item EndTimestamp => Str

=item Format => Str

=item ImageSelectorType => Str

=item StartTimestamp => Str

=item [FormatConfig => L<Paws::KinesisVideoArchivedMedia::FormatConfig>]

=item [HeightPixels => Int]

=item [MaxResults => Int]

=item [NextToken => Str]

=item [SamplingInterval => Int]

=item [StreamARN => Str]

=item [StreamName => Str]

=item [WidthPixels => Int]


=back

Each argument is described in detail in: L<Paws::KinesisVideoArchivedMedia::GetImages>

Returns: a L<Paws::KinesisVideoArchivedMedia::GetImagesOutput> instance
...
=head2 GetAllImages(sub { },EndTimestamp => Str, Format => Str, ImageSelectorType => Str, StartTimestamp => Str, [FormatConfig => L<Paws::KinesisVideoArchivedMedia::FormatConfig>, HeightPixels => Int, MaxResults => Int, NextToken => Str, SamplingInterval => Int, StreamARN => Str, StreamName => Str, WidthPixels => Int])

=head2 GetAllImages(EndTimestamp => Str, Format => Str, ImageSelectorType => Str, StartTimestamp => Str, [FormatConfig => L<Paws::KinesisVideoArchivedMedia::FormatConfig>, HeightPixels => Int, MaxResults => Int, NextToken => Str, SamplingInterval => Int, StreamARN => Str, StreamName => Str, WidthPixels => Int])
...
If not, it will return a a L<Paws::KinesisVideoArchivedMedia::GetImagesOutput> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
