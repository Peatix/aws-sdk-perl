package Paws::StorageGateway;
  ...
  sub CancelCacheReport {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::CancelCacheReport', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub CancelRetrieval {
    my $self = shift;
  ...
  sub DeleteCacheReport {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::DeleteCacheReport', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeCacheReport {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::DescribeCacheReport', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub EvictFilesFailingUpload {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::EvictFilesFailingUpload', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub JoinDomain {
    my $self = shift;
  ...
  sub ListCacheReports {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::ListCacheReports', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListFileShares {
    my $self = shift;
  ...
  sub StartCacheReport {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::StartCacheReport', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub StartGateway {
    my $self = shift;
  ...
  sub UpdateSMBLocalGroups {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::StorageGateway::UpdateSMBLocalGroups', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllCacheReports {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListCacheReports(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->Marker) {
        $next_result = $self->ListCacheReports(@_, Marker => $next_result->Marker);
        push @{ $result->CacheReportList }, @{ $next_result->CacheReportList };
      }
      return $result;
    } else {
      while ($result->Marker) {
        $callback->($_ => 'CacheReportList') foreach (@{ $result->CacheReportList });
        $result = $self->ListCacheReports(@_, Marker => $result->Marker);
      }
      $callback->($_ => 'CacheReportList') foreach (@{ $result->CacheReportList });
    }
  ...
  sub operations { qw/ActivateGateway AddCache AddTagsToResource AddUploadBuffer AddWorkingStorage AssignTapePool AssociateFileSystem AttachVolume CancelArchival CancelCacheReport CancelRetrieval CreateCachediSCSIVolume CreateNFSFileShare CreateSMBFileShare CreateSnapshot CreateSnapshotFromVolumeRecoveryPoint CreateStorediSCSIVolume CreateTapePool CreateTapes CreateTapeWithBarcode DeleteAutomaticTapeCreationPolicy DeleteBandwidthRateLimit DeleteCacheReport DeleteChapCredentials DeleteFileShare DeleteGateway DeleteSnapshotSchedule DeleteTape DeleteTapeArchive DeleteTapePool DeleteVolume DescribeAvailabilityMonitorTest DescribeBandwidthRateLimit DescribeBandwidthRateLimitSchedule DescribeCache DescribeCachediSCSIVolumes DescribeCacheReport DescribeChapCredentials DescribeFileSystemAssociations DescribeGatewayInformation DescribeMaintenanceStartTime DescribeNFSFileShares DescribeSMBFileShares DescribeSMBSettings DescribeSnapshotSchedule DescribeStorediSCSIVolumes DescribeTapeArchives DescribeTapeRecoveryPoints DescribeTapes DescribeUploadBuffer DescribeVTLDevices DescribeWorkingStorage DetachVolume DisableGateway DisassociateFileSystem EvictFilesFailingUpload JoinDomain ListAutomaticTapeCreationPolicies ListCacheReports ListFileShares ListFileSystemAssociations ListGateways ListLocalDisks ListTagsForResource ListTapePools ListTapes ListVolumeInitiators ListVolumeRecoveryPoints ListVolumes NotifyWhenUploaded RefreshCache RemoveTagsFromResource ResetCache RetrieveTapeArchive RetrieveTapeRecoveryPoint SetLocalConsolePassword SetSMBGuestPassword ShutdownGateway StartAvailabilityMonitorTest StartCacheReport StartGateway UpdateAutomaticTapeCreationPolicy UpdateBandwidthRateLimit UpdateBandwidthRateLimitSchedule UpdateChapCredentials UpdateFileSystemAssociation UpdateGatewayInformation UpdateGatewaySoftwareNow UpdateMaintenanceStartTime UpdateNFSFileShare UpdateSMBFileShare UpdateSMBFileShareVisibility UpdateSMBLocalGroups UpdateSMBSecurityStrategy UpdateSnapshotSchedule UpdateVTLDeviceType / }
...
Use the following links to get started using the I<Storage Gateway
...
list of each Amazon Web Services Region and the endpoints available for
use with Storage Gateway.
...
For more information, see Announcement: Heads-up E<ndash> Longer
...
=item [EndpointNetworkConfiguration => L<Paws::StorageGateway::EndpointNetworkConfiguration>]
...
=head2 CancelCacheReport

=over

=item CacheReportARN => Str


=back

Each argument is described in detail in: L<Paws::StorageGateway::CancelCacheReport>

Returns: a L<Paws::StorageGateway::CancelCacheReportOutput> instance
...
only cancel in-progress reports. If the the report you attempt to
cancel is in FAILED, ERROR, or COMPLETED state, the cancel operation
returns an error.
...
when you want to create a volume from a snapshot. This operation is
...
Each argument is described in detail in: L<Paws::StorageGateway::DeleteCacheReport>

Returns: a L<Paws::StorageGateway::DeleteCacheReportOutput> instance
...
status of the report you attempt to delete still IN-PROGRESS, the
delete operation returns an error. You can use C<CancelCacheReport> to
cancel an IN-PROGRESS report.

C<DeleteCacheReport> does not delete the report object from your Amazon
S3 bucket.
...
Each argument is described in detail in: L<Paws::StorageGateway::DescribeCacheReport>

Returns: a L<Paws::StorageGateway::DescribeCacheReportOutput> instance
...
If a specific C<TapeARN> is not specified, Storage Gateway returns a
...
Amazon Resource Names (ARNs). If C<TapeARN> is not specified, returns a
...
includes a C<Marker> field. You can use this C<Marker> value in your
...
=head2 EvictFilesFailingUpload

=over

=item FileShareARN => Str

=item [ForceRemove => Bool]


=back

Each argument is described in detail in: L<Paws::StorageGateway::EvictFilesFailingUpload>

Returns: a L<Paws::StorageGateway::EvictFilesFailingUploadOutput> instance
...
there are no other cache clean operations currently in-progress for the
specified file share. After a successful request, the cache clean
operation occurs asynchronously and reports progress using CloudWatch
logs and notifications.

If C<ForceRemove> is set to C<True>, the cache clean operation will
delete file data from the gateway which might otherwise be recoverable.
We recommend using this operation only after all other methods to clear
...
default organizational unit, using the gateway's B<Gateway ID> as the
...
Each argument is described in detail in: L<Paws::StorageGateway::ListCacheReports>

Returns: a L<Paws::StorageGateway::ListCacheReportsOutput> instance
...
information provided by the C<DescribeCacheReport> action, such as
...
when your C<RefreshCache> operation completes. For more information,
see Getting notified about file operations
(https://docs.aws.amazon.com/filegateway/latest/files3/monitoring-file-gateway.html#get-notification)
in the I<Amazon S3 File Gateway User Guide>. This operation is Only
supported for S3 File Gateways.
...
C<GuestAccess>. This operation only supported for S3 File Gateways
...
=item [ExclusionFilters => ArrayRef[L<Paws::StorageGateway::CacheReportFilter>]]

=item [InclusionFilters => ArrayRef[L<Paws::StorageGateway::CacheReportFilter>]]
...
Each argument is described in detail in: L<Paws::StorageGateway::StartCacheReport>

Returns: a L<Paws::StorageGateway::StartCacheReportOutput> instance
...
The B<Files Failing Upload> flag is reset every 24 hours and during
gateway reboot. If this report captures the files after the reset, but
before they become flagged again, they will not be reported as B<Files
Failing Upload>.
...
No other cache reports can currently be in-progress for the specified
file share.
...
You must specify at least one value for C<InclusionFilters> or
C<ExclusionFilters> in the request.
...
=item [SoftwareUpdatePreferences => L<Paws::StorageGateway::SoftwareUpdatePreferences>]
...
maintenance schedule is passed, the entire request will be rejected
with an error and no changes will occur.

A complete maintenance schedule must include values for I<both>
C<MinuteOfHour> and C<HourOfDay>, and I<either> C<DayOfMonth> I<or>
C<DayOfWeek>.
...
=head2 UpdateSMBLocalGroups

=over

=item GatewayARN => Str

=item SMBLocalGroups => L<Paws::StorageGateway::SMBLocalGroups>


=back

Each argument is described in detail in: L<Paws::StorageGateway::UpdateSMBLocalGroups>

Returns: a L<Paws::StorageGateway::UpdateSMBLocalGroupsOutput> instance
...
Services console, see Setting a security level for your gateway
...
=head2 ListAllCacheReports(sub { },[Marker => Str])

=head2 ListAllCacheReports([Marker => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - CacheReportList, passing the object as the first parameter, and the string 'CacheReportList' as the second parameter 

If not, it will return a a L<Paws::StorageGateway::ListCacheReportsOutput> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
