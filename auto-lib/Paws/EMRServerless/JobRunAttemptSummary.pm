# Generated by default/object.tt
package Paws::EMRServerless::JobRunAttemptSummary;
  use Moose;
  has ApplicationId => (is => 'ro', isa => 'Str', request_name => 'applicationId', traits => ['NameInRequest'], required => 1);
  has Arn => (is => 'ro', isa => 'Str', request_name => 'arn', traits => ['NameInRequest'], required => 1);
  has Attempt => (is => 'ro', isa => 'Int', request_name => 'attempt', traits => ['NameInRequest']);
  has CreatedAt => (is => 'ro', isa => 'Str', request_name => 'createdAt', traits => ['NameInRequest'], required => 1);
  has CreatedBy => (is => 'ro', isa => 'Str', request_name => 'createdBy', traits => ['NameInRequest'], required => 1);
  has ExecutionRole => (is => 'ro', isa => 'Str', request_name => 'executionRole', traits => ['NameInRequest'], required => 1);
  has Id => (is => 'ro', isa => 'Str', request_name => 'id', traits => ['NameInRequest'], required => 1);
  has JobCreatedAt => (is => 'ro', isa => 'Str', request_name => 'jobCreatedAt', traits => ['NameInRequest'], required => 1);
  has Mode => (is => 'ro', isa => 'Str', request_name => 'mode', traits => ['NameInRequest']);
  has Name => (is => 'ro', isa => 'Str', request_name => 'name', traits => ['NameInRequest']);
  has ReleaseLabel => (is => 'ro', isa => 'Str', request_name => 'releaseLabel', traits => ['NameInRequest'], required => 1);
  has State => (is => 'ro', isa => 'Str', request_name => 'state', traits => ['NameInRequest'], required => 1);
  has StateDetails => (is => 'ro', isa => 'Str', request_name => 'stateDetails', traits => ['NameInRequest'], required => 1);
  has Type => (is => 'ro', isa => 'Str', request_name => 'type', traits => ['NameInRequest']);
  has UpdatedAt => (is => 'ro', isa => 'Str', request_name => 'updatedAt', traits => ['NameInRequest'], required => 1);

1;

### main pod documentation begin ###

=head1 NAME

Paws::EMRServerless::JobRunAttemptSummary

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::EMRServerless::JobRunAttemptSummary object:

  $service_obj->Method(Att1 => { ApplicationId => $value, ..., UpdatedAt => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::EMRServerless::JobRunAttemptSummary object:

  $result = $service_obj->Method(...);
  $result->Att1->ApplicationId

=head1 DESCRIPTION

The summary of attributes associated with a job run attempt.

=head1 ATTRIBUTES


=head2 B<REQUIRED> ApplicationId => Str

The ID of the application the job is running on.


=head2 B<REQUIRED> Arn => Str

The Amazon Resource Name (ARN) of the job run.


=head2 Attempt => Int

The attempt number of the job run execution.


=head2 B<REQUIRED> CreatedAt => Str

The date and time when the job run attempt was created.


=head2 B<REQUIRED> CreatedBy => Str

The user who created the job run.


=head2 B<REQUIRED> ExecutionRole => Str

The Amazon Resource Name (ARN) of the execution role of the job run..


=head2 B<REQUIRED> Id => Str

The ID of the job run attempt.


=head2 B<REQUIRED> JobCreatedAt => Str

The date and time of when the job run was created.


=head2 Mode => Str

The mode of the job run attempt.


=head2 Name => Str

The name of the job run attempt.


=head2 B<REQUIRED> ReleaseLabel => Str

The Amazon EMR release label of the job run attempt.


=head2 B<REQUIRED> State => Str

The state of the job run attempt.


=head2 B<REQUIRED> StateDetails => Str

The state details of the job run attempt.


=head2 Type => Str

The type of the job run, such as Spark or Hive.


=head2 B<REQUIRED> UpdatedAt => Str

The date and time of when the job run attempt was last updated.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::EMRServerless>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

