...
=head1 SYNOPSIS
    ...
    # Results:
    my $DBSnapshotAttributesResult =
      $ModifyDBSnapshotAttributeResult->DBSnapshotAttributesResult;

    # Returns a L<Paws::RDS::ModifyDBSnapshotAttributeResult> object.
...
Services account IDs, or C<all> to make the manual DB snapshot
restorable by any Amazon Web Services account. Do not add the C<all>
...
Amazon Web Services account identifiers, or C<all> to remove
authorization for any Amazon Web Services account to copy or restore
the DB snapshot. If you specify C<all>, an Amazon Web Services account
