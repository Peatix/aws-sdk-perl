...
package Paws::MediaPackageV2::CreateDashManifestConfiguration;
  use Moose;
...
Paws::MediaPackageV2::CreateDashManifestConfiguration
...
As an example, if Att1 is expected to be a Paws::MediaPackageV2::CreateDashManifestConfiguration object:

  $service_obj->Method(Att1 => { BaseUrls => $value, ..., UtcTiming => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::MediaPackageV2::CreateDashManifestConfiguration object:
...
For information about compactness, see DASH manifest compactness
(https://docs.aws.amazon.com/mediapackage/latest/userguide/compacted.html)
in the I<Elemental MediaPackage v2 User Guide>.
...
name creates a unique path to this endpoint.
...
separates the MPEG-DASH manifest into multiple periods. Type C<ADS> to
indicate that AWS Elemental MediaPackage must create periods in the
output manifest that correspond to SCTE-35 ad markers in the input
source. Leave this value empty to indicate that the manifest is
contained all in one period. For more information about periods in the
DASH manifest, see Multi-period DASH in AWS Elemental MediaPackage
