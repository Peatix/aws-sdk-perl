...
package Paws::MediaPackageV2::CreateLowLatencyHlsManifestConfiguration;
  use Moose;
...
Paws::MediaPackageV2::CreateLowLatencyHlsManifestConfiguration
...
As an example, if Att1 is expected to be a Paws::MediaPackageV2::CreateLowLatencyHlsManifestConfiguration object:

  $service_obj->Method(Att1 => { ChildManifestName => $value, ..., UrlEncodeChildManifest => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::MediaPackageV2::CreateLowLatencyHlsManifestConfiguration object:
...
value, MediaPackage uses the default manifest name, index, with an
added suffix to distinguish it from the manifest name. The manifestName
on the HLSManifest object overrides the manifestName you provided on
