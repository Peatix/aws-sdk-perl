...
package Paws::TaxSettings::BatchPutTaxRegistrationError;
  use Moose;
...
Paws::TaxSettings::BatchPutTaxRegistrationError
...
As an example, if Att1 is expected to be a Paws::TaxSettings::BatchPutTaxRegistrationError object:

  $service_obj->Method(Att1 => { AccountId => $value, ..., Message => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::TaxSettings::BatchPutTaxRegistrationError object:
...
C<BatchPutTaxRegistration> operation.
...
couldn't be added, or updated during the C<BatchPutTaxRegistration>
operation.
