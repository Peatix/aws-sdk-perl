...
package Paws::CodePipeline::RuleRevision;
  use Moose;
...
Paws::CodePipeline::RuleRevision
...
As an example, if Att1 is expected to be a Paws::CodePipeline::RuleRevision object:

  $service_obj->Method(Att1 => { Created => $value, ..., RevisionId => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CodePipeline::RuleRevision object:
...
The date and time when the most recent version of the rule was created,
in timestamp format.
