...
package Paws::CodePipeline::GitTagFilterCriteria;
  use Moose;
...
Paws::CodePipeline::GitTagFilterCriteria
...
As an example, if Att1 is expected to be a Paws::CodePipeline::GitTagFilterCriteria object:

  $service_obj->Method(Att1 => { Excludes => $value, ..., Includes => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CodePipeline::GitTagFilterCriteria object:
...
The list of patterns of Git tags that, when pushed, are to be excluded
from starting the pipeline.


=head2 Includes => ArrayRef[Str|Undef]

The list of patterns of Git tags that, when pushed, are to be included
