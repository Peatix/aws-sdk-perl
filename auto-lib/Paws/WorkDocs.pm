package Paws::WorkDocs;
  ...
  sub DeleteDocumentVersion {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::WorkDocs::DeleteDocumentVersion', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub DeleteFolder {
    my $self = shift;
  ...
  sub RestoreDocumentVersions {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::WorkDocs::RestoreDocumentVersions', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub SearchResources {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::WorkDocs::SearchResources', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub UpdateDocument {
    my $self = shift;
  ...
  sub SearchAllResources {
    ...
    if (not defined $callback) {
      while ($next_result->Marker) {
        $next_result = $self->SearchResources(@_, Marker => $next_result->Marker);
      ...
      while ($result->Marker) {
        ...
        $result = $self->SearchResources(@_, Marker => $result->Marker);
      }
      $callback->($_ => 'Items') foreach (@{ $result->Items });
    }
...
with CloudTrail, to replicate data for eDiscovery, backup, or
...
is not limited to, the ability to modify file permissions and upload
...
C<WRITE (Activate*, Add*, Create*, Deactivate*, Initiate*, Update*)>
...
=head2 DeleteDocumentVersion

=over

=item DeletePriorVersions => Bool
...
Each argument is described in detail in: L<Paws::WorkDocs::DeleteDocumentVersion>
...
Each argument is described in detail in: L<Paws::WorkDocs::RestoreDocumentVersions>
...
=item [AdditionalResponseFields => ArrayRef[Str|Undef]]

=item [AuthenticationToken => Str]

=item [Filters => L<Paws::WorkDocs::Filters>]

=item [Limit => Int]

=item [Marker => Str]

=item [OrderBy => ArrayRef[L<Paws::WorkDocs::SearchSortResult>]]

=item [OrganizationId => Str]

=item [QueryScopes => ArrayRef[Str|Undef]]
...
Each argument is described in detail in: L<Paws::WorkDocs::SearchResources>

Returns: a L<Paws::WorkDocs::SearchResourcesResponse> instance
...
=head2 SearchAllResources(sub { },[AdditionalResponseFields => ArrayRef[Str|Undef], AuthenticationToken => Str, Filters => L<Paws::WorkDocs::Filters>, Limit => Int, Marker => Str, OrderBy => ArrayRef[L<Paws::WorkDocs::SearchSortResult>], OrganizationId => Str, QueryScopes => ArrayRef[Str|Undef], QueryText => Str])

=head2 SearchAllResources([AdditionalResponseFields => ArrayRef[Str|Undef], AuthenticationToken => Str, Filters => L<Paws::WorkDocs::Filters>, Limit => Int, Marker => Str, OrderBy => ArrayRef[L<Paws::WorkDocs::SearchSortResult>], OrganizationId => Str, QueryScopes => ArrayRef[Str|Undef], QueryText => Str])
...
If not, it will return a a L<Paws::WorkDocs::SearchResourcesResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
