...
package Paws::DirectConnect::NewTransitVirtualInterfaceAllocation;
  use Moose;
...
Paws::DirectConnect::NewTransitVirtualInterfaceAllocation
...
As an example, if Att1 is expected to be a Paws::DirectConnect::NewTransitVirtualInterfaceAllocation object:

  $service_obj->Method(Att1 => { AddressFamily => $value, ..., Vlan => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::DirectConnect::NewTransitVirtualInterfaceAllocation object:
