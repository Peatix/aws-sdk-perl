...
C<IS_NOT>. To include the forecasts that match the statement, specify
C<IS>. To exclude matching forecasts, specify C<IS_NOT>.

=item *

C<Key> - The name of the parameter to filter on. Valid values are
C<DatasetGroupArn>, C<PredictorArn>, and C<Status>.
...
C<"Filters": [ { "Condition": "IS_NOT", "Key": "Status", "Value":
"ACTIVE" } ]>
