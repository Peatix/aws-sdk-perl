...
Paws::CostExplorer::SavingsPlansUtilizationAggregates
...
As an example, if Att1 is expected to be a Paws::CostExplorer::SavingsPlansUtilizationAggregates object:

  $service_obj->Method(Att1 => { AmortizedCommitment => $value, ..., Utilization => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CostExplorer::SavingsPlansUtilizationAggregates object:
