...
If C<Values> and C<Key> aren't specified, the C<ABSENT> C<MatchOption>
is applied to all tags. That is, it's filtered on resources with no
tags.

If C<Key> is provided and C<Values> isn't specified, the C<ABSENT>
C<MatchOption> is applied to the tag C<Key> only. That is, it's
