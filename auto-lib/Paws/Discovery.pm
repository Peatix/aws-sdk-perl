package Paws::Discovery;
  ...
  sub BatchDeleteAgents {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Discovery::BatchDeleteAgents', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeBatchDeleteConfigurationTask {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Discovery::DescribeBatchDeleteConfigurationTask', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub StartBatchDeleteConfigurationTask {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Discovery::StartBatchDeleteConfigurationTask', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeAllImportTasks {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->DescribeImportTasks(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->DescribeImportTasks(@_, nextToken => $next_result->nextToken);
      ...
      while ($result->nextToken) {
        ...
        $result = $self->DescribeImportTasks(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'tasks') foreach (@{ $result->tasks });
    }
  ...
  sub operations { qw/AssociateConfigurationItemsToApplication BatchDeleteAgents BatchDeleteImportData CreateApplication CreateTags DeleteApplications DeleteTags DescribeAgents DescribeBatchDeleteConfigurationTask DescribeConfigurations DescribeContinuousExports DescribeExportConfigurations DescribeExportTasks DescribeImportTasks DescribeTags DisassociateConfigurationItemsFromApplication ExportConfigurations GetDiscoverySummary ListConfigurations ListServerNeighbors StartBatchDeleteConfigurationTask StartContinuousExport StartDataCollectionByAgentIds StartExportTask StartImportTask StopContinuousExport StopDataCollectionByAgentIds UpdateApplication / }
...
Amazon Web Services Application Discovery Service (Application
...
dependencies in your on-premises data centers. For more information,
see the Amazon Web Services Application Discovery Service FAQ
...
B<Agentless discovery> using Amazon Web Services Application Discovery
Service Agentless Collector (Agentless Collector), which doesn't
...
B<Agent-based discovery> using the Amazon Web Services Application
...
to the Application Discovery Service database in the Amazon Web
Services cloud. For more information, see Amazon Web Services
...
B<Amazon Web Services Partner Network (APN) solutions> integrate with
...
(Migration Hub) without using Agentless Collector or Application
Discovery Agent.
...
In this way, you can import data into Migration Hub and view it, so
that you can associate applications with servers and track migrations.

=back

=back

B<Working With This Guide>
...
you call APIs outside the home Region, an C<InvalidInputException> is
returned.

=item *

You must call C<GetHomeRegion> to obtain the latest Migration Hub home
Region.
...
(https://aws.amazon.com/privacy/). You can operate Application
...
=head2 BatchDeleteAgents

=over

=item DeleteAgents => ArrayRef[L<Paws::Discovery::DeleteAgent>]


=back

Each argument is described in detail in: L<Paws::Discovery::BatchDeleteAgents>

Returns: a L<Paws::Discovery::BatchDeleteAgentsResponse> instance
...
To delete the data collected, use C<StartBatchDeleteConfigurationTask>.
...
C<DescribeAgents> as is without passing any parameters.


=head2 DescribeBatchDeleteConfigurationTask
...
Each argument is described in detail in: L<Paws::Discovery::DescribeBatchDeleteConfigurationTask>

Returns: a L<Paws::Discovery::DescribeBatchDeleteConfigurationTaskResponse> instance
...
with your user can be listed if you call C<DescribeContinuousExports>
...
C<DescribeExportConfigurations> is deprecated. Use DescribeExportTasks
...
can be listed if you call C<DescribeTags> as is without passing any
parameters.
...
passed to the required parameter C<configurationType>. Optional
...
=head2 StartBatchDeleteConfigurationTask
...
Each argument is described in detail in: L<Paws::Discovery::StartBatchDeleteConfigurationTask>

Returns: a L<Paws::Discovery::StartBatchDeleteConfigurationTaskResponse> instance
...
=item [Preferences => L<Paws::Discovery::ExportPreferences>]
...
If you do not specify C<preferences> or C<agentIds> in the filter, a
...
If you enable C<ec2RecommendationsPreferences> in C<preferences> , an
...
the C<ec2RecommendationsPreferences> changes the criteria of the
recommendation.
...
Use the console or the C<StartImportTask> command with the Amazon Web
...
Amazon Web Services Application Discovery Service Limits
...
=head2 DescribeAllImportTasks(sub { },[Filters => ArrayRef[L<Paws::Discovery::ImportTaskFilter>], MaxResults => Int, NextToken => Str])

=head2 DescribeAllImportTasks([Filters => ArrayRef[L<Paws::Discovery::ImportTaskFilter>], MaxResults => Int, NextToken => Str])
...
If not, it will return a a L<Paws::Discovery::DescribeImportTasksResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
