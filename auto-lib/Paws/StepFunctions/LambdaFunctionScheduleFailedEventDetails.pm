...
Paws::StepFunctions::LambdaFunctionScheduleFailedEventDetails
...
As an example, if Att1 is expected to be a Paws::StepFunctions::LambdaFunctionScheduleFailedEventDetails object:

  $service_obj->Method(Att1 => { Cause => $value, ..., Error => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::StepFunctions::LambdaFunctionScheduleFailedEventDetails object:
