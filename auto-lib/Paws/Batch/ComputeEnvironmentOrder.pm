...
job placement first. Compute environments must be in the C<VALID> state
before you can associate them with a job queue. All of the compute
environments must be either EC2 (C<EC2> or C<SPOT>) or Fargate
(C<FARGATE> or C<FARGATE_SPOT>); Amazon EC2 and Fargate compute
...
ascending order. For example, if two compute environments are
associated with a job queue, the compute environment with a lower
C<order> integer value is tried for job placement first.
