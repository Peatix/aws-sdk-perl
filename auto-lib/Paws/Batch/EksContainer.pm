...
package Paws::Batch::EksContainer;
  use Moose;
...
As an example, if Att1 is expected to be a Paws::Batch::EksContainer object:

  $service_obj->Method(Att1 => { Args => $value, ..., VolumeMounts => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Batch::EksContainer object:
...
this isn't specified, the C<ENTRYPOINT> of the container image is used.
...
command string will remain "C<$(NAME1)>." C<$$> is replaced with C<$>
and the resulting string isn't expanded. For example, C<$$(VAR_NAME)>
will be passed as C<$(VAR_NAME)> whether or not the C<VAR_NAME>
environment variable exists. The entrypoint can't be updated. For more
information, see ENTRYPOINT
(https://docs.docker.com/engine/reference/builder/#entrypoint) in the
I<Dockerfile reference> and Define a command and arguments for a
container
(https://kubernetes.io/docs/tasks/inject-data-application/define-command-argument-container/)
and Entrypoint
(https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#entrypoint)
...
C<IfNotPresent>. However, if the C<:latest> tag is specified, it
defaults to C<Always>. For more information, see Updating images
