...
package Paws::KafkaConnect::KafkaClusterClientAuthenticationDescription;
  use Moose;
...
Paws::KafkaConnect::KafkaClusterClientAuthenticationDescription
...
As an example, if Att1 is expected to be a Paws::KafkaConnect::KafkaClusterClientAuthenticationDescription object:

  $service_obj->Method(Att1 => { AuthenticationType => $value, ..., AuthenticationType => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::KafkaConnect::KafkaClusterClientAuthenticationDescription object:
