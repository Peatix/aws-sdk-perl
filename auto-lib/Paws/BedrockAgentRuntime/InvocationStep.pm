# Generated by default/object.tt
package Paws::BedrockAgentRuntime::InvocationStep;
  use Moose;
  has InvocationId => (is => 'ro', isa => 'Str', request_name => 'invocationId', traits => ['NameInRequest'], required => 1);
  has InvocationStepId => (is => 'ro', isa => 'Str', request_name => 'invocationStepId', traits => ['NameInRequest'], required => 1);
  has InvocationStepTime => (is => 'ro', isa => 'Str', request_name => 'invocationStepTime', traits => ['NameInRequest'], required => 1);
  has Payload => (is => 'ro', isa => 'Paws::BedrockAgentRuntime::InvocationStepPayload', request_name => 'payload', traits => ['NameInRequest'], required => 1);
  has SessionId => (is => 'ro', isa => 'Str', request_name => 'sessionId', traits => ['NameInRequest'], required => 1);

1;

### main pod documentation begin ###

=head1 NAME

Paws::BedrockAgentRuntime::InvocationStep

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::BedrockAgentRuntime::InvocationStep object:

  $service_obj->Method(Att1 => { InvocationId => $value, ..., SessionId => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::BedrockAgentRuntime::InvocationStep object:

  $result = $service_obj->Method(...);
  $result->Att1->InvocationId

=head1 DESCRIPTION

Stores fine-grained state checkpoints, including text and images, for
each interaction in an invocation in a session. For more information
about sessions, see Store and retrieve conversation history and context
with Amazon Bedrock sessions
(https://docs.aws.amazon.com/bedrock/latest/userguide/sessions.html).

=head1 ATTRIBUTES


=head2 B<REQUIRED> InvocationId => Str

The unique identifier (in UUID format) for the invocation that includes
the invocation step.


=head2 B<REQUIRED> InvocationStepId => Str

The unique identifier (in UUID format) for the invocation step.


=head2 B<REQUIRED> InvocationStepTime => Str

The timestamp for when the invocation step was created.


=head2 B<REQUIRED> Payload => L<Paws::BedrockAgentRuntime::InvocationStepPayload>

Payload content, such as text and images, for the invocation step.


=head2 B<REQUIRED> SessionId => Str

The unique identifier of the session containing the invocation step.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::BedrockAgentRuntime>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

