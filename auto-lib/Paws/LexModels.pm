package Paws::LexModels;
  ...
  sub GetMigration {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::LexModels::GetMigration', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetMigrations {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::LexModels::GetMigrations', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetSlotType {
    my $self = shift;
  ...
  sub StartMigration {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::LexModels::StartMigration', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub TagResource {
    my $self = shift;
...
=item MigrationId => Str


=back

Each argument is described in detail in: L<Paws::LexModels::GetMigration>

Returns: a L<Paws::LexModels::GetMigrationResponse> instance
...
=item [MigrationStatusEquals => Str]

=item [NextToken => Str]

=item [SortByAttribute => Str]

=item [SortByOrder => Str]

=item [V1BotNameContains => Str]


=back

Each argument is described in detail in: L<Paws::LexModels::GetMigrations>

Returns: a L<Paws::LexModels::GetMigrationsResponse> instance
...
If you set C<childDirected> field to true when you created your bot, if
...
=item MigrationStrategy => Str

=item V1BotName => Str

=item V1BotVersion => Str
...
Each argument is described in detail in: L<Paws::LexModels::StartMigration>

Returns: a L<Paws::LexModels::StartMigrationResponse> instance
...
(https://docs.aws.amazon.com/lex/latest/dg/migrate.html) in the
I<Amazon Lex developer guide>.
