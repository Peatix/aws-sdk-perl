package Paws::Organizations;
  ...
  sub CloseAccount {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Organizations::CloseAccount', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub CreateAccount {
    my $self = shift;
  ...
  sub DeleteResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Organizations::DeleteResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Organizations::DescribeResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub DetachPolicy {
    my $self = shift;
  ...
  sub PutResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Organizations::PutResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
multiple Amazon Web Services accounts into an I<organization> and
...
B<Support and feedback for Organizations>
...
information about the Amazon Web Services support forums, see Forums
Help (http://forums.aws.amazon.com/help.jspa).

B<Endpoint to call When using the CLI or the Amazon Web Services SDK>
...
commercial Amazon Web Services Regions outside of China. If calling
from one of the Amazon Web Services Regions in China, then specify
C<cn-northwest-1>. You can do this in the CLI by using these parameters
and commands:
...
I<(from Amazon Web Services Regions in China)>
...
C<aws configure set default.region us-east-1> I<(from commercial Amazon
Web Services Regions outside of China)>

or

C<aws configure set default.region cn-northwest-1> I<(from Amazon Web
...
C<--region us-east-1> I<(from commercial Amazon Web Services Regions
outside of China)>

or

C<--region cn-northwest-1> I<(from Amazon Web Services Regions in
China)>
...
in the I<Organizations User Guide>. To learn more about CloudTrail,
...
C<AWSServiceRoleForOrganizations>. For more information, see
...
in the I<Organizations User Guide>. For more information about requests
...
of policy. Refer to the I<Organizations User Guide> for information
...
Each argument is described in detail in: L<Paws::Organizations::CloseAccount>
...
background. Because C<CloseAccount> operates asynchronously, it can
...
Use the C<AccountId> that you sent in the C<CloseAccount> request to
...
Check the CloudTrail log for the C<CloseAccountResult> event that gets
...
can't close additional accounts. For more information, see Closing a
...
Amazon Web Services GovCloud (US) account, the C<CloseAccount> request
...
asynchronous request that Amazon Web Services performs in the
background. Because C<CreateAccount> operates asynchronously, it can
...
Use the C<Id> value of the C<CreateAccountStatus> response element from
this operation to provide as a parameter to the
DescribeCreateAccountStatus operation.

=item *

Check the CloudTrail log for the C<CreateAccountResult> event. For
...
to operate as a standalone account, such as a payment method is I<not>
...
an account from an organization
...
try again. If the error persists, contact Amazon Web Services Support
(https://console.aws.amazon.com/support/home#/).

=item *

It isn't recommended to use C<CreateAccount> to create multiple
temporary accounts, and using the C<CreateAccount> API to close
...
GovCloud (US) Region, see the I<Amazon Web Services GovCloud User
Guide>.
...
named C<AWSServiceRoleForOrganizations>. For more information, see
Organizations and service-linked roles
(https://docs.aws.amazon.com/organizations/latest/userguide/orgs_integrate_services.html#orgs_integrate_services-using_slrs)
in the I<Organizations User Guide>.
...
Calling C<CreateGovCloudAccount> is an asynchronous request that Amazon
Web Services performs in the background. Because
C<CreateGovCloudAccount> operates asynchronously, it can return a
...
commercial organization. For more information and to view a diagram
that explains how account access works, see Organizations
(https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-organizations.html)
in the I<Amazon Web Services GovCloud User Guide>.
...
When you create an account in an organization using the Organizations
console, API, or CLI commands, the information required for the account
to operate as a standalone account is I<not> automatically collected.
This includes a payment method and signing the end user license
agreement (EULA). If you must remove an account from your organization
...
Services Billing and Cost Management console, and you must be signed in
as the root user. For information on the requirements and process for
closing an account, see Closing a member account in your organization
(https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_accounts_close.html)
in the I<Organizations User Guide>.
...
calling the C<CreateOrganization> operation automatically becomes the
management account
...
C<CONSOLIDATED_BILLING>, no policy types are enabled by default and you
...
Each argument is described in detail in: L<Paws::Organizations::DeleteResourcePolicy>
...
see the column I<Supports Delegated Administrator> in the table at
Amazon Web Services Services that you can use with Organizations
...
to authorization policies: service control policies (SCPs) and resource
...
Each argument is described in detail in: L<Paws::Organizations::DescribeResourcePolicy>

Returns: a L<Paws::Organizations::DescribeResourcePolicyResponse> instance
...
that is specified by C<ServicePrincipal>) with Organizations. When you
...
Services service. Instead, use the console or commands that are
...
under the B<Supports Trusted Access> column at Amazon Web Services
services that you can use with Organizations
(https://docs.aws.amazon.com/organizations/latest/userguide/orgs_integrate_services_list.html).
on this page.
...
appears enabled for the organization if all features
(https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_org_support-all-features.html)
...
by C<ServicePrincipal>) with permissions to view the structure of an
...
because your organization is still initializing, wait one hour and then
try again. If the error persists after an hour, contact Amazon Web
...
billing in your account. For more information, see About IAM access to
...
If you are using an organization principal to call C<LeaveOrganization>
across multiple accounts, you can only do this up to 5 accounts per
second in a single organization.
...
Handshakes that are C<ACCEPTED>, C<DECLINED>, C<CANCELED>, or
C<EXPIRED> appear in the results of this API for only 30 days after
...
Each argument is described in detail in: L<Paws::Organizations::PutResourcePolicy>

Returns: a L<Paws::Organizations::PutResourcePolicyResponse> instance
...
account. When you create an account in an organization using the
Organizations console, API, or CLI commands, the information required
of standalone accounts is I<not> automatically collected. For more
