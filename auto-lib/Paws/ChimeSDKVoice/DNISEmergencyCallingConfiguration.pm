...
package Paws::ChimeSDKVoice::DNISEmergencyCallingConfiguration;
  use Moose;
...
Paws::ChimeSDKVoice::DNISEmergencyCallingConfiguration
...
As an example, if Att1 is expected to be a Paws::ChimeSDKVoice::DNISEmergencyCallingConfiguration object:

  $service_obj->Method(Att1 => { CallingCountry => $value, ..., TestPhoneNumber => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ChimeSDKVoice::DNISEmergencyCallingConfiguration object:
