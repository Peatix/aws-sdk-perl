...
package Paws::ChimeSDKVoice::PhoneNumberAssociation;
  use Moose;
...
Paws::ChimeSDKVoice::PhoneNumberAssociation
...
As an example, if Att1 is expected to be a Paws::ChimeSDKVoice::PhoneNumberAssociation object:

  $service_obj->Method(Att1 => { AssociatedTimestamp => $value, ..., Value => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ChimeSDKVoice::PhoneNumberAssociation object:
...
Defines the association with an Amazon Chime SDK account ID, user ID,
