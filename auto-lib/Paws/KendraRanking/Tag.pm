...
package Paws::KendraRanking::Tag;
  use Moose;
...
As an example, if Att1 is expected to be a Paws::KendraRanking::Tag object:

  $service_obj->Method(Att1 => { Key => $value, ..., Value => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::KendraRanking::Tag object:
...
resource used for provisioning the C<Rescore> API. You can also use a
...
The value associated with the tag. The value can be an empty string but
it can't be null.
