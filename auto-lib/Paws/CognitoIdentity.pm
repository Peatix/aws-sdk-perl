...
as an OpenID token provider trusted by Security Token Service (STS) to
access temporary, limited-privilege Amazon Web Services credentials.
...
account. The keys for C<SupportedLoginProviders> are as follows:
...
Sign in With Apple: C<appleid.apple.com>
...
for C<cognito-identity.amazonaws.com>, it will be passed through to
