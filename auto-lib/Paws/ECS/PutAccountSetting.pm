...
=head1 SYNOPSIS
   ...
   # resource ID format for Amazon ECS services. If you’re using this command as
   # the root user, then changes apply to the entire AWS account, unless an IAM
 ...
 # This example modifies the account setting for a specific IAM user or IAM role
 # to opt in to the new ARN and resource ID format for Amazon ECS container
 # instances. If you’re using this command as the root user, then changes apply
 # to the entire AWS account, unless an IAM user or role explicitly overrides
...
C<defaultLogDriverMode> - Amazon ECS supports setting a default
delivery mode of log messages from a container to the C<logDriver> that
you specify in the container's C<logConfiguration>. The delivery mode
...
(C<fargateTaskRetirementWaitPeriod>).
...
values are C<enabled>, C<disabled>, C<enhanced>, C<on>, and C<off>.
