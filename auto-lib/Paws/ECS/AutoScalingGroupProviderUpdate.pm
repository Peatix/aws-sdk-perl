...
package Paws::ECS::AutoScalingGroupProviderUpdate;
  use Moose;
  has ManagedDraining => (is => 'ro', isa => 'Str', request_name => 'managedDraining', traits => ['NameInRequest']);
...
Paws::ECS::AutoScalingGroupProviderUpdate
...
As an example, if Att1 is expected to be a Paws::ECS::AutoScalingGroupProviderUpdate object:

  $service_obj->Method(Att1 => { ManagedDraining => $value, ..., ManagedTerminationProtection => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ECS::AutoScalingGroupProviderUpdate object:

  $result = $service_obj->Method(...);
  $result->Att1->ManagedDraining
...
drains the EC2 container instances that are in the Auto Scaling group
capacity provider.
...
from scale-in actions on. For more information, see Instance Protection
