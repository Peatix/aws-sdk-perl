...
package Paws::ECS::TaskManagedEBSVolumeConfiguration;
  use Moose;
  ...
  has TerminationPolicy => (is => 'ro', isa => 'Paws::ECS::TaskManagedEBSVolumeTerminationPolicy', request_name => 'terminationPolicy', traits => ['NameInRequest']);
...
Paws::ECS::TaskManagedEBSVolumeConfiguration
...
As an example, if Att1 is expected to be a Paws::ECS::TaskManagedEBSVolumeConfiguration object:

  $service_obj->Method(Att1 => { Encrypted => $value, ..., VolumeType => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ECS::TaskManagedEBSVolumeConfiguration object:
...
The available filesystem types are C<ext3>, C<ext4>, and C<xfs>. If no
value is specified, the C<xfs> filesystem type is used by default.
...
encrypting Amazon EBS volumes attached to a task, see Encrypt data
...
specify either a snapshot ID or a volume size. This parameter maps 1:1
...
=head2 TerminationPolicy => L<Paws::ECS::TaskManagedEBSVolumeTerminationPolicy>

The termination policy for the volume when the task exits. This
provides a way to control whether Amazon ECS terminates the Amazon EBS
...
existing Amazon EBS volume to create a new volume for attachment to the
task. This property can be specified only if you specify a
C<snapshotId>. For more information, see Initialize Amazon EBS volumes
