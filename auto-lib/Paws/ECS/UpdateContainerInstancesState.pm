...
only valid values for this action are C<ACTIVE> and C<DRAINING>. A
container instance can only be updated to C<DRAINING> status once it
has reached an C<ACTIVE> state. If a container instance is in
C<REGISTERING>, C<DEREGISTERING>, or C<REGISTRATION_FAILED> state you
