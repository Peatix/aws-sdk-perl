package Paws::S3Outposts;
  ...
  sub ListOutpostsWithS3 {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::S3Outposts::ListOutpostsWithS3', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListSharedEndpoints {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::S3Outposts::ListSharedEndpoints', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllOutpostsWithS3 {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListOutpostsWithS3(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListOutpostsWithS3(@_, NextToken => $next_result->NextToken);
      ...
      while ($result->NextToken) {
        ...
        $result = $self->ListOutpostsWithS3(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'Outposts') foreach (@{ $result->Outposts });
    }
  ...
  sub ListAllSharedEndpoints {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListSharedEndpoints(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListSharedEndpoints(@_, NextToken => $next_result->NextToken);
        push @{ $result->Endpoints }, @{ $next_result->Endpoints };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'Endpoints') foreach (@{ $result->Endpoints });
        $result = $self->ListSharedEndpoints(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'Endpoints') foreach (@{ $result->Endpoints });
    }
  ...
  sub operations { qw/CreateEndpoint DeleteEndpoint ListEndpoints ListOutpostsWithS3 ListSharedEndpoints / }
...
Each argument is described in detail in: L<Paws::S3Outposts::ListOutpostsWithS3>

Returns: a L<Paws::S3Outposts::ListOutpostsWithS3Result> instance
...
Each argument is described in detail in: L<Paws::S3Outposts::ListSharedEndpoints>

Returns: a L<Paws::S3Outposts::ListSharedEndpointsResult> instance
...
=head2 ListAllOutpostsWithS3(sub { },[MaxResults => Int, NextToken => Str])

=head2 ListAllOutpostsWithS3([MaxResults => Int, NextToken => Str])
...
If not, it will return a a L<Paws::S3Outposts::ListOutpostsWithS3Result> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllSharedEndpoints(sub { },OutpostId => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllSharedEndpoints(OutpostId => Str, [MaxResults => Int, NextToken => Str])
...
If not, it will return a a L<Paws::S3Outposts::ListSharedEndpointsResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
