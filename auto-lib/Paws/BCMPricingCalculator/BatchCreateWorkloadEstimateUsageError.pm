...
package Paws::BCMPricingCalculator::BatchCreateWorkloadEstimateUsageError;
  use Moose;
...
Paws::BCMPricingCalculator::BatchCreateWorkloadEstimateUsageError
...
As an example, if Att1 is expected to be a Paws::BCMPricingCalculator::BatchCreateWorkloadEstimateUsageError object:

  $service_obj->Method(Att1 => { ErrorCode => $value, ..., Key => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::BCMPricingCalculator::BatchCreateWorkloadEstimateUsageError object:
