...
package Paws::CostOptimizationHub::ComputeSavingsPlansConfiguration;
  use Moose;
...
Paws::CostOptimizationHub::ComputeSavingsPlansConfiguration
...
As an example, if Att1 is expected to be a Paws::CostOptimizationHub::ComputeSavingsPlansConfiguration object:

  $service_obj->Method(Att1 => { AccountScope => $value, ..., Term => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CostOptimizationHub::ComputeSavingsPlansConfiguration object:
