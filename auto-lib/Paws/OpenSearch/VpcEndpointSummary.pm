# Generated by default/object.tt
package Paws::OpenSearch::VpcEndpointSummary;
  use Moose;
  has DomainArn => (is => 'ro', isa => 'Str');
  has Status => (is => 'ro', isa => 'Str');
  has VpcEndpointId => (is => 'ro', isa => 'Str');
  has VpcEndpointOwner => (is => 'ro', isa => 'Str');

1;

### main pod documentation begin ###

=head1 NAME

Paws::OpenSearch::VpcEndpointSummary

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::OpenSearch::VpcEndpointSummary object:

  $service_obj->Method(Att1 => { DomainArn => $value, ..., VpcEndpointOwner => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::OpenSearch::VpcEndpointSummary object:

  $result = $service_obj->Method(...);
  $result->Att1->DomainArn

=head1 DESCRIPTION

Summary information for an Amazon OpenSearch Service-managed VPC
endpoint.

=head1 ATTRIBUTES


=head2 DomainArn => Str

The Amazon Resource Name (ARN) of the domain associated with the
endpoint.


=head2 Status => Str

The current status of the endpoint.


=head2 VpcEndpointId => Str

The unique identifier of the endpoint.


=head2 VpcEndpointOwner => Str

The creator of the endpoint.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::OpenSearch>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

