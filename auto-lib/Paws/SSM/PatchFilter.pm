...
a patch property. For example, the available filter keys for C<WINDOWS>
are C<PATCH_SET>, C<PRODUCT>, C<PRODUCT_FAMILY>, C<CLASSIFICATION>, and
C<MSRC_SEVERITY>.

The filter values define a matching criterion for the patch property
indicated by the key. For example, if the filter key is C<PRODUCT> and
the filter values are C<["Office 2013", "Office 2016"]>, then the
