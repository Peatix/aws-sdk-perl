...
package Paws::VoiceID::SpeakerEnrollmentJob;
  use Moose;
...
Paws::VoiceID::SpeakerEnrollmentJob
...
As an example, if Att1 is expected to be a Paws::VoiceID::SpeakerEnrollmentJob object:

  $service_obj->Method(Att1 => { CreatedAt => $value, ..., OutputDataConfig => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::VoiceID::SpeakerEnrollmentJob object:
...
already enrolled in Voice ID, and the C<FraudDetectionConfig> to use.
