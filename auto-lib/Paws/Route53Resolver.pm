package Paws::Route53Resolver;
  ...
  sub CreateOutpostResolver {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::CreateOutpostResolver', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DeleteOutpostResolver {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::DeleteOutpostResolver', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetOutpostResolver {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::GetOutpostResolver', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetResolverConfig {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::GetResolverConfig', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListOutpostResolvers {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::ListOutpostResolvers', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListResolverConfigs {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::ListResolverConfigs', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub UpdateOutpostResolver {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::UpdateOutpostResolver', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub UpdateResolverConfig {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53Resolver::UpdateResolverConfig', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllOutpostResolvers {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListOutpostResolvers(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListOutpostResolvers(@_, NextToken => $next_result->NextToken);
        push @{ $result->OutpostResolvers }, @{ $next_result->OutpostResolvers };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'OutpostResolvers') foreach (@{ $result->OutpostResolvers });
        $result = $self->ListOutpostResolvers(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'OutpostResolvers') foreach (@{ $result->OutpostResolvers });
    }
  ...
  sub ListAllResolverConfigs {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListResolverConfigs(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListResolverConfigs(@_, NextToken => $next_result->NextToken);
        push @{ $result->ResolverConfigs }, @{ $next_result->ResolverConfigs };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'ResolverConfigs') foreach (@{ $result->ResolverConfigs });
        $result = $self->ListResolverConfigs(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'ResolverConfigs') foreach (@{ $result->ResolverConfigs });
    }
...
=item [ConfidenceThreshold => Str]

=item [DnsThreatProtection => Str]

=item [FirewallDomainListId => Str]

=item [FirewallDomainRedirectionAction => Str]
...
=item PreferredInstanceType => Str
...
Each argument is described in detail in: L<Paws::Route53Resolver::CreateOutpostResolver>

Returns: a L<Paws::Route53Resolver::CreateOutpostResolverResponse> instance
...
=item [FirewallThreatProtectionId => Str]
...
Each argument is described in detail in: L<Paws::Route53Resolver::DeleteOutpostResolver>

Returns: a L<Paws::Route53Resolver::DeleteOutpostResolverResponse> instance
...
logging configuration is shared with other Amazon Web Services
...
Each argument is described in detail in: L<Paws::Route53Resolver::GetOutpostResolver>

Returns: a L<Paws::Route53Resolver::GetOutpostResolverResponse> instance
...
Each argument is described in detail in: L<Paws::Route53Resolver::GetResolverConfig>

Returns: a L<Paws::Route53Resolver::GetResolverConfigResponse> instance
...
Each argument is described in detail in: L<Paws::Route53Resolver::ListOutpostResolvers>

Returns: a L<Paws::Route53Resolver::ListOutpostResolversResponse> instance

Lists all the Resolvers on Outposts that were created using the current
Amazon Web Services account.


=head2 ListResolverConfigs
...
Each argument is described in detail in: L<Paws::Route53Resolver::ListResolverConfigs>

Returns: a L<Paws::Route53Resolver::ListResolverConfigsResponse> instance
...
Each argument is described in detail in: L<Paws::Route53Resolver::UpdateOutpostResolver>

Returns: a L<Paws::Route53Resolver::UpdateOutpostResolverResponse> instance

You can use C<UpdateOutpostResolver> to update the instance count,
type, or name of a Resolver on an Outpost.


=head2 UpdateResolverConfig

=over

=item AutodefinedReverseFlag => Str
...
Each argument is described in detail in: L<Paws::Route53Resolver::UpdateResolverConfig>

Returns: a L<Paws::Route53Resolver::UpdateResolverConfigResponse> instance
...
=item [UpdateIpAddresses => ArrayRef[L<Paws::Route53Resolver::UpdateIpAddress>]]
...
=head2 ListAllOutpostResolvers(sub { },[MaxResults => Int, NextToken => Str, OutpostArn => Str])

=head2 ListAllOutpostResolvers([MaxResults => Int, NextToken => Str, OutpostArn => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - OutpostResolvers, passing the object as the first parameter, and the string 'OutpostResolvers' as the second parameter 

If not, it will return a a L<Paws::Route53Resolver::ListOutpostResolversResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllResolverConfigs(sub { },[MaxResults => Int, NextToken => Str])

=head2 ListAllResolverConfigs([MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - ResolverConfigs, passing the object as the first parameter, and the string 'ResolverConfigs' as the second parameter 

If not, it will return a a L<Paws::Route53Resolver::ListResolverConfigsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
