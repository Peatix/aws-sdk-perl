...
package Paws::FIS::ExperimentTemplateCloudWatchLogsLogConfiguration;
  use Moose;
...
Paws::FIS::ExperimentTemplateCloudWatchLogsLogConfiguration
...
As an example, if Att1 is expected to be a Paws::FIS::ExperimentTemplateCloudWatchLogsLogConfiguration object:

  $service_obj->Method(Att1 => { LogGroupArn => $value, ..., LogGroupArn => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::FIS::ExperimentTemplateCloudWatchLogsLogConfiguration object:
