package Paws::Route53;
  ...
  sub ChangeCidrCollection {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53::ChangeCidrCollection', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub CreateCidrCollection {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53::CreateCidrCollection', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DeleteCidrCollection {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53::DeleteCidrCollection', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListCidrBlocks {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53::ListCidrBlocks', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListCidrCollections {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53::ListCidrCollections', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListCidrLocations {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Route53::ListCidrLocations', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllCidrBlocks {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListCidrBlocks(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListCidrBlocks(@_, NextToken => $next_result->NextToken);
        push @{ $result->CidrBlocks }, @{ $next_result->CidrBlocks };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'CidrBlocks') foreach (@{ $result->CidrBlocks });
        $result = $self->ListCidrBlocks(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'CidrBlocks') foreach (@{ $result->CidrBlocks });
    }
  ...
  sub ListAllCidrCollections {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListCidrCollections(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListCidrCollections(@_, NextToken => $next_result->NextToken);
        push @{ $result->CidrCollections }, @{ $next_result->CidrCollections };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'CidrCollections') foreach (@{ $result->CidrCollections });
        $result = $self->ListCidrCollections(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'CidrCollections') foreach (@{ $result->CidrCollections });
    }
  ...
  sub ListAllCidrLocations {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListCidrLocations(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListCidrLocations(@_, NextToken => $next_result->NextToken);
        push @{ $result->CidrLocations }, @{ $next_result->CidrLocations };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'CidrLocations') foreach (@{ $result->CidrLocations });
        $result = $self->ListCidrLocations(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'CidrLocations') foreach (@{ $result->CidrLocations });
    }
...
C<CreateVPCAssociationAuthorization> request. Then the account that
created the VPC must submit an C<AssociateVPCWithHostedZone> request.
...
C<aws> - Amazon Web Services Regions

=item *

C<aws-cn> - China Regions

=item *

C<aws-us-gov> - Amazon Web Services GovCloud (US) Region
...
=head2 ChangeCidrCollection

=over

=item Changes => ArrayRef[L<Paws::Route53::CidrCollectionChange>]

=item Id => Str

=item [CollectionVersion => Int]


=back

Each argument is described in detail in: L<Paws::Route53::ChangeCidrCollection>

Returns: a L<Paws::Route53::ChangeCidrCollectionResponse> instance
...
B<PUT and DELETE_IF_EXISTS>

Use C<ChangeCidrCollection> to perform the following actions:

=over

=item *

C<PUT>: Create a CIDR block within the specified collection.

=item *

C<DELETE_IF_EXISTS>: Delete an existing CIDR block from the collection.
...
C<UPSERT>: If a resource set doesn't exist, Route 53 creates it. If a
resource set exists Route 53 updates it with the values in the request.
...
C<GetChange> returns a status of C<PENDING>. When propagation is
complete, C<GetChange> returns a status of C<INSYNC>. Changes generally
...
Each argument is described in detail in: L<Paws::Route53::CreateCidrCollection>

Returns: a L<Paws::Route53::CreateCidrCollectionResponse> instance
...
of resource policies that you can create, so we recommend that you use
a consistent prefix so you can use the same resource policy for all the
...
streams. You must create the CloudWatch Logs resource policy in the
us-east-1 region. For the value of C<Resource>, specify the ARN for the
...
without a permission for an action can coerce a more-privileged entity
to perform it, you can optionally limit the permissions that a service
has to a resource in a resource-based policy by supplying the following
values:
...
For C<aws:SourceArn>, supply the hosted zone ARN used in creating the
query logging configuration. For example, C<aws:SourceArn:
arn:aws:route53:::hostedzone/hosted zone ID>.

=item *

For C<aws:SourceAccount>, supply the account ID for the account that
creates the query logging configuration. For example,
C<aws:SourceAccount:111111111111>.

=back

For more information, see The confused deputy problem
(https://docs.aws.amazon.com/IAM/latest/UserGuide/confused-deputy.html)
in the I<Amazon Web Services IAM User Guide>.
...
After you submit an C<CreateTrafficPolicyInstance> request, there's a
...
C<GetTrafficPolicyInstance> with the C<id> of new traffic policy
instance to confirm that the C<CreateTrafficPolicyInstance> request
completed successfully. For more information, see the C<State> response
element.
...
to submit an C<AssociateVPCWithHostedZone> request to associate the VPC
with a specified hosted zone that was created by a different account.
To submit a C<CreateVPCAssociationAuthorization> request, you must use
...
Each argument is described in detail in: L<Paws::Route53::DeleteCidrCollection>

Returns: a L<Paws::Route53::DeleteCidrCollectionResponse> instance
...
to verify that the KSK is in an C<INACTIVE> status.
...
created the VPC from associating the VPC with the Amazon Route 53
hosted zone in the future. If the VPC is already associated with the
hosted zone, C<DeleteVPCAssociationAuthorization> won't disassociate
the VPC from the hosted zone. If you want to delete an existing
association, use C<DisassociateVPCFromHostedZone>.
...
EFS) automatically create hosted zones and associate VPCs with the
hosted zones. A service can create a hosted zone using your account or
using its own account. You can disassociate a VPC from a hosted zone
...
Advisor. Sign in to the Amazon Web Services Management Console and open
the Trusted Advisor console at
...
It doesnE<rsquo>t support production use-cases with high query rates
...
C<returns the VPCs associated with the specified hosted zone and does
not reflect the VPC associations by Route 53 Profiles. To get the
associations to a Profile, call the ListProfileAssociations
...
Use C<GetTrafficPolicyInstance> with the C<id> of new traffic policy
instance to confirm that the C<CreateTrafficPolicyInstance> or an
C<UpdateTrafficPolicyInstance> request completed successfully. For more
information, see the C<State> response element.
...
Each argument is described in detail in: L<Paws::Route53::ListCidrBlocks>

Returns: a L<Paws::Route53::ListCidrBlocksResponse> instance

Returns a paginated list of location objects and their CIDR blocks.


=head2 ListCidrCollections
...
Each argument is described in detail in: L<Paws::Route53::ListCidrCollections>

Returns: a L<Paws::Route53::ListCidrCollectionsResponse> instance

Returns a paginated list of CIDR collections in the Amazon Web Services
account (metadata only).


=head2 ListCidrLocations
...
Each argument is described in detail in: L<Paws::Route53::ListCidrLocations>

Returns: a L<Paws::Route53::ListCidrLocationsResponse> instance
...
includes a C<HostedZones> child element for each hosted zone.
...
C<ListHostedZonesByName>, and specify the value of C<NextDNSName> and
C<NextHostedZoneId> in the C<dnsname> and C<hostedzoneid> parameters,
respectively.
...
Services service owns the hosted zones. The C<HostedZoneOwner>
...
An C<OwningService> element, which identifies the Amazon Web Services
service that created and owns the hosted zone. For example, if a hosted
zone was created by Amazon Elastic File System (Amazon EFS), the value
of C<Owner> is C<efs.amazonaws.com>.

=back

C<ListHostedZonesByVPC> returns the hosted zones associated with the
specified VPC and does not reflect the hosted zone associations to VPCs
via Route 53 Profiles. To get the associations to a Profile, call the
...
The C<TestDnsAnswer > returns information similar to what you would
expect from the answer section of the C<dig> command. Therefore, if you
...
After you submit a C<UpdateTrafficPolicyInstance> request, there's a
brief delay while Route 53 creates the resource record sets that are
specified in the traffic policy definition. Use
C<GetTrafficPolicyInstance> with the C<id> of updated traffic policy
instance confirm that the C<UpdateTrafficPolicyInstance> request
completed successfully. For more information, see the C<State> response
element.
...
=head2 ListAllCidrBlocks(sub { },CollectionId => Str, [LocationName => Str, MaxResults => Str, NextToken => Str])

=head2 ListAllCidrBlocks(CollectionId => Str, [LocationName => Str, MaxResults => Str, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - CidrBlocks, passing the object as the first parameter, and the string 'CidrBlocks' as the second parameter 

If not, it will return a a L<Paws::Route53::ListCidrBlocksResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllCidrCollections(sub { },[MaxResults => Str, NextToken => Str])

=head2 ListAllCidrCollections([MaxResults => Str, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - CidrCollections, passing the object as the first parameter, and the string 'CidrCollections' as the second parameter 

If not, it will return a a L<Paws::Route53::ListCidrCollectionsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllCidrLocations(sub { },CollectionId => Str, [MaxResults => Str, NextToken => Str])

=head2 ListAllCidrLocations(CollectionId => Str, [MaxResults => Str, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - CidrLocations, passing the object as the first parameter, and the string 'CidrLocations' as the second parameter 

If not, it will return a a L<Paws::Route53::ListCidrLocationsResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllHostedZones(sub { },[DelegationSetId => Str, HostedZoneType => Str, Marker => Str, MaxItems => Str])

=head2 ListAllHostedZones([DelegationSetId => Str, HostedZoneType => Str, Marker => Str, MaxItems => Str])
