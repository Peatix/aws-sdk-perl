...
As an example, if Att1 is expected to be a Paws::Kafka::JmxExporter object:

  $service_obj->Method(Att1 => { EnabledInBroker => $value, ..., EnabledInBroker => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Kafka::JmxExporter object:
