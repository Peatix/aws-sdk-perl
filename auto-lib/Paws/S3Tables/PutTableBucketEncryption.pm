
package Paws::S3Tables::PutTableBucketEncryption;
  use Moose;
  has EncryptionConfiguration => (is => 'ro', isa => 'Paws::S3Tables::EncryptionConfiguration', traits => ['NameInRequest'], request_name => 'encryptionConfiguration', required => 1);
  has TableBucketARN => (is => 'ro', isa => 'Str', traits => ['ParamInURI'], uri_name => 'tableBucketARN', required => 1);

  use MooseX::ClassAttribute;

  class_has _api_call => (isa => 'Str', is => 'ro', default => 'PutTableBucketEncryption');
  class_has _api_uri  => (isa => 'Str', is => 'ro', default => '/buckets/{tableBucketARN}/encryption');
  class_has _api_method  => (isa => 'Str', is => 'ro', default => 'PUT');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::API::Response');
1;

### main pod documentation begin ###

=head1 NAME

Paws::S3Tables::PutTableBucketEncryption - Arguments for method PutTableBucketEncryption on L<Paws::S3Tables>

=head1 DESCRIPTION

This class represents the parameters used for calling the method PutTableBucketEncryption on the
L<Amazon S3 Tables|Paws::S3Tables> service. Use the attributes of this class
as arguments to method PutTableBucketEncryption.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to PutTableBucketEncryption.

=head1 SYNOPSIS

    my $s3tables = Paws->service('S3Tables');
    $s3tables->PutTableBucketEncryption(
      EncryptionConfiguration => {
        SseAlgorithm => 'AES256',    # values: AES256, aws:kms
        KmsKeyArn    => 'MyEncryptionConfigurationKmsKeyArnString'
        ,                            # min: 1, max: 2048; OPTIONAL
      },
      TableBucketARN => 'MyTableBucketARN',

    );

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/s3tables/PutTableBucketEncryption>

=head1 ATTRIBUTES


=head2 B<REQUIRED> EncryptionConfiguration => L<Paws::S3Tables::EncryptionConfiguration>

The encryption configuration to apply to the table bucket.



=head2 B<REQUIRED> TableBucketARN => Str

The Amazon Resource Name (ARN) of the table bucket.




=head1 SEE ALSO

This class forms part of L<Paws>, documenting arguments for method PutTableBucketEncryption in L<Paws::S3Tables>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

