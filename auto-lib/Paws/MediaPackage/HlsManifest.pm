...
As an example, if Att1 is expected to be a Paws::MediaPackage::HlsManifest object:

  $service_obj->Method(Att1 => { AdMarkers => $value, ..., Url => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::MediaPackage::HlsManifest object:
