...
package Paws::Bedrock::TextInferenceConfig;
  use Moose;
...
Paws::Bedrock::TextInferenceConfig
...
As an example, if Att1 is expected to be a Paws::Bedrock::TextInferenceConfig object:

  $service_obj->Method(Att1 => { MaxTokens => $value, ..., TopP => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Bedrock::TextInferenceConfig object:
...
via the C<RetrieveAndGenerate> function.
...
here are arbitrary values, for actual values consult the limits defined
