...
Paws::CloudTrail::StopLogging - Arguments for method StopLogging on L<Paws::CloudTrail>
...
You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to StopLogging.

=head1 SYNOPSIS

    my $cloudtrail = Paws->service('CloudTrail');
    my $StopLoggingResponse = $cloudtrail->StopLogging(
      Name => 'MyString',

    );

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/cloudtrail/StopLogging>
