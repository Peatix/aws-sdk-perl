...
package Paws::PaymentCryptographyData::CardGenerationAttributes;
  use Moose;
...
Paws::PaymentCryptographyData::CardGenerationAttributes
...
As an example, if Att1 is expected to be a Paws::PaymentCryptographyData::CardGenerationAttributes object:

  $service_obj->Method(Att1 => { AmexCardSecurityCodeVersion1 => $value, ..., DynamicCardVerificationValue => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::PaymentCryptographyData::CardGenerationAttributes object:
