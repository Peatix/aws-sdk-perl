# Generated by default/object.tt
package Paws::GuardDuty::ResourceV2;
  use Moose;
  has AccountId => (is => 'ro', isa => 'Str', request_name => 'accountId', traits => ['NameInRequest']);
  has CloudPartition => (is => 'ro', isa => 'Str', request_name => 'cloudPartition', traits => ['NameInRequest']);
  has Data => (is => 'ro', isa => 'Paws::GuardDuty::ResourceData', request_name => 'data', traits => ['NameInRequest']);
  has Name => (is => 'ro', isa => 'Str', request_name => 'name', traits => ['NameInRequest']);
  has Region => (is => 'ro', isa => 'Str', request_name => 'region', traits => ['NameInRequest']);
  has ResourceType => (is => 'ro', isa => 'Str', request_name => 'resourceType', traits => ['NameInRequest'], required => 1);
  has Service => (is => 'ro', isa => 'Str', request_name => 'service', traits => ['NameInRequest']);
  has Tags => (is => 'ro', isa => 'ArrayRef[Paws::GuardDuty::Tag]', request_name => 'tags', traits => ['NameInRequest']);
  has Uid => (is => 'ro', isa => 'Str', request_name => 'uid', traits => ['NameInRequest'], required => 1);

1;

### main pod documentation begin ###

=head1 NAME

Paws::GuardDuty::ResourceV2

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::GuardDuty::ResourceV2 object:

  $service_obj->Method(Att1 => { AccountId => $value, ..., Uid => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::GuardDuty::ResourceV2 object:

  $result = $service_obj->Method(...);
  $result->Att1->AccountId

=head1 DESCRIPTION

Contains information about the Amazon Web Services resource that is
associated with the GuardDuty finding.

=head1 ATTRIBUTES


=head2 AccountId => Str

The Amazon Web Services account ID to which the resource belongs.


=head2 CloudPartition => Str

The cloud partition within the Amazon Web Services Region to which the
resource belongs.


=head2 Data => L<Paws::GuardDuty::ResourceData>

Contains information about the Amazon Web Services resource associated
with the activity that prompted GuardDuty to generate a finding.


=head2 Name => Str

The name of the resource.


=head2 Region => Str

The Amazon Web Services Region where the resource belongs.


=head2 B<REQUIRED> ResourceType => Str

The type of the Amazon Web Services resource.


=head2 Service => Str

The Amazon Web Services service of the resource.


=head2 Tags => ArrayRef[L<Paws::GuardDuty::Tag>]

Contains information about the tags associated with the resource.


=head2 B<REQUIRED> Uid => Str

The unique identifier of the resource.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::GuardDuty>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

