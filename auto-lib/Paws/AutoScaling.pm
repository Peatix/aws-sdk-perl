package Paws::AutoScaling;
  ...
  sub AttachTrafficSources {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AutoScaling::AttachTrafficSources', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeTrafficSources {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AutoScaling::DescribeTrafficSources', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DetachTrafficSources {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AutoScaling::DetachTrafficSources', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub RollbackInstanceRefresh {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AutoScaling::RollbackInstanceRefresh', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeAllWarmPool {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->DescribeWarmPool(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->DescribeWarmPool(@_, NextToken => $next_result->NextToken);
      ...
      while ($result->NextToken) {
        ...
        $result = $self->DescribeWarmPool(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'Instances') foreach (@{ $result->Instances });
    }
...
For more information, see Detach or attach instances
...
C<AttachTrafficSources> to simplify how you manage traffic sources.
However, we continue to support C<AttachLoadBalancers>. You can use
both the original C<AttachLoadBalancers> API operation and
C<AttachTrafficSources> on the same Auto Scaling group.
...
which can attach multiple traffic sources types. We recommend using
C<AttachTrafficSources> to simplify how you manage traffic sources.
However, we continue to support C<AttachLoadBalancerTargetGroups>. You
can use both the original C<AttachLoadBalancerTargetGroups> API
operation and C<AttachTrafficSources> on the same Auto Scaling group.
...
=head2 AttachTrafficSources

=over

=item AutoScalingGroupName => Str

=item TrafficSources => ArrayRef[L<Paws::AutoScaling::TrafficSourceIdentifier>]

=item [SkipZonalShiftValidation => Bool]


=back

Each argument is described in detail in: L<Paws::AutoScaling::AttachTrafficSources>

Returns: a L<Paws::AutoScaling::AttachTrafficSourcesResultType> instance
...
API to return details about the state of the attachments between
traffic sources and your Auto Scaling group. To detach a traffic source
from the Auto Scaling group, call the DetachTrafficSources
...
C<ActiveInstanceRefreshNotFound> error occurs.
...
changes that it made. Use the RollbackInstanceRefresh
...
EventBridge to invoke your Lambda function when an instance is put into
a wait state due to a lifecycle hook.
...
B<If you finish before the timeout period ends, send a callback by
using the CompleteLifecycleAction
...
=item [AvailabilityZoneDistribution => L<Paws::AutoScaling::AvailabilityZoneDistribution>]

=item [AvailabilityZoneImpairmentPolicy => L<Paws::AutoScaling::AvailabilityZoneImpairmentPolicy>]

=item [AvailabilityZones => ArrayRef[Str|Undef]]

=item [CapacityRebalance => Bool]

=item [CapacityReservationSpecification => L<Paws::AutoScaling::CapacityReservationSpecification>]

=item [Context => Str]

=item [DefaultCooldown => Int]

=item [DefaultInstanceWarmup => Int]

=item [DesiredCapacity => Int]

=item [DesiredCapacityType => Str]
...
=item [InstanceMaintenancePolicy => L<Paws::AutoScaling::InstanceMaintenancePolicy>]
...
fails. To query this limit, call the DescribeAccountLimits
...
Every Auto Scaling group has three size properties (C<DesiredCapacity>,
...
For more information, see Delete a scaling policy
...
must call the DescribeWarmPool
...
previously initiated, including their status, start time, end time, the
percentage of the instance refresh that is complete, and the number of
instances remaining to update before the instance refresh is complete.
If a rollback is initiated while an instance refresh is in progress,
Amazon EC2 Auto Scaling also returns information about the rollback of
...
C<DescribeTrafficSources> to simplify how you manage traffic sources.
However, we continue to support C<DescribeLoadBalancers>. You can use
both the original C<DescribeLoadBalancers> API operation and
C<DescribeTrafficSources> on the same Auto Scaling group.
...
Balancers, use the DescribeLoadBalancerTargetGroups
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeLoadBalancerTargetGroups.html)
API instead.

To determine the attachment status of the load balancer, use the
C<State> element in the response. When you attach a load balancer to an
Auto Scaling group, the initial C<State> value is C<Adding>. The state
...
the CreateAutoScalingGroup
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_CreateAutoScalingGroup.html)
API call. If your load balancer state is C<InService>, but it is not
...
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeTrafficSources.html),
which can describe multiple traffic sources types. We recommend using
C<DetachTrafficSources> to simplify how you manage traffic sources.
However, we continue to support C<DescribeLoadBalancerTargetGroups>.
You can use both the original C<DescribeLoadBalancerTargetGroups> API
operation and C<DescribeTrafficSources> on the same Auto Scaling group.
...
C<State> element in the response. When you attach a target group to an
Auto Scaling group, the initial C<State> value is C<Adding>. The state
transitions to C<Added> after all Auto Scaling instances are registered
...
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_CreateAutoScalingGroup.html)
API call. If your target group state is C<InService>, but it is not
working properly, check the scaling activities by calling
DescribeScalingActivities
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeScalingActivities.html)
and take any corrective actions necessary.
...
by using AttachLoadBalancerTargetGroups
...
already run, call the DescribeScalingActivities
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeScalingActivities.html)
API.
...
=item [TrafficSourceType => Str]


=back

Each argument is described in detail in: L<Paws::AutoScaling::DescribeTrafficSources>

Returns: a L<Paws::AutoScaling::DescribeTrafficSourcesResponse> instance
...
C<DetachTrafficSources> to simplify how you manage traffic sources.
However, we continue to support C<DetachLoadBalancers>. You can use
both the original C<DetachLoadBalancers> API operation and
C<DetachTrafficSources> on the same Auto Scaling group.
...
Application Load Balancers, Network Load Balancers, or Gateway Load
Balancers, use the DetachLoadBalancerTargetGroups
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DetachLoadBalancerTargetGroups.html)
API instead.
...
which can detach multiple traffic sources types. We recommend using
C<DetachTrafficSources> to simplify how you manage traffic sources.
However, we continue to support C<DetachLoadBalancerTargetGroups>. You
can use both the original C<DetachLoadBalancerTargetGroups> API
operation and C<DetachTrafficSources> on the same Auto Scaling group.

Detaches one or more target groups from the specified Auto Scaling
group.

When you detach a target group, it enters the C<Removing> state while
deregistering the instances in the group. When all instances are
deregistered, then you can no longer describe the target group using
the DescribeLoadBalancerTargetGroups
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeLoadBalancerTargetGroups.html)
API call. The instances remain running.

You can use this operation to detach target groups that were attached
by using AttachLoadBalancerTargetGroups
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_AttachLoadBalancerTargetGroups.html),
but not for target groups that were attached by using
AttachTrafficSources
...
Each argument is described in detail in: L<Paws::AutoScaling::DetachTrafficSources>

Returns: a L<Paws::AutoScaling::DetachTrafficSourcesResultType> instance

Detaches one or more traffic sources from the specified Auto Scaling
group.

When you detach a traffic source, it enters the C<Removing> state while
deregistering the instances in the group. When all instances are
deregistered, then you can no longer describe the traffic source using
the DescribeTrafficSources
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeTrafficSources.html)
...
API call. If you are no longer using a lifecycle hook, you can delete
it by calling the DeleteLifecycleHook
...
API call. If you are no longer using a scaling policy, you can delete
it by calling the DeletePolicy
...
it by calling the DeleteScheduledAction
...
=item [InstanceReusePolicy => L<Paws::AutoScaling::InstanceReusePolicy>]
...
time defined using the PutLifecycleHook
...
Each argument is described in detail in: L<Paws::AutoScaling::RollbackInstanceRefresh>

Returns: a L<Paws::AutoScaling::RollbackInstanceRefreshAnswer> instance
...
For more information, see Use instance scale-in protection
...
=item [DesiredConfiguration => L<Paws::AutoScaling::DesiredConfiguration>]
...
call the DescribeInstanceRefreshes
(https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeInstanceRefreshes.html)
...
termination of instances that are in C<Standby> state or protected from
...
back automatically when the instance refresh fails. You can enable this
feature before starting an instance refresh by specifying the
C<AutoRollback> property in the instance refresh preferences.
...
=head2 DescribeAllAutoScalingGroups(sub { },[AutoScalingGroupNames => ArrayRef[Str|Undef], Filters => ArrayRef[L<Paws::AutoScaling::Filter>], MaxRecords => Int, NextToken => Str])

=head2 DescribeAllAutoScalingGroups([AutoScalingGroupNames => ArrayRef[Str|Undef], Filters => ArrayRef[L<Paws::AutoScaling::Filter>], MaxRecords => Int, NextToken => Str])
...
=head2 DescribeAllWarmPool(sub { },AutoScalingGroupName => Str, [MaxRecords => Int, NextToken => Str])

=head2 DescribeAllWarmPool(AutoScalingGroupName => Str, [MaxRecords => Int, NextToken => Str])
...
If not, it will return a a L<Paws::AutoScaling::DescribeWarmPoolAnswer> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
