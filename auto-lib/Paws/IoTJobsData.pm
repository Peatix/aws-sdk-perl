package Paws::IoTJobsData;
  ...
  sub StartCommandExecution {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::IoTJobsData::StartCommandExecution', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub operations { qw/DescribeJobExecution GetPendingJobExecutions StartCommandExecution StartNextPendingJobExecution UpdateJobExecution / }
...
IoT Jobs is a service that allows you to define a set of jobs E<mdash>
remote operations that are sent to and executed on one or more devices
connected to Amazon Web Services IoT Core. For example, you can define
a job that instructs a set of devices to download and install
application or firmware updates, reboot, rotate certificates, or
...
C<aws iot describe-endpoint --endpoint-type iot:Jobs>

The service name used by Amazon Web Services Signature Version 4
(https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html)
to sign requests is: I<iot-jobs-data>.
...
The target starts the execution of the job by downloading the job
document, performing the operations it specifies, and reporting its
progress to Amazon Web Services IoT Core. The Jobs service provides
commands to track the progress of a job on a specific target and for
all the targets of the job
...
=item [ExecutionTimeoutSeconds => Int]

=item [Parameters => L<Paws::IoTJobsData::CommandExecutionParameterMap>]


=back

Each argument is described in detail in: L<Paws::IoTJobsData::StartCommandExecution>

Returns: a L<Paws::IoTJobsData::StartCommandExecutionResponse> instance

Using the command created with the C<CreateCommand> API, start a
