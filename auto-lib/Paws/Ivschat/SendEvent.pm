
package Paws::Ivschat::SendEvent;
  use Moose;
  has Attributes => (is => 'ro', isa => 'Paws::Ivschat::EventAttributes', traits => ['NameInRequest'], request_name => 'attributes');
  has EventName => (is => 'ro', isa => 'Str', traits => ['NameInRequest'], request_name => 'eventName', required => 1);
  has RoomIdentifier => (is => 'ro', isa => 'Str', traits => ['NameInRequest'], request_name => 'roomIdentifier', required => 1);

  use MooseX::ClassAttribute;

  class_has _api_call => (isa => 'Str', is => 'ro', default => 'SendEvent');
  class_has _api_uri  => (isa => 'Str', is => 'ro', default => '/SendEvent');
  class_has _api_method  => (isa => 'Str', is => 'ro', default => 'POST');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::Ivschat::SendEventResponse');
1;

### main pod documentation begin ###

=head1 NAME

Paws::Ivschat::SendEvent - Arguments for method SendEvent on L<Paws::Ivschat>

=head1 DESCRIPTION

This class represents the parameters used for calling the method SendEvent on the
L<Amazon Interactive Video Service Chat|Paws::Ivschat> service. Use the attributes of this class
as arguments to method SendEvent.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to SendEvent.

=head1 SYNOPSIS

    my $ivschat = Paws->service('Ivschat');
    my $SendEventResponse = $ivschat->SendEvent(
      EventName      => 'MyEventName',
      RoomIdentifier => 'MyRoomIdentifier',
      Attributes     => { 'MyString' => 'MyString', },    # OPTIONAL
    );

    # Results:
    my $Id = $SendEventResponse->Id;

    # Returns a L<Paws::Ivschat::SendEventResponse> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/ivschat/SendEvent>

=head1 ATTRIBUTES


=head2 Attributes => L<Paws::Ivschat::EventAttributes>

Application-defined metadata to attach to the event sent to clients.
The maximum length of the metadata is 1 KB total.



=head2 B<REQUIRED> EventName => Str

Application-defined name of the event to send to clients.



=head2 B<REQUIRED> RoomIdentifier => Str

Identifier of the room to which the event will be sent. Currently this
must be an ARN.




=head1 SEE ALSO

This class forms part of L<Paws>, documenting arguments for method SendEvent in L<Paws::Ivschat>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

