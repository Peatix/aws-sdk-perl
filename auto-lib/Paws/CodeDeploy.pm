...
contains individually tagged instances, Amazon EC2 instances in Amazon
...
B<Application revisions>: For an Lambda deployment, this is an AppSpec
...
definition, container, and port where production traffic is rerouted.
For an EC2/On-premises deployment, this is an archive file that
contains source contentE<mdash>source code, webpages, executable files,
and deployment scriptsE<mdash>along with an AppSpec file. Revisions are
...
B<CodeDeploy Information Resources>
...
The newer C<BatchGetDeploymentTargets> works with all compute
...
=item [OverrideAlarmConfiguration => L<Paws::CodeDeploy::AlarmConfiguration>]
...
=item [ZonalConfig => L<Paws::CodeDeploy::ZonalConfig>]
...
=item [TerminationHookEnabled => Bool]
...
are C<BeforeAllowTraffic> and C<AfterAllowTraffic>. For Amazon ECS
deployments, the available lifecycle hooks are C<BeforeInstall>,
C<AfterInstall>, C<AfterAllowTestTraffic>, C<BeforeAllowTraffic>, and
C<AfterAllowTraffic>. Lambda validation functions return C<Succeeded>
or C<Failed>. For more information, see AppSpec 'hooks' Section for an
Lambda Deployment
