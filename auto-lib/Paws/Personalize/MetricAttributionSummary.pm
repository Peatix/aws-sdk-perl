...
package Paws::Personalize::MetricAttributionSummary;
  use Moose;
...
Paws::Personalize::MetricAttributionSummary
...
As an example, if Att1 is expected to be a Paws::Personalize::MetricAttributionSummary object:

  $service_obj->Method(Att1 => { CreationDateTime => $value, ..., Status => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Personalize::MetricAttributionSummary object:
...
complete listing, call the DescribeMetricAttribution
