...
package Paws::Personalize::SolutionUpdateSummary;
  use Moose;
  ...
  has SolutionUpdateConfig => (is => 'ro', isa => 'Paws::Personalize::SolutionUpdateConfig', request_name => 'solutionUpdateConfig', traits => ['NameInRequest']);
...
Paws::Personalize::SolutionUpdateSummary
...
As an example, if Att1 is expected to be a Paws::Personalize::SolutionUpdateSummary object:

  $service_obj->Method(Att1 => { CreationDateTime => $value, ..., Status => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Personalize::SolutionUpdateSummary object:
...
complete listing, call the DescribeSolution
...
The status of the solution update. A solution update can be in one of
