...
package Paws::Personalize::RecommenderUpdateSummary;
  use Moose;
...
Paws::Personalize::RecommenderUpdateSummary
...
As an example, if Att1 is expected to be a Paws::Personalize::RecommenderUpdateSummary object:

  $service_obj->Method(Att1 => { CreationDateTime => $value, ..., Status => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Personalize::RecommenderUpdateSummary object:
...
complete listing, call the DescribeRecommender
