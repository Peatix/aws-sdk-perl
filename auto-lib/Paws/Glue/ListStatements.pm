
package Paws::Glue::ListStatements;
  use Moose;
  has NextToken => (is => 'ro', isa => 'Str');
  has RequestOrigin => (is => 'ro', isa => 'Str');
  has SessionId => (is => 'ro', isa => 'Str', required => 1);

  use MooseX::ClassAttribute;

  class_has _api_call => (isa => 'Str', is => 'ro', default => 'ListStatements');
  class_has _returns => (isa => 'Str', is => 'ro', default => 'Paws::Glue::ListStatementsResponse');
  class_has _result_key => (isa => 'Str', is => 'ro');
1;

### main pod documentation begin ###

=head1 NAME

Paws::Glue::ListStatements - Arguments for method ListStatements on L<Paws::Glue>

=head1 DESCRIPTION

This class represents the parameters used for calling the method ListStatements on the
L<AWS Glue|Paws::Glue> service. Use the attributes of this class
as arguments to method ListStatements.

You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to ListStatements.

=head1 SYNOPSIS

    my $glue = Paws->service('Glue');
    my $ListStatementsResponse = $glue->ListStatements(
      SessionId     => 'MyNameString',
      NextToken     => 'MyOrchestrationToken',         # OPTIONAL
      RequestOrigin => 'MyOrchestrationNameString',    # OPTIONAL
    );

    # Results:
    my $NextToken  = $ListStatementsResponse->NextToken;
    my $Statements = $ListStatementsResponse->Statements;

    # Returns a L<Paws::Glue::ListStatementsResponse> object.

Values for attributes that are native types (Int, String, Float, etc) can passed as-is (scalar values). Values for complex Types (objects) can be passed as a HashRef. The keys and values of the hashref will be used to instance the underlying object.
For the AWS API documentation, see L<https://docs.aws.amazon.com/goto/WebAPI/glue/ListStatements>

=head1 ATTRIBUTES


=head2 NextToken => Str

A continuation token, if this is a continuation call.



=head2 RequestOrigin => Str

The origin of the request to list statements.



=head2 B<REQUIRED> SessionId => Str

The Session ID of the statements.




=head1 SEE ALSO

This class forms part of L<Paws>, documenting arguments for method ListStatements in L<Paws::Glue>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

