...
package Paws::Glue::StorageDescriptor;
  use Moose;
  has AdditionalLocations => (is => 'ro', isa => 'ArrayRef[Str|Undef]');
...
As an example, if Att1 is expected to be a Paws::Glue::StorageDescriptor object:
  ...
  $service_obj->Method(Att1 => { AdditionalLocations => $value, ..., StoredAsSubDirectories => $value  });
...
Use accessors for each attribute. If Att1 is expected to be an Paws::Glue::StorageDescriptor object:
  ...
  $result->Att1->AdditionalLocations
...
=head2 AdditionalLocations => ArrayRef[Str|Undef]
