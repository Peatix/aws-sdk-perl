...
package Paws::ComputeOptimizer::IdleRecommendationFilter;
  use Moose;
...
Paws::ComputeOptimizer::IdleRecommendationFilter
...
As an example, if Att1 is expected to be a Paws::ComputeOptimizer::IdleRecommendationFilter object:

  $service_obj->Method(Att1 => { Name => $value, ..., Values => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ComputeOptimizer::IdleRecommendationFilter object:
...
You can filter your idle resource recommendations by C<tag:key> and
C<tag-key> tags.

A C<tag:key> is a key and value combination of a tag assigned to your
idle resource recommendations. Use the tag key in the filter name and
the tag value as the filter value. For example, to find all idle
resource service recommendations that have a tag with the key of
C<Owner> and the value of C<TeamA>, specify C<tag:Owner> for the filter
name and C<TeamA> for the filter value.

A C<tag-key> is the key of a tag assigned to your idle resource
...
idle resource service recommendations with a tag key value of C<Owner>
