...
package Paws::ComputeOptimizer::RDSInstanceEstimatedMonthlySavings;
  use Moose;
...
Paws::ComputeOptimizer::RDSInstanceEstimatedMonthlySavings
...
As an example, if Att1 is expected to be a Paws::ComputeOptimizer::RDSInstanceEstimatedMonthlySavings object:

  $service_obj->Method(Att1 => { Currency => $value, ..., Value => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::ComputeOptimizer::RDSInstanceEstimatedMonthlySavings object:
