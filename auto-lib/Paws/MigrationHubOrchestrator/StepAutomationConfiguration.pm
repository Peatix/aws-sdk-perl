...
package Paws::MigrationHubOrchestrator::StepAutomationConfiguration;
  use Moose;
...
Paws::MigrationHubOrchestrator::StepAutomationConfiguration
...
As an example, if Att1 is expected to be a Paws::MigrationHubOrchestrator::StepAutomationConfiguration object:

  $service_obj->Method(Att1 => { Command => $value, ..., TargetType => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::MigrationHubOrchestrator::StepAutomationConfiguration object:
