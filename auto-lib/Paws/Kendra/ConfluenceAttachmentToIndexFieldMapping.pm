...
package Paws::Kendra::ConfluenceAttachmentToIndexFieldMapping;
  use Moose;
...
Paws::Kendra::ConfluenceAttachmentToIndexFieldMapping
...
As an example, if Att1 is expected to be a Paws::Kendra::ConfluenceAttachmentToIndexFieldMapping object:

  $service_obj->Method(Att1 => { DataSourceFieldName => $value, ..., IndexFieldName => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Kendra::ConfluenceAttachmentToIndexFieldMapping object:
