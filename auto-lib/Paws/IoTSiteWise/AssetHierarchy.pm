...
As an example, if Att1 is expected to be a Paws::IoTSiteWise::AssetHierarchy object:
  ...
  $service_obj->Method(Att1 => { ExternalId => $value, ..., Name => $value  });
...
asset hierarchy, you may assign an external ID if it doesn't already
have one. You can't change the external ID of an asset hierarchy that
already has one. For more information, see Using external IDs
