...
package Paws::S3Control::GetAccessGrantsLocationResult;
  use Moose;
...
Paws::S3Control::GetAccessGrantsLocationResult
...
The S3 URI path to the registered location. The location scope can be
the default S3 location C<s3://>, the S3 path to a bucket, or the S3
path to a bucket and prefix. A prefix in S3 is a string of characters
at the beginning of an object key name used to organize the objects
that you store in your S3 buckets. For example, object key names that
start with the C<engineering/> prefix or object key names that start
with the C<marketing/campaigns/> prefix.
