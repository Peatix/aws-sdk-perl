# Generated by default/object.tt
package Paws::S3Control::ObjectLambdaAccessPoint;
  use Moose;
  has Alias => (is => 'ro', isa => 'Paws::S3Control::ObjectLambdaAccessPointAlias');
  has Name => (is => 'ro', isa => 'Str', required => 1);
  has ObjectLambdaAccessPointArn => (is => 'ro', isa => 'Str');

1;

### main pod documentation begin ###

=head1 NAME

Paws::S3Control::ObjectLambdaAccessPoint

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::S3Control::ObjectLambdaAccessPoint object:

  $service_obj->Method(Att1 => { Alias => $value, ..., ObjectLambdaAccessPointArn => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::S3Control::ObjectLambdaAccessPoint object:

  $result = $service_obj->Method(...);
  $result->Att1->Alias

=head1 DESCRIPTION

An access point with an attached Lambda function used to access
transformed data from an Amazon S3 bucket.

=head1 ATTRIBUTES


=head2 Alias => L<Paws::S3Control::ObjectLambdaAccessPointAlias>

The alias of the Object Lambda Access Point.


=head2 B<REQUIRED> Name => Str

The name of the Object Lambda Access Point.


=head2 ObjectLambdaAccessPointArn => Str

Specifies the ARN for the Object Lambda Access Point.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::S3Control>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

