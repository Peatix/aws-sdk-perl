...
package Paws::S3Control::AdvancedCostOptimizationMetrics;
  use Moose;
...
Paws::S3Control::AdvancedCostOptimizationMetrics
...
As an example, if Att1 is expected to be a Paws::S3Control::AdvancedCostOptimizationMetrics object:

  $service_obj->Method(Att1 => { IsEnabled => $value, ..., IsEnabled => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::S3Control::AdvancedCostOptimizationMetrics object:
...
A container that indicates whether advanced cost-optimization metrics
are enabled.
