package Paws::SSMIncidents;
  ...
  sub BatchGetIncidentFindings {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMIncidents::BatchGetIncidentFindings', @_);
  ...
  sub ListIncidentFindings {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSMIncidents::ListIncidentFindings', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllIncidentFindings {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListIncidentFindings(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->ListIncidentFindings(@_, nextToken => $next_result->nextToken);
      ...
      while ($result->nextToken) {
        ...
        $result = $self->ListIncidentFindings(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'findings') foreach (@{ $result->findings });
    }
...
=head2 BatchGetIncidentFindings
...
Each argument is described in detail in: L<Paws::SSMIncidents::BatchGetIncidentFindings>

Returns: a L<Paws::SSMIncidents::BatchGetIncidentFindingsOutput> instance
...
=item [Integrations => ArrayRef[L<Paws::SSMIncidents::Integration>]]
...
=item [EventReferences => ArrayRef[L<Paws::SSMIncidents::EventReference>]]
...
that mark key moments during an incident. You can create custom
...
Each argument is described in detail in: L<Paws::SSMIncidents::ListIncidentFindings>

Returns: a L<Paws::SSMIncidents::ListIncidentFindingsOutput> instance
...
=item [IncidentTemplateTags => L<Paws::SSMIncidents::TagMapUpdate>]
...
=head2 ListAllIncidentFindings(sub { },IncidentRecordArn => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllIncidentFindings(IncidentRecordArn => Str, [MaxResults => Int, NextToken => Str])
...
If not, it will return a a L<Paws::SSMIncidents::ListIncidentFindingsOutput> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
