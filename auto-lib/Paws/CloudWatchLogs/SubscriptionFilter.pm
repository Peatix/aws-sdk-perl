...
As an example, if Att1 is expected to be a Paws::CloudWatchLogs::SubscriptionFilter object:
  ...
  $service_obj->Method(Att1 => { ApplyOnTransformedLogs => $value, ..., RoleArn => $value  });
...
If this value is C<true>, the subscription filter is applied on the
...
of milliseconds after C<Jan 1, 1970 00:00:00 UTC>.
