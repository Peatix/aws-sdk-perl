...
C<Jan 1, 1970 00:00:00 UTC>. The C<lastEventTime> value updates on an
...
The ingestion time, expressed as the number of milliseconds after C<Jan
1, 1970 00:00:00 UTC> The C<lastIngestionTime> value updates on an
eventual consistency basis. It typically updates in less than an hour
...
B<Important:> As of June 17, 2019, this parameter is no longer
...
C<PutLogEvents> actions are always accepted regardless of receiving an
invalid sequence token. You don't need to obtain C<uploadSequenceToken>
to use a C<PutLogEvents> action.
