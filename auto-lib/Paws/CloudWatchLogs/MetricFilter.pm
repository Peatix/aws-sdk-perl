...
package Paws::CloudWatchLogs::MetricFilter;
  use Moose;
  has ApplyOnTransformedLogs => (is => 'ro', isa => 'Bool', request_name => 'applyOnTransformedLogs', traits => ['NameInRequest']);
...
As an example, if Att1 is expected to be a Paws::CloudWatchLogs::MetricFilter object:
  ...
  $service_obj->Method(Att1 => { ApplyOnTransformedLogs => $value, ..., MetricTransformations => $value  });
...
Use accessors for each attribute. If Att1 is expected to be an Paws::CloudWatchLogs::MetricFilter object:
  ...
  $result->Att1->ApplyOnTransformedLogs
...
If this value is C<true>, the metric filter is applied on the
transformed version of the log events instead of the original ingested
log events.
