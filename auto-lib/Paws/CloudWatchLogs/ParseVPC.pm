...
package Paws::CloudWatchLogs::ParseVPC;
  use Moose;
...
As an example, if Att1 is expected to be a Paws::CloudWatchLogs::ParseVPC object:

  $service_obj->Method(Att1 => { Source => $value, ..., Source => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CloudWatchLogs::ParseVPC object:
...
Use this processor to parse Amazon VPC vended logs, extract fields, and
and convert them into a JSON format. This processor always processes
the entire log event message.

This processor doesn't support custom log formats, such as NAT gateway
logs. For more information about custom log formats in Amazon VPC, see
parseVPC
