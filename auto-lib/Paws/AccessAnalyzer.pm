package Paws::AccessAnalyzer;
  ...
  sub CheckAccessNotGranted {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::CheckAccessNotGranted', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub CheckNoNewAccess {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::CheckNoNewAccess', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub CheckNoPublicAccess {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::CheckNoPublicAccess', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GenerateFindingRecommendation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::GenerateFindingRecommendation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetFindingRecommendation {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::GetFindingRecommendation', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetFindingsStatistics {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::GetFindingsStatistics', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetFindingV2 {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::GetFindingV2', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListFindingsV2 {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::ListFindingsV2', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub UpdateAnalyzer {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::AccessAnalyzer::UpdateAnalyzer', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetAllFindingRecommendation {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->GetFindingRecommendation(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->GetFindingRecommendation(@_, nextToken => $next_result->nextToken);
        push @{ $result->recommendedSteps }, @{ $next_result->recommendedSteps };
      }
      return $result;
    } else {
      while ($result->nextToken) {
        $callback->($_ => 'recommendedSteps') foreach (@{ $result->recommendedSteps });
        $result = $self->GetFindingRecommendation(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'recommendedSteps') foreach (@{ $result->recommendedSteps });
    }
  ...
  sub GetAllFindingV2 {
    ...
    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->GetFindingV2(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->GetFindingV2(@_, nextToken => $next_result->nextToken);
        push @{ $result->findingDetails }, @{ $next_result->findingDetails };
      }
      return $result;
    } else {
      while ($result->nextToken) {
        $callback->($_ => 'findingDetails') foreach (@{ $result->findingDetails });
        $result = $self->GetFindingV2(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'findingDetails') foreach (@{ $result->findingDetails });
    }
  ...
  sub ListAllFindingsV2 {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListFindingsV2(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->nextToken) {
        $next_result = $self->ListFindingsV2(@_, nextToken => $next_result->nextToken);
        push @{ $result->findings }, @{ $next_result->findings };
      }
      return $result;
    } else {
      while ($result->nextToken) {
        $callback->($_ => 'findings') foreach (@{ $result->findings });
        $result = $self->ListFindingsV2(@_, nextToken => $result->nextToken);
      }
      $callback->($_ => 'findings') foreach (@{ $result->findings });
    }
  ...
  sub operations { qw/ApplyArchiveRule CancelPolicyGeneration CheckAccessNotGranted CheckNoNewAccess CheckNoPublicAccess CreateAccessPreview CreateAnalyzer CreateArchiveRule DeleteAnalyzer DeleteArchiveRule GenerateFindingRecommendation GetAccessPreview GetAnalyzedResource GetAnalyzer GetArchiveRule GetFinding GetFindingRecommendation GetFindingsStatistics GetFindingV2 GetGeneratedPolicy ListAccessPreviewFindings ListAccessPreviews ListAnalyzedResources ListAnalyzers ListArchiveRules ListFindings ListFindingsV2 ListPolicyGenerations ListTagsForResource StartPolicyGeneration StartResourceScan TagResource UntagResource UpdateAnalyzer UpdateArchiveRule UpdateFindings ValidatePolicy / }
...
Identity and Access Management Access Analyzer helps you to set,
verify, and refine your IAM policies by providing a suite of
capabilities. Its features include findings for external and unused
access, basic and custom policy checks for validating policies, and
policy generation to generate fine-grained policies. To start using IAM
Access Analyzer to identify external or unused access, you first need
to create an analyzer.

B<External access analyzers> help identify potential risks of accessing
...
reasoning to analyze resource-based policies in your Amazon Web
Services environment. An external principal can be another Amazon Web
Services account, a root user, an IAM user or role, a federated user,
an Amazon Web Services service, or an anonymous user. You can also use
IAM Access Analyzer to preview public and cross-account access to your
resources before deploying permissions changes.

B<Unused access analyzers> help identify potential identity access
...
Analyzer, see Identity and Access Management Access Analyzer
...
=head2 CheckAccessNotGranted

=over

=item Access => ArrayRef[L<Paws::AccessAnalyzer::Access>]
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::CheckAccessNotGranted>

Returns: a L<Paws::AccessAnalyzer::CheckAccessNotGrantedResponse> instance

Checks whether the specified access isn't allowed by a policy.


=head2 CheckNoNewAccess

=over

=item ExistingPolicyDocument => Str

=item NewPolicyDocument => Str
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::CheckNoNewAccess>

Returns: a L<Paws::AccessAnalyzer::CheckNoNewAccessResponse> instance
...
and run a custom policy check for new access in the IAM Access Analyzer
custom policy checks samples
...
meant to be passed to the C<existingPolicyDocument> request parameter.


=head2 CheckNoPublicAccess
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::CheckNoPublicAccess>

Returns: a L<Paws::AccessAnalyzer::CheckNoPublicAccessResponse> instance
...
=item [Configuration => L<Paws::AccessAnalyzer::AnalyzerConfiguration>]
...
see IAM Access Analyzer filter keys
...
=head2 GenerateFindingRecommendation
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::GenerateFindingRecommendation>
...
GetFindingV2 both use C<access-analyzer:GetFinding> in the C<Action>
element of an IAM policy statement. You must have permission to perform
the C<access-analyzer:GetFinding> action.


=head2 GetFindingRecommendation
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::GetFindingRecommendation>

Returns: a L<Paws::AccessAnalyzer::GetFindingRecommendationResponse> instance
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::GetFindingsStatistics>

Returns: a L<Paws::AccessAnalyzer::GetFindingsStatisticsResponse> instance
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::GetFindingV2>

Returns: a L<Paws::AccessAnalyzer::GetFindingV2Response> instance
...
C<access-analyzer:ListFindings> in the C<Action> element of an IAM
policy statement. You must have permission to perform the
C<access-analyzer:ListFindings> action.
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::ListFindingsV2>

Returns: a L<Paws::AccessAnalyzer::ListFindingsV2Response> instance
...
Each argument is described in detail in: L<Paws::AccessAnalyzer::UpdateAnalyzer>

Returns: a L<Paws::AccessAnalyzer::UpdateAnalyzerResponse> instance
...
=item [ValidatePolicyResourceType => Str]
...
=head2 GetAllFindingRecommendation(sub { },AnalyzerArn => Str, Id => Str, [MaxResults => Int, NextToken => Str])

=head2 GetAllFindingRecommendation(AnalyzerArn => Str, Id => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - recommendedSteps, passing the object as the first parameter, and the string 'recommendedSteps' as the second parameter 

If not, it will return a a L<Paws::AccessAnalyzer::GetFindingRecommendationResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 GetAllFindingV2(sub { },AnalyzerArn => Str, Id => Str, [MaxResults => Int, NextToken => Str])

=head2 GetAllFindingV2(AnalyzerArn => Str, Id => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - findingDetails, passing the object as the first parameter, and the string 'findingDetails' as the second parameter 

If not, it will return a a L<Paws::AccessAnalyzer::GetFindingV2Response> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllFindingsV2(sub { },AnalyzerArn => Str, [Filter => L<Paws::AccessAnalyzer::FilterCriteriaMap>, MaxResults => Int, NextToken => Str, Sort => L<Paws::AccessAnalyzer::SortCriteria>])

=head2 ListAllFindingsV2(AnalyzerArn => Str, [Filter => L<Paws::AccessAnalyzer::FilterCriteriaMap>, MaxResults => Int, NextToken => Str, Sort => L<Paws::AccessAnalyzer::SortCriteria>])
...
If not, it will return a a L<Paws::AccessAnalyzer::ListFindingsV2Response> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ValidateAllPolicies(sub { },PolicyDocument => Str, PolicyType => Str, [Locale => Str, MaxResults => Int, NextToken => Str, ValidatePolicyResourceType => Str])

=head2 ValidateAllPolicies(PolicyDocument => Str, PolicyType => Str, [Locale => Str, MaxResults => Int, NextToken => Str, ValidatePolicyResourceType => Str])
