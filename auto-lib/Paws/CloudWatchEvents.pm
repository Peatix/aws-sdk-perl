package Paws::CloudWatchEvents;
  ...
  sub CreateEndpoint {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatchEvents::CreateEndpoint', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub CreateEventBus {
    my $self = shift;
  ...
  sub DeleteEndpoint {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatchEvents::DeleteEndpoint', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub DeleteEventBus {
    my $self = shift;
  ...
  sub DescribeEndpoint {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatchEvents::DescribeEndpoint', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListEndpoints {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatchEvents::ListEndpoints', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListEventBuses {
    my $self = shift;
  ...
  sub UpdateEndpoint {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatchEvents::UpdateEndpoint', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub UpdateEventBus {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatchEvents::UpdateEventBus', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub operations { qw/ActivateEventSource CancelReplay CreateApiDestination CreateArchive CreateConnection CreateEndpoint CreateEventBus CreatePartnerEventSource DeactivateEventSource DeauthorizeConnection DeleteApiDestination DeleteArchive DeleteConnection DeleteEndpoint DeleteEventBus DeletePartnerEventSource DeleteRule DescribeApiDestination DescribeArchive DescribeConnection DescribeEndpoint DescribeEventBus DescribeEventSource DescribePartnerEventSource DescribeReplay DescribeRule DisableRule EnableRule ListApiDestinations ListArchives ListConnections ListEndpoints ListEventBuses ListEventSources ListPartnerEventSourceAccounts ListPartnerEventSources ListReplays ListRuleNamesByTarget ListRules ListTagsForResource ListTargetsByRule PutEvents PutPartnerEvents PutPermission PutRule PutTargets RemovePermission RemoveTargets StartReplay TagResource TestEventPattern UntagResource UpdateApiDestination UpdateArchive UpdateConnection UpdateEndpoint UpdateEventBus / }
...
=item [InvocationConnectivityParameters => L<Paws::CloudWatchEvents::ConnectivityResourceParameters>]
...
=item EventBuses => ArrayRef[L<Paws::CloudWatchEvents::EndpointEventBus>]

=item Name => Str

=item RoutingConfig => L<Paws::CloudWatchEvents::RoutingConfig>

=item [Description => Str]

=item [ReplicationConfig => L<Paws::CloudWatchEvents::ReplicationConfig>]
...
Each argument is described in detail in: L<Paws::CloudWatchEvents::CreateEndpoint>

Returns: a L<Paws::CloudWatchEvents::CreateEndpointResponse> instance
...
=item [DeadLetterConfig => L<Paws::CloudWatchEvents::DeadLetterConfig>]
...
I<event_namespace> is determined by the partner, and is a way for the
...
The I<event_name> must be unique across all Amazon Web Services
...
The combination of I<event_namespace> and I<event_name> should help
...
Each argument is described in detail in: L<Paws::CloudWatchEvents::DeleteEndpoint>

Returns: a L<Paws::CloudWatchEvents::DeleteEndpointResponse> instance
...
endpoints and event replication
...
Amazon Web Services services to support functionality in those
services. You can delete these rules using the C<Force> option, but you
...
Each argument is described in detail in: L<Paws::CloudWatchEvents::DescribeEndpoint>

Returns: a L<Paws::CloudWatchEvents::DescribeEndpointResponse> instance
...
Each argument is described in detail in: L<Paws::CloudWatchEvents::ListEndpoints>

Returns: a L<Paws::CloudWatchEvents::ListEndpointsResponse> instance
...
information, see Calculating PutEvents event entry size
...
EventBridge PutEvents event entry size
...
Running C<PutPermission> permits the specified Amazon Web Services
account or Amazon Web Services organization to put events to the
specified I<event bus>. Amazon EventBridge rules in your account are
...
event bus, run C<PutPermission> once for each of these accounts. Or, if
all the accounts are members of the same Amazon Web Services
organization, you can run C<PutPermission> once specifying C<Principal>
as "*" and specifying the Amazon Web Services organization ID in
C<Condition>, to grant permissions to all accounts in that
organization.
...
C<Amazon EBS CreateSnapshot API call>

=item *

C<Amazon EC2 RebootInstances API call>

=item *

C<Amazon EC2 StopInstances API call>

=item *

C<Amazon EC2 TerminateInstances API call>
...
in the C<RoleARN> argument in C<PutTargets>.
...
granted you permission (using C<PutPermission>), you can send events to
...
that account's event bus as the C<Arn> value when you run
C<PutTargets>. If your account sends events to another account, your
...
is not charged. For more information, see Amazon EventBridge Pricing
(http://aws.amazon.com/eventbridge/pricing/).

C<Input>, C<InputPath>, and C<InputTransformer> are not available with
C<PutTarget> if the target is an event bus of a different Amazon Web
Services account.
...
C<PutTargets> call without a role on the same target (same C<Id> and
C<Arn>) will not remove the role.
...
revoke by the C<StatementId> value that you associated with the account
when you granted it permission with C<PutPermission>. You can find the
...
A successful execution of C<RemoveTargets> doesn't guarantee all
...
Each argument is described in detail in: L<Paws::CloudWatchEvents::UpdateEndpoint>

Returns: a L<Paws::CloudWatchEvents::UpdateEndpointResponse> instance
...
Each argument is described in detail in: L<Paws::CloudWatchEvents::UpdateEventBus>

Returns: a L<Paws::CloudWatchEvents::UpdateEventBusResponse> instance
