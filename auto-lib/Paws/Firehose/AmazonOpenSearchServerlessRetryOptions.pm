...
package Paws::Firehose::AmazonOpenSearchServerlessRetryOptions;
  use Moose;
...
Paws::Firehose::AmazonOpenSearchServerlessRetryOptions
...
As an example, if Att1 is expected to be a Paws::Firehose::AmazonOpenSearchServerlessRetryOptions object:

  $service_obj->Method(Att1 => { DurationInSeconds => $value, ..., DurationInSeconds => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Firehose::AmazonOpenSearchServerlessRetryOptions object:
