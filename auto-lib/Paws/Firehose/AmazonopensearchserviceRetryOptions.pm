...
package Paws::Firehose::AmazonopensearchserviceRetryOptions;
  use Moose;
...
Paws::Firehose::AmazonopensearchserviceRetryOptions
...
As an example, if Att1 is expected to be a Paws::Firehose::AmazonopensearchserviceRetryOptions object:

  $service_obj->Method(Att1 => { DurationInSeconds => $value, ..., DurationInSeconds => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Firehose::AmazonopensearchserviceRetryOptions object:
...
total amount of time during which Firehose retries delivery (including
