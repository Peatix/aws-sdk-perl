...
package Paws::Firehose::AmazonOpenSearchServerlessBufferingHints;
  use Moose;
...
Paws::Firehose::AmazonOpenSearchServerlessBufferingHints
...
As an example, if Att1 is expected to be a Paws::Firehose::AmazonOpenSearchServerlessBufferingHints object:

  $service_obj->Method(Att1 => { IntervalInSeconds => $value, ..., SizeInMBs => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Firehose::AmazonOpenSearchServerlessBufferingHints object:
