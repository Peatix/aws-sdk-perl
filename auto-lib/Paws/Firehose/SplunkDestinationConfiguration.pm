...
package Paws::Firehose::SplunkDestinationConfiguration;
  use Moose;
  has BufferingHints => (is => 'ro', isa => 'Paws::Firehose::SplunkBufferingHints');
...
Paws::Firehose::SplunkDestinationConfiguration
...
As an example, if Att1 is expected to be a Paws::Firehose::SplunkDestinationConfiguration object:

  $service_obj->Method(Att1 => { BufferingHints => $value, ..., SecretsManagerConfiguration => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Firehose::SplunkDestinationConfiguration object:
...
=head2 BufferingHints => L<Paws::Firehose::SplunkBufferingHints>
...
from Splunk after it sends it data. At the end of the timeout period,
Firehose either tries to send the data again or considers it an error,
based on your retry settings.


=head2 B<REQUIRED> HECEndpoint => Str

The HTTP Event Collector (HEC) endpoint to which Firehose sends your
data.


=head2 B<REQUIRED> HECEndpointType => Str
...
C<FailedEventsOnly>, Firehose writes any data that could not be indexed
to the configured Amazon S3 destination. When set to C<AllEvents>,
Firehose delivers all incoming records to Amazon S3, and also writes
failed documents to Amazon S3. The default value is
C<FailedEventsOnly>.
