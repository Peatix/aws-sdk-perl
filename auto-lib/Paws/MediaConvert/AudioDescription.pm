...
package Paws::MediaConvert::AudioDescription;
  use Moose;
  has AudioChannelTaggingSettings => (is => 'ro', isa => 'Paws::MediaConvert::AudioChannelTaggingSettings', request_name => 'audioChannelTaggingSettings', traits => ['NameInRequest']);
...
As an example, if Att1 is expected to be a Paws::MediaConvert::AudioDescription object:
  ...
  $service_obj->Method(Att1 => { AudioChannelTaggingSettings => $value, ..., StreamName => $value  });
...
Use accessors for each attribute. If Att1 is expected to be an Paws::MediaConvert::AudioDescription object:
  ...
  $result->Att1->AudioChannelTaggingSettings
...
audio track. Depending on how you set up your input audio selectors and
whether you use audio selector groups, one audio tab can correspond to
a group of output audio tracks.

=head1 ATTRIBUTES


=head2 AudioChannelTaggingSettings => L<Paws::MediaConvert::AudioChannelTaggingSettings>
...
language code when you set Language code control to Follow input, but
your input file doesn't specify a language code. For all outputs, you
...
track. When you choose Follow input, the service uses the language code
from the input track if it's present. If there's no languge code on the
input track, the service uses the code that you specify in the setting
Language code. When you choose Use configured, the service uses the
