# Generated by default/object.tt
package Paws::FMS::FirewallSubnetIsOutOfScopeViolation;
  use Moose;
  has FirewallSubnetId => (is => 'ro', isa => 'Str');
  has SubnetAvailabilityZone => (is => 'ro', isa => 'Str');
  has SubnetAvailabilityZoneId => (is => 'ro', isa => 'Str');
  has VpcEndpointId => (is => 'ro', isa => 'Str');
  has VpcId => (is => 'ro', isa => 'Str');

1;

### main pod documentation begin ###

=head1 NAME

Paws::FMS::FirewallSubnetIsOutOfScopeViolation

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::FMS::FirewallSubnetIsOutOfScopeViolation object:

  $service_obj->Method(Att1 => { FirewallSubnetId => $value, ..., VpcId => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::FMS::FirewallSubnetIsOutOfScopeViolation object:

  $result = $service_obj->Method(...);
  $result->Att1->FirewallSubnetId

=head1 DESCRIPTION

Contains details about the firewall subnet that violates the policy
scope.

=head1 ATTRIBUTES


=head2 FirewallSubnetId => Str

The ID of the firewall subnet that violates the policy scope.


=head2 SubnetAvailabilityZone => Str

The Availability Zone of the firewall subnet that violates the policy
scope.


=head2 SubnetAvailabilityZoneId => Str

The Availability Zone ID of the firewall subnet that violates the
policy scope.


=head2 VpcEndpointId => Str

The VPC endpoint ID of the firewall subnet that violates the policy
scope.


=head2 VpcId => Str

The VPC ID of the firewall subnet that violates the policy scope.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::FMS>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

