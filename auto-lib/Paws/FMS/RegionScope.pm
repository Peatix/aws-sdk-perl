# Generated by default/object.tt
package Paws::FMS::RegionScope;
  use Moose;
  has AllRegionsEnabled => (is => 'ro', isa => 'Bool');
  has Regions => (is => 'ro', isa => 'ArrayRef[Str|Undef]');

1;

### main pod documentation begin ###

=head1 NAME

Paws::FMS::RegionScope

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::FMS::RegionScope object:

  $service_obj->Method(Att1 => { AllRegionsEnabled => $value, ..., Regions => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::FMS::RegionScope object:

  $result = $service_obj->Method(...);
  $result->Att1->AllRegionsEnabled

=head1 DESCRIPTION

Defines the Amazon Web Services Regions that the specified Firewall
Manager administrator can manage.

=head1 ATTRIBUTES


=head2 AllRegionsEnabled => Bool

Allows the specified Firewall Manager administrator to manage all
Amazon Web Services Regions.


=head2 Regions => ArrayRef[Str|Undef]

The Amazon Web Services Regions that the specified Firewall Manager
administrator can perform actions in.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::FMS>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut

