...
package Paws::AmplifyBackend::UpdateBackendAuthVerificationMessageConfig;
  use Moose;
...
Paws::AmplifyBackend::UpdateBackendAuthVerificationMessageConfig
...
As an example, if Att1 is expected to be a Paws::AmplifyBackend::UpdateBackendAuthVerificationMessageConfig object:

  $service_obj->Method(Att1 => { DeliveryMethod => $value, ..., SmsSettings => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::AmplifyBackend::UpdateBackendAuthVerificationMessageConfig object:
