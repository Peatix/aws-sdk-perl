package Paws::DynamoDB;
  ...
  sub DeleteResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::DeleteResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub DeleteTable {
    my $self = shift;
  ...
  sub DescribeImport {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::DescribeImport', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::GetResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ImportTable {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::ImportTable', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListBackups {
    my $self = shift;
  ...
  sub ListImports {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::ListImports', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListTables {
    my $self = shift;
  ...
  sub PutResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::PutResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub Query {
    my $self = shift;
  ...
  sub UpdateKinesisStreamingDestination {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::DynamoDB::UpdateKinesisStreamingDestination', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub UpdateTable {
    my $self = shift;
...
C<BatchExecuteStatement> must specify an equality condition on all key
attributes. This enforces that each C<SELECT> statement in a batch
returns at most a single item. For more information, see Running batch
operations with PartiQL for DynamoDB
...
field of the C<BatchStatementResponse> for each statement.
...
operation returns a value for C<UnprocessedKeys>. You can use this
...
In order to minimize response latency, C<BatchGetItem> may retrieve
items in parallel.
...
C<BatchGetItem> will result in a C<ValidationException> if the same key
...
or more tables. A single call to C<BatchWriteItem> can transmit up to
...
greater than 400KB while being sent in DynamoDB's JSON format for the
API call. For more details on this distinction, see Naming Rules and
Data Types
(https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.NamingRulesDataTypes.html).

C<BatchWriteItem> cannot update items. If you perform a
C<BatchWriteItem> operation on an existing item, that item's values
will be overwritten by the operation and it will appear like it was
updated. To update items, we recommend you use the C<UpdateItem>
action.
...
the tables in the request, then C<BatchWriteItem> returns a
C<ProvisionedThroughputExceededException>. For all tables and indexes,
if none of the items can be processed due to other throttling scenarios
(such as exceeding partition level limits), then C<BatchWriteItem>
...
=item [OnDemandThroughput => L<Paws::DynamoDB::OnDemandThroughput>]
...
=item [WarmThroughput => L<Paws::DynamoDB::WarmThroughput>]
...
=item [ReturnValuesOnConditionCheckFailure => Str]
...
Each argument is described in detail in: L<Paws::DynamoDB::DeleteResourcePolicy>

Returns: a L<Paws::DynamoDB::DeleteResourcePolicyOutput> instance

Deletes the resource-based policy attached to the resource, which can
be a table or stream.

C<DeleteResourcePolicy> is an idempotent operation; running it multiple
times on the same resource I<doesn't> result in an error response,
unless you specify an C<ExpectedRevisionId>, which will then return a
...
perform C<DeleteResourcePolicy> requests, even if your resource-based
policy explicitly denies the root principal's access.

C<DeleteResourcePolicy> is an asynchronous operation. If you issue a
C<GetResourcePolicy> request immediately after running the
C<DeleteResourcePolicy> request, DynamoDB might still return the
...
C<GetResourcePolicy> request again.
...
policy permissions, please see Internetwork traffic privacy
...
Each argument is described in detail in: L<Paws::DynamoDB::DescribeImport>

Returns: a L<Paws::DynamoDB::DescribeImportOutput> instance
...
(https://console.aws.amazon.com/support/home#/), obtaining the increase
is not instantaneous. The C<DescribeLimits> action lets you write code
...
=item [EnableKinesisStreamingConfiguration => L<Paws::DynamoDB::EnableKinesisStreamingConfiguration>]
...
For PartiQL reads (C<SELECT> statement), if the total number of
processed items exceeds the maximum dataset size limit of 1 MB, the
read stops and results are returned to the user as a
C<LastEvaluatedKey> value to continue the read in a subsequent
operation. If the filter criteria in C<WHERE> clause does not match any
data, the read will return an empty result set.

A single C<SELECT> statement response can return up to the maximum
number of items (if using the Limit parameter) or a maximum of 1 MB of
data (and then apply any filtering to the results using C<WHERE>
clause). If C<LastEvaluatedKey> is present in the response, you need to
paginate the result set. If C<NextToken> is present, you need to
paginate the result set and include C<NextToken>.
...
attributes of the item in a similar manner to C<ConditionCheck> in the
TransactWriteItems
...
=item [IncrementalExportSpecification => L<Paws::DynamoDB::IncrementalExportSpecification>]
...
Each argument is described in detail in: L<Paws::DynamoDB::GetResourcePolicy>

Returns: a L<Paws::DynamoDB::GetResourcePolicyOutput> instance

Returns the resource-based policy document attached to the resource,
which can be a table or stream, in JSON format.

C<GetResourcePolicy> follows an I<eventually consistent>
(https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadConsistency.html)
model. The following list describes the outcomes when you issue the
C<GetResourcePolicy> request immediately after issuing another request:

=over

=item *

If you issue a C<GetResourcePolicy> request immediately after a
C<PutResourcePolicy> request, DynamoDB might return a
C<PolicyNotFoundException>.

=item *

If you issue a C<GetResourcePolicy>request immediately after a
C<DeleteResourcePolicy> request, DynamoDB might return the policy that
was present before the deletion request.

=item *

If you issue a C<GetResourcePolicy> request immediately after a
C<CreateTable> request, which includes a resource-based policy,
DynamoDB might return a C<ResourceNotFoundException> or a
C<PolicyNotFoundException>.

=back

Because C<GetResourcePolicy> uses an I<eventually consistent> query,
the metadata for your policy or table might not be available at that
moment. Wait for a few seconds, and then retry the C<GetResourcePolicy>
request.

After a C<GetResourcePolicy> request returns a policy created using the
C<PutResourcePolicy> request, the policy will be applied in the
...
table using the C<CreateTable> request will always be applied to all
...
=item InputFormat => Str

=item S3BucketSource => L<Paws::DynamoDB::S3BucketSource>

=item TableCreationParameters => L<Paws::DynamoDB::TableCreationParameters>

=item [ClientToken => Str]

=item [InputCompressionType => Str]

=item [InputFormatOptions => L<Paws::DynamoDB::InputFormatOptions>]


=back

Each argument is described in detail in: L<Paws::DynamoDB::ImportTable>

Returns: a L<Paws::DynamoDB::ImportTableOutput> instance
...
backups for a given table, specify C<TableName>. C<ListBackups> returns
a paginated list of results with at most 1 MB worth of items in a page.
...
Each argument is described in detail in: L<Paws::DynamoDB::ListImports>

Returns: a L<Paws::DynamoDB::ListImportsOutput> instance
...
=item [ConfirmRemoveSelfResourceAccess => Bool]
...
Each argument is described in detail in: L<Paws::DynamoDB::PutResourcePolicy>

Returns: a L<Paws::DynamoDB::PutResourcePolicyOutput> instance
...
API, the policy application is I<eventually consistent>
(https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadConsistency.html).

C<PutResourcePolicy> is an idempotent operation; running it multiple
times on the same resource using the same policy document will return
the same revision ID. If you specify an C<ExpectedRevisionId> that
doesn't match the current policy's C<RevisionId>, the
C<PolicyNotFoundException> will be returned.

C<PutResourcePolicy> is an asynchronous operation. If you issue a
C<GetResourcePolicy> request immediately after a C<PutResourcePolicy>
request, DynamoDB might return your previous policy, if there was one,
or return the C<PolicyNotFoundException>. This is because
C<GetResourcePolicy> uses an eventually consistent query, and the
metadata for your policy or table might not be available at that
moment. Wait for a few seconds, and then try the C<GetResourcePolicy>
request again.
...
value for that attribute. C<Query> returns all items with that
...
=item [OnDemandThroughputOverride => L<Paws::DynamoDB::OnDemandThroughput>]
...
The C<LastEvaluatedKey> value is also returned and the requestor can
use the C<LastEvaluatedKey> to continue the scan in a subsequent
...
C<FilterExpression>, a scan result can result in no items meeting the
criteria and the C<Count> will result in zero. If you did not use a
C<FilterExpression> in the scan request, then C<Count> is the same as
C<ScannedCount>.

C<Count> and C<ScannedCount> only return the count of items specific to
a single scan request and, unless the table is less than 1MB, do not
represent the total number of items in the table.

A single C<Scan> operation first reads up to the maximum number of
items set (if using the C<Limit> parameter) or a maximum of 1 MB of
data and then applies any filtering to the results if a
C<FilterExpression> is provided. If C<LastEvaluatedKey> is present in
...
By default, a C<Scan> uses eventually consistent reads when accessing
the items in a table. Therefore, the results from an eventually
consistent C<Scan> may not include the latest item changes at the time
...
items in the table, you can set the C<ConsistentRead> parameter to
true. Strong consistency only relates to the consistency of the read at
the item level.

DynamoDB does not provide snapshot isolation for a scan operation when
the C<ConsistentRead> parameter is set to true. Thus, a DynamoDB scan
...
C<TagResource> is an asynchronous operation. If you issue a
ListTagsOfResource request immediately after a C<TagResource> request,
DynamoDB might return your previous tag set, if there was one, or an
empty tag set. This is because C<ListTagsOfResource> uses an eventually
...
C<ListTagsOfResource> request again.

=item *

The application or removal of tags using C<TagResource> and
C<UntagResource> APIs is eventually consistent. C<ListTagsOfResource>
...
up to 100 C<TransactGetItem> objects, each of which contains a C<Get>
...
from tables in more than one Amazon Web Services account or Region. The
...
both C<ConditionCheck> and C<Update> the same item. The aggregate size
...
C<Put> E<mdash> Initiates a C<PutItem> operation to write a new item.
...
C<Update> E<mdash> Initiates an C<UpdateItem> operation to update an
...
C<Delete> E<mdash> Initiates a C<DeleteItem> operation to delete an
existing item. This structure specifies the primary key of the item to
...
C<ConditionCheck> E<mdash> Applies a condition to an item that is not
...
C<UntagResource> is an asynchronous operation. If you issue a
ListTagsOfResource request immediately after an C<UntagResource>
request, DynamoDB might return your previous tag set, if there was one,
or an empty tag set. This is because C<ListTagsOfResource> uses an
...
try the C<ListTagsOfResource> request again.
...
C<LatestRestorableDateTime> is typically 5 minutes before the current
time. You can restore your table to any point in time in the last 35
days. You can set the C<RecoveryPeriodInDays> to any value between 1
and 35 days.
...
Amazon Web Services Key Management Service (KMS) to encrypt this
tableE<rsquo>s partition key and sort key data with an Amazon Web
...
=head2 UpdateKinesisStreamingDestination
...
=item [UpdateKinesisStreamingConfiguration => L<Paws::DynamoDB::UpdateKinesisStreamingConfiguration>]


=back

Each argument is described in detail in: L<Paws::DynamoDB::UpdateKinesisStreamingDestination>

Returns: a L<Paws::DynamoDB::UpdateKinesisStreamingDestinationOutput> instance
...
=item [MultiRegionConsistency => Str]
...
C<UpdateTable> is an asynchronous operation; while it's executing, the
table status changes from C<ACTIVE> to C<UPDATING>. While it's
C<UPDATING>, you can't issue another C<UpdateTable> request. When the
