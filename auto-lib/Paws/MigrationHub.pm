package Paws::MigrationHub;
  ...
  sub AssociateSourceResource {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::MigrationHub::AssociateSourceResource', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DisassociateSourceResource {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::MigrationHub::DisassociateSourceResource', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListMigrationTaskUpdates {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::MigrationHub::ListMigrationTaskUpdates', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListSourceResources {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::MigrationHub::ListSourceResources', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListAllMigrationTaskUpdates {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListMigrationTaskUpdates(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListMigrationTaskUpdates(@_, NextToken => $next_result->NextToken);
        push @{ $result->MigrationTaskUpdateList }, @{ $next_result->MigrationTaskUpdateList };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'MigrationTaskUpdateList') foreach (@{ $result->MigrationTaskUpdateList });
        $result = $self->ListMigrationTaskUpdates(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'MigrationTaskUpdateList') foreach (@{ $result->MigrationTaskUpdateList });
    }
  ...
  sub ListAllSourceResources {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListSourceResources(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListSourceResources(@_, NextToken => $next_result->NextToken);
        push @{ $result->SourceResourceList }, @{ $next_result->SourceResourceList };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'SourceResourceList') foreach (@{ $result->SourceResourceList });
        $result = $self->ListSourceResources(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'SourceResourceList') foreach (@{ $result->SourceResourceList });
    }
  ...
  sub operations { qw/AssociateCreatedArtifact AssociateDiscoveredResource AssociateSourceResource CreateProgressUpdateStream DeleteProgressUpdateStream DescribeApplicationState DescribeMigrationTask DisassociateCreatedArtifact DisassociateDiscoveredResource DisassociateSourceResource ImportMigrationTask ListApplicationStates ListCreatedArtifacts ListDiscoveredResources ListMigrationTasks ListMigrationTaskUpdates ListProgressUpdateStreams ListSourceResources NotifyApplicationState NotifyMigrationTaskState PutResourceAttributes / }
...
=item SourceResource => L<Paws::MigrationHub::SourceResource>
...
Each argument is described in detail in: L<Paws::MigrationHub::AssociateSourceResource>

Returns: a L<Paws::MigrationHub::AssociateSourceResourceResult> instance
...
=head2 DisassociateSourceResource
...
Each argument is described in detail in: L<Paws::MigrationHub::DisassociateSourceResource>

Returns: a L<Paws::MigrationHub::DisassociateSourceResourceResult> instance
...
=head2 ListMigrationTaskUpdates
...
Each argument is described in detail in: L<Paws::MigrationHub::ListMigrationTaskUpdates>

Returns: a L<Paws::MigrationHub::ListMigrationTaskUpdatesResult> instance

This is a paginated API that returns all the migration-task states for
the specified C<MigrationTaskName> and C<ProgressUpdateStream>.
...
Each argument is described in detail in: L<Paws::MigrationHub::ListSourceResources>

Returns: a L<Paws::MigrationHub::ListSourceResourcesResult> instance

Lists all the source resource that are associated with the specified
C<MigrationTaskName> and C<ProgressUpdateStream>.
...
=head2 ListAllMigrationTaskUpdates(sub { },MigrationTaskName => Str, ProgressUpdateStream => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllMigrationTaskUpdates(MigrationTaskName => Str, ProgressUpdateStream => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - MigrationTaskUpdateList, passing the object as the first parameter, and the string 'MigrationTaskUpdateList' as the second parameter 

If not, it will return a a L<Paws::MigrationHub::ListMigrationTaskUpdatesResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllSourceResources(sub { },MigrationTaskName => Str, ProgressUpdateStream => Str, [MaxResults => Int, NextToken => Str])

=head2 ListAllSourceResources(MigrationTaskName => Str, ProgressUpdateStream => Str, [MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - SourceResourceList, passing the object as the first parameter, and the string 'SourceResourceList' as the second parameter 

If not, it will return a a L<Paws::MigrationHub::ListSourceResourcesResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
