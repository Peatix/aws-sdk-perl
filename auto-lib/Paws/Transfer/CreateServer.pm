...
Required when C<IdentityProviderType> is set to
C<AWS_DIRECTORY_SERVICE>, C<Amazon Web Services_LAMBDA> or
C<API_GATEWAY>. Accepts an array containing all of the information
required to use a directory in C<AWS_DIRECTORY_SERVICE> or invoke a
customer-supplied authentication API, including the API Gateway URL.
Cannot be specified when C<IdentityProviderType> is set to
C<SERVICE_MANAGED>.
...
Valid values are: C<"SERVICE_MANAGED">, C<"API_GATEWAY">, C<"AWS_DIRECTORY_SERVICE">, C<"AWS_LAMBDA">
