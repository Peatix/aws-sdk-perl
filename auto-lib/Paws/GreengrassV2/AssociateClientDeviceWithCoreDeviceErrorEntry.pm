...
package Paws::GreengrassV2::AssociateClientDeviceWithCoreDeviceErrorEntry;
  use Moose;
...
Paws::GreengrassV2::AssociateClientDeviceWithCoreDeviceErrorEntry
...
As an example, if Att1 is expected to be a Paws::GreengrassV2::AssociateClientDeviceWithCoreDeviceErrorEntry object:

  $service_obj->Method(Att1 => { Code => $value, ..., ThingName => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::GreengrassV2::AssociateClientDeviceWithCoreDeviceErrorEntry object:
...
device with a core device. The BatchAssociateClientDeviceWithCoreDevice
