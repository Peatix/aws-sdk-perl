package Paws::ACM;
  ...
  sub RenewCertificate {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::ACM::RenewCertificate', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
Each tag consists of a C<key> and an optional C<value>. You specify the
...
If you have just created a certificate using the C<RequestCertificate>
...
C<RequestCertificate> action, or a certificate imported into ACM using
the C<ImportCertificate> action. The chain consists of the certificate
...
use OpenSSL (https://wiki.openssl.org/index.php/Command_Line_Utilities)
...
Each argument is described in detail in: L<Paws::ACM::RenewCertificate>
...
grant the ACM service principal permission to do so
...
qualified domain name (FQDN) in the C<DomainName> parameter. You can
also specify additional FQDNs in the C<SubjectAlternativeNames>
parameter.
...
(https://datatracker.ietf.org/doc/html/rfc6125#appendix-B.2)
specification of the certificate validation process. ACM first checks
for a Subject Alternative Name, and, if it finds one, ignores the
common name (CN).

After successful completion of the C<RequestCertificate> action, there
...
=head2 ListAllCertificates(sub { },[CertificateStatuses => ArrayRef[Str|Undef], Includes => L<Paws::ACM::Filters>, MaxItems => Int, NextToken => Str, SortBy => Str, SortOrder => Str])

=head2 ListAllCertificates([CertificateStatuses => ArrayRef[Str|Undef], Includes => L<Paws::ACM::Filters>, MaxItems => Int, NextToken => Str, SortBy => Str, SortOrder => Str])
