package Paws::SSM;
  ...
  sub DeleteOpsItem {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::DeleteOpsItem', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DeleteResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::DeleteResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub DescribeInstanceProperties {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::DescribeInstanceProperties', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetAccessToken {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::GetAccessToken', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub GetExecutionPreview {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::GetExecutionPreview', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub GetInventory {
    my $self = shift;
  ...
  sub GetResourcePolicies {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::GetResourcePolicies', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub ListNodes {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::ListNodes', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListNodesSummary {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::ListNodesSummary', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub PutResourcePolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::PutResourcePolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub StartAccessRequest {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::StartAccessRequest', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub StartExecutionPreview {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SSM::StartExecutionPreview', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub StartSession {
    my $self = shift;
  ...
  sub DescribeAllInstanceProperties {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->DescribeInstanceProperties(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->DescribeInstanceProperties(@_, NextToken => $next_result->NextToken);
        push @{ $result->InstanceProperties }, @{ $next_result->InstanceProperties };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'InstanceProperties') foreach (@{ $result->InstanceProperties });
        $result = $self->DescribeInstanceProperties(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'InstanceProperties') foreach (@{ $result->InstanceProperties });
    }
  ...
  sub ListAllNodesSummary {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->ListNodesSummary(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->ListNodesSummary(@_, NextToken => $next_result->NextToken);
        push @{ $result->Summary }, @{ $next_result->Summary };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'Summary') foreach (@{ $result->Summary });
        $result = $self->ListNodesSummary(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'Summary') foreach (@{ $result->Summary });
    }
...
see Using Systems Manager tools
...
Web Services Systems Manager, see the I< Systems Manager Automation
runbook reference
...
For information about AppConfig, a tool in Systems Manager, see the I<
AppConfig User Guide
(https://docs.aws.amazon.com/appconfig/latest/userguide/) > and the I<
...
the I< Systems Manager Incident Manager User Guide
(https://docs.aws.amazon.com/incident-manager/latest/userguide/) > and
the I< Systems Manager Incident Manager API Reference
...
Adds or overwrites one or more tags for the specified resource. I<Tags>
...
For example, you could define a set of tags for your account's managed
nodes that helps you track each node's owner and stack level. For
example:
...
C<Key=Owner,Value=DbAdmin>

=item *

C<Key=Owner,Value=SysAdmin>

=item *

C<Key=Owner,Value=Dev>

=item *

C<Key=Stack,Value=Production>

=item *

C<Key=Stack,Value=Pre-Production>

=item *

C<Key=Stack,Value=Test>
...
(Amazon EC2) instances, see Tag your Amazon EC2 resources
...
with an OpsItem. Incident Manager and OpsCenter are tools in Amazon Web
...
=item [RegistrationMetadata => ArrayRef[L<Paws::SSM::RegistrationMetadataItem>]]
...
=item [AlarmConfiguration => L<Paws::SSM::AlarmConfiguration>]
...
tool in Amazon Web Services Systems Manager applies the configuration
when new managed nodes are added to the group. The association also
specifies actions to take when applying the configuration. For example,
an association for anti-virus software might run once a day. If the
...
Web Services Systems Manager Documents
...
=item [AvailableSecurityUpdatesComplianceStatus => Str]
...
For information about valid key-value pairs in C<PatchFilters> for each
...
of resource data sync: C<SyncToDestination> and C<SyncFromSource>.
...
or C<EntireOrganization> by using Organizations. For more information,
...
the association by using the C<Targets> parameter, then you must delete
...
Each argument is described in detail in: L<Paws::SSM::DeleteOpsItem>

Returns: a L<Paws::SSM::DeleteOpsItemResponse> instance
...
This operation uses an I<eventual consistency model>, which means the
system can take a few minutes to complete this operation. If you delete
an OpsItem and immediately call, for example, GetOpsItem, the deleted
...
Each argument is described in detail in: L<Paws::SSM::DeleteResourcePolicy>

Returns: a L<Paws::SSM::DeleteResourcePolicyResponse> instance
...
C<OpsItemGroup> - The resource policy for C<OpsItemGroup> enables
Amazon Web Services accounts to view and interact with OpsCenter
operational work items (OpsItems).

=item *

C<Parameter> - The resource policy is used to share a parameter with
...
you must use a different Activation Code and Activation ID than used to
...
you created the association by using the C<Targets> parameter, then you
...
Currently, C<DescribeAvailablePatches> supports only the Amazon Linux
...
The C<IamRole> field returned for this API operation is the role
...
=head2 DescribeInstanceProperties

=over

=item [FiltersWithOperator => ArrayRef[L<Paws::SSM::InstancePropertyStringFilter>]]

=item [InstancePropertyFilterList => ArrayRef[L<Paws::SSM::InstancePropertyFilter>]]
...
Each argument is described in detail in: L<Paws::SSM::DescribeInstanceProperties>

Returns: a L<Paws::SSM::DescribeInstancePropertiesResult> instance
...
information, see Set up OpsCenter
(https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setup.html)
in the I<Amazon Web Services Systems Manager User Guide>.
...
to reference KMS. Otherwise, C<DescribeParameters> retrieves whatever
...
Valid properties: C<PRODUCT> | C<CLASSIFICATION> | C<SEVERITY>

=item AMAZON_LINUX_2

Valid properties: C<PRODUCT> | C<CLASSIFICATION> | C<SEVERITY>

=item AMAZON_LINUX_2023
...
Valid properties: C<PRODUCT> | C<PRIORITY>

=item MACOS

Valid properties: C<PRODUCT> | C<CLASSIFICATION>
...
Valid properties: C<PRODUCT> | C<PRODUCT_FAMILY> | C<CLASSIFICATION> |
C<MSRC_SEVERITY>
...
=item AccessRequestId => Str


=back

Each argument is described in detail in: L<Paws::SSM::GetAccessToken>

Returns: a L<Paws::SSM::GetAccessTokenResponse> instance
...
C<GetCalendarState> returns the state of the calendar at that specific
time, and returns the next time that the change calendar state will
transition. If you don't specify a time, C<GetCalendarState> uses the
current time. Change Calendar entries have two possible states: C<OPEN>
or C<CLOSED>.
...
uses. This API is primarily used by the C<AWS-RunPatchBaseline> Systems
...
command. For example, run the command using the C<AWS-RunShellScript>
document or the C<AWS-RunPowerShellScript> document.
...
=head2 GetExecutionPreview

=over

=item ExecutionPreviewId => Str


=back

Each argument is described in detail in: L<Paws::SSM::GetExecutionPreview>

Returns: a L<Paws::SSM::GetExecutionPreviewResponse> instance
...
about an OpsItem. For more information, see Set up OpsCenter
(https://docs.aws.amazon.com/systems-manager/latest/userguide/OpsCenter-setup.html)
in the I<Amazon Web Services Systems Manager User Guide>.
...
as well as information about any Amazon Web Services resource or
service configured to report OpsData to Amazon Web Services Systems
Manager Explorer.
...
If you change the KMS key alias for the KMS key used to encrypt a
parameter, then you must also update the key alias the parameter uses
to reference KMS. Otherwise, C<GetParameterHistory> retrieves whatever
the original key alias was referencing.
...
Each argument is described in detail in: L<Paws::SSM::GetResourcePolicies>

Returns: a L<Paws::SSM::GetResourcePoliciesResponse> instance

Returns an array of the C<Policy> object.
...
default setting of C<false>. This means the user can't use this feature
unless they change the setting to C<true> and intentionally opt in for
a paid feature.
...
Labels can't begin with a number, "C<aws>" or "C<ssm>" (not case
...
each requested managed node ID. C<ListCommandInvocations> provide
...
=item [Filters => ArrayRef[L<Paws::SSM::NodeFilter>]]
...
Each argument is described in detail in: L<Paws::SSM::ListNodes>

Returns: a L<Paws::SSM::ListNodesResult> instance

Takes in filters and returns a list of managed nodes matching the
filter criteria.


=head2 ListNodesSummary

=over

=item Aggregators => ArrayRef[L<Paws::SSM::NodeAggregator>]
...
Each argument is described in detail in: L<Paws::SSM::ListNodesSummary>

Returns: a L<Paws::SSM::ListNodesSummaryResult> instance
...
designated resource. This operation lets you register custom compliance
...
Each argument is described in detail in: L<Paws::SSM::PutResourcePolicy>

Returns: a L<Paws::SSM::PutResourcePolicyResponse> instance
...
To share a C<SecureString> parameter, it must be encrypted with a
...
C<PutResourcePolicy> operation, we recommend using Resource Access
Manager (RAM) instead. This is because using C<PutResourcePolicy>
...
API operation using the C<--shared> option.
...
Each argument is described in detail in: L<Paws::SSM::StartAccessRequest>

Returns: a L<Paws::SSM::StartAccessRequestResponse> instance
...
=item [ExecutionInputs => L<Paws::SSM::ExecutionInputs>]


=back

Each argument is described in detail in: L<Paws::SSM::StartExecutionPreview>

Returns: a L<Paws::SSM::StartExecutionPreviewResponse> instance
...
Amazon Web Services CLI usage: C<start-session> is an interactive
...
Storage Service (Amazon S3) output. When you call C<UpdateAssociation>,
...
even if you are not changing the parameters. This includes the C<Name>
...
parameters required for your C<UpdateAssociation> call.
...
you don't have permission to call C<DescribeAssociation>, then you
receive the following error: C<An error occurred
(AccessDeniedException) when calling the UpdateAssociation operation:
User: E<lt>user_arnE<gt> isn't authorized to perform:
...
C<ApplyOnlyAtCronInterval> parameter to run the association during the
next schedule run.
...
C<UpdateAssociationStatus> is primarily used by the Amazon Web Services
...
C<TaskARN>. For example, you can change a C<RUN_COMMAND> task from
C<AWS-RunPowerShellScript> to C<AWS-RunShellScript>.
...
then the corresponding field isn't modified. If you set C<Replace> to
...
choose the C<Include only the current account> option, you can't edit
that sync later and choose the C<Include all accounts from my
...
=head2 DescribeAllInstanceProperties(sub { },[FiltersWithOperator => ArrayRef[L<Paws::SSM::InstancePropertyStringFilter>], InstancePropertyFilterList => ArrayRef[L<Paws::SSM::InstancePropertyFilter>], MaxResults => Int, NextToken => Str])

=head2 DescribeAllInstanceProperties([FiltersWithOperator => ArrayRef[L<Paws::SSM::InstancePropertyStringFilter>], InstancePropertyFilterList => ArrayRef[L<Paws::SSM::InstancePropertyFilter>], MaxResults => Int, NextToken => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - InstanceProperties, passing the object as the first parameter, and the string 'InstanceProperties' as the second parameter 

If not, it will return a a L<Paws::SSM::DescribeInstancePropertiesResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 DescribeAllParameters(sub { },[Filters => ArrayRef[L<Paws::SSM::ParametersFilter>], MaxResults => Int, NextToken => Str, ParameterFilters => ArrayRef[L<Paws::SSM::ParameterStringFilter>], Shared => Bool])

=head2 DescribeAllParameters([Filters => ArrayRef[L<Paws::SSM::ParametersFilter>], MaxResults => Int, NextToken => Str, ParameterFilters => ArrayRef[L<Paws::SSM::ParameterStringFilter>], Shared => Bool])
...
If not, it will return a a L<Paws::SSM::GetResourcePoliciesResponse> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllNodes(sub { },[Filters => ArrayRef[L<Paws::SSM::NodeFilter>], MaxResults => Int, NextToken => Str, SyncName => Str])

=head2 ListAllNodes([Filters => ArrayRef[L<Paws::SSM::NodeFilter>], MaxResults => Int, NextToken => Str, SyncName => Str])
...
If not, it will return a a L<Paws::SSM::ListNodesResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.


=head2 ListAllNodesSummary(sub { },Aggregators => ArrayRef[L<Paws::SSM::NodeAggregator>], [Filters => ArrayRef[L<Paws::SSM::NodeFilter>], MaxResults => Int, NextToken => Str, SyncName => Str])

=head2 ListAllNodesSummary(Aggregators => ArrayRef[L<Paws::SSM::NodeAggregator>], [Filters => ArrayRef[L<Paws::SSM::NodeFilter>], MaxResults => Int, NextToken => Str, SyncName => Str])


If passed a sub as first parameter, it will call the sub for each element found in :

 - Summary, passing the object as the first parameter, and the string 'Summary' as the second parameter 

If not, it will return a a L<Paws::SSM::ListNodesSummaryResult> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
