...
package Paws::CloudFormation::ResourceTargetDefinition;
  ...
  has AttributeChangeType => (is => 'ro', isa => 'Str');
...
As an example, if Att1 is expected to be a Paws::CloudFormation::ResourceTargetDefinition object:
  ...
  $service_obj->Method(Att1 => { AfterValue => $value, ..., RequiresRecreation => $value  });
...
C<Add> The item will be added.

=item *

C<Remove> The item will be removed.

=item *

C<Modify> The item will be modified.
...
The value of the property before the change is executed. Large values
can be truncated.
...
property in the Amazon Web Services resource and property types
reference
