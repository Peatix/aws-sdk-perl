...
As a best practice, configure both C<verified_email> and
C<verified_phone_number>, with one having a higher priority than the
other.
...
information about alias attributes, see Customizing sign-in attributes
...
user pool. For more information, see Working with user attributes
...
your Amazon Web Services account. For more information see SMS message
settings
...
C<CaseSensitive> is C<false> (case insensitive), users can sign in with
any combination of capital and lowercase letters. For example,
C<username>, C<USERNAME>, or C<UserName>, or for email,
C<email@example.com> or C<EMaiL@eXamplE.Com>. For most use cases, set
case sensitivity to C<false> as a best practice. When usernames and
...
When C<CaseSensitive> is C<true> (case sensitive), Amazon Cognito
interprets C<USERNAME> and C<UserName> as distinct users.
