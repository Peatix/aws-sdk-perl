...
event publishing to send information about these events to other Amazon
Web Services services such as and Amazon CloudWatch

=item IP pool management

When leasing dedicated IP addresses with Amazon Simple Email Service,
you can create groups of IP addresses, called dedicated IP pools. You
...
typical production environments, the default email limit is less than
the required delivery volume. To achieve a higher delivery volume,
...
The default FROM address is C<no-reply@verificationemail.com>. To
customize the FROM address, provide the Amazon Resource Name (ARN) of
an Amazon SES verified email address for the C<SourceArn> parameter.
...
address for the C<SourceArn> parameter.
...
This role contains the permissions that allow you to access Amazon SES
and send email messages from your email address. For more information
...
Either the senderE<rsquo>s email address or the senderE<rsquo>s name
with their email address. For example, C<testuser@example.com> or
C<Test User E<lt>testuser@example.comE<gt>>. This address appears
...
domain in Amazon SES. You can set a C<SourceArn> email from a verified
...
C<EmailSendingAccount> parameter:
...
the custom FROM address when it emails your users using its built-in
email account.
...
The Region value of the C<SourceArn> parameter must indicate a
supported Amazon Web Services Region of your user pool. Typically, the
Region in the C<SourceArn> and the user pool Region are the same. For
more information, see Amazon SES email configuration regions
