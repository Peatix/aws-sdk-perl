...
type is a member of C<ResourceServerScopeType>. For more information,
see Scopes, M2M, and API authorization with resource servers
...
format C<resourceServerIdentifier/ScopeName>. For example, if this
parameter is C<exampleScope> in the resource server with the identifier
C<exampleResourceServer>, you request and receive the scope
C<exampleResourceServer/exampleScope>.
