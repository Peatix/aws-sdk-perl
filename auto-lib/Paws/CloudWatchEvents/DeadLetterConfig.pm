...
package Paws::CloudWatchEvents::DeadLetterConfig;
  use Moose;
...
Paws::CloudWatchEvents::DeadLetterConfig
...
As an example, if Att1 is expected to be a Paws::CloudWatchEvents::DeadLetterConfig object:

  $service_obj->Method(Att1 => { Arn => $value, ..., Arn => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CloudWatchEvents::DeadLetterConfig object:
...
Configuration details of the Amazon SQS queue for EventBridge to use as
