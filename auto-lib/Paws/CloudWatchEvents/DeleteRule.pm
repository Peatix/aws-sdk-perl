...
your behalf, you must specify C<Force> as C<True> to delete the rule.
This parameter is ignored for rules that are not managed rules. You can
check whether a rule is a managed rule by using C<DescribeRule> or
C<ListRules> and checking the C<ManagedBy> field of the response.
