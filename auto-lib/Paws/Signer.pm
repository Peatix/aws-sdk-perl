package Paws::Signer;
  ...
  sub GetRevocationStatus {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Signer::GetRevocationStatus', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub SignPayload {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::Signer::SignPayload', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub StartSigningJob {
    my $self = shift;
...
(http://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/), and
AWS CloudTrail
(http://docs.aws.amazon.com/awscloudtrail/latest/userguide/). In order
...
signatures are stored in the registry alongside the images, where they
are available for verifying image authenticity and integrity.
...
=head2 GetRevocationStatus

=over

=item CertificateHashes => ArrayRef[Str|Undef]

=item JobArn => Str

=item PlatformId => Str

=item ProfileVersionArn => Str

=item SignatureTimestamp => Str


=back

Each argument is described in detail in: L<Paws::Signer::GetRevocationStatus>

Returns: a L<Paws::Signer::GetRevocationStatusResponse> instance
...
with your C<maxResults> parameter and with new values that Signer
...
new values that Signer returns in the C<nextToken> parameter until all
...
Each argument is described in detail in: L<Paws::Signer::SignPayload>

Returns: a L<Paws::Signer::SignPayloadResponse> instance
