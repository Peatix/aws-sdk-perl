...
Paws::MachineLearning::DeleteRealtimeEndpoint - Arguments for method DeleteRealtimeEndpoint on L<Paws::MachineLearning>
...
You shouldn't make instances of this class. Each attribute should be used as a named argument in the call to DeleteRealtimeEndpoint.

=head1 SYNOPSIS

    my $machinelearning = Paws->service('MachineLearning');
    my $DeleteRealtimeEndpointOutput = $machinelearning->DeleteRealtimeEndpoint(
      MLModelId => 'MyEntityId',

    );

    # Results:
    my $MLModelId = $DeleteRealtimeEndpointOutput->MLModelId;
    my $RealtimeEndpointInfo =
      $DeleteRealtimeEndpointOutput->RealtimeEndpointInfo;

    # Returns a L<Paws::MachineLearning::DeleteRealtimeEndpointOutput> object.
