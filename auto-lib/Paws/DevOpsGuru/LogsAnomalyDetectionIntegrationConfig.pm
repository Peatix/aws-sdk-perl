...
package Paws::DevOpsGuru::LogsAnomalyDetectionIntegrationConfig;
  use Moose;
...
Paws::DevOpsGuru::LogsAnomalyDetectionIntegrationConfig
...
As an example, if Att1 is expected to be a Paws::DevOpsGuru::LogsAnomalyDetectionIntegrationConfig object:

  $service_obj->Method(Att1 => { OptInStatus => $value, ..., OptInStatus => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::DevOpsGuru::LogsAnomalyDetectionIntegrationConfig object:
