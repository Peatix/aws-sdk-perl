...
package Paws::DevOpsGuru::TagCostEstimationResourceCollectionFilter;
  use Moose;
...
Paws::DevOpsGuru::TagCostEstimationResourceCollectionFilter
...
As an example, if Att1 is expected to be a Paws::DevOpsGuru::TagCostEstimationResourceCollectionFilter object:

  $service_obj->Method(Att1 => { AppBoundaryKey => $value, ..., TagValues => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::DevOpsGuru::TagCostEstimationResourceCollectionFilter object:
...
can specify for a cost estimate is one. The estimate created is for the
cost to analyze the Amazon Web Services resources defined by the tag.
