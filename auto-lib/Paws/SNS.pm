package Paws::SNS;
  ...
  sub GetDataProtectionPolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SNS::GetDataProtectionPolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub PublishBatch {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SNS::PublishBatch', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub PutDataProtectionPolicy {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SNS::PutDataProtectionPolicy', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
permissions to the C<AddPermission>, C<RemovePermission>, and
C<SetTopicAttributes> actions in your IAM policy.
...
For ADM, C<PlatformPrincipal> is C<client id> and C<PlatformCredential>
is C<client secret>.

=item *

For APNS and C<APNS_SANDBOX> using certificate credentials,
C<PlatformPrincipal> is C<SSL certificate> and C<PlatformCredential> is
C<private key>.

=item *

For APNS and C<APNS_SANDBOX> using token credentials,
C<PlatformPrincipal> is C<signing key ID> and C<PlatformCredential> is
C<signing key>.

=item *

For Baidu, C<PlatformPrincipal> is C<API key> and C<PlatformCredential>
is C<secret key>.
...
For MPNS, C<PlatformPrincipal> is C<TLS certificate> and
C<PlatformCredential> is C<private key>.

=item *

For WNS, C<PlatformPrincipal> is C<Package Security Identifier> and
...
Services account is in the I<SMS sandbox>. The SMS sandbox provides a
...
Each argument is described in detail in: L<Paws::SNS::GetDataProtectionPolicy>

Returns: a L<Paws::SNS::GetDataProtectionPolicyResponse> instance

Retrieves the specified inline C<DataProtectionPolicy> document that is
...
numbers, see Origination numbers
...
When a C<messageId> is returned, the message is saved and Amazon SNS
immediately delivers it to subscribers.

To use the C<Publish> action for publishing a message to a mobile
...
=item PublishBatchRequestEntries => ArrayRef[L<Paws::SNS::PublishBatchRequestEntry>]
...
Each argument is described in detail in: L<Paws::SNS::PublishBatch>

Returns: a L<Paws::SNS::PublishBatchResponse> instance

Publishes up to ten messages to the specified topic. This is a batch
version of C<Publish>. For FIFO topics, multiple messages within a
single batch are published in the order they are sent, and messages are
...
When a C<messageId> is returned, the batch message is saved and Amazon
...
Each argument is described in detail in: L<Paws::SNS::PutDataProtectionPolicy>
...
C<ConfirmSubscription> action to confirm the subscription.
...
Amazon SQS queue subscriptions require authentication for deletion.
