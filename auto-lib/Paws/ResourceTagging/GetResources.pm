...
following string would limit the response to only Amazon EC2 instances,
Amazon S3 buckets, or any Audit Manager resource:

C<ec2:instance,s3:bucket,auditmanager>
...
included, the specified values. Each C<TagFilter> must contain a key
with values optional. A request can include up to 50 keys, and each key
...
the response returns resources that match I<any> of the specified
values for that key.
...
Amazon Web Services recommends using C<ResourcesPerPage> instead of
this parameter.
