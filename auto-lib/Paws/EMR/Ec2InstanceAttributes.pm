...
from among the list of C<RequestedEc2AvailabilityZones>, and then
...
you. C<RequestedEc2SubnetIDs> and C<RequestedEc2AvailabilityZones>
...
the same VPC. Amazon EMR chooses the Amazon EC2 subnet with the best
fit from among the list of C<RequestedEc2SubnetIds>, and then launches
all cluster instances within that Subnet. If this value is not
specified, and the account and Region support EC2-Classic networks, the
