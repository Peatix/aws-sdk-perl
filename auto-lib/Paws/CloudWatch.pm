package Paws::CloudWatch;
  ...
  sub ListManagedInsightRules {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatch::ListManagedInsightRules', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub ListMetrics {
    my $self = shift;
  ...
  sub PutManagedInsightRules {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CloudWatch::PutManagedInsightRules', @_);
    return $self->caller->do_call($self, $call_object);
  }
  sub PutMetricAlarm {
    my $self = shift;
  ...
  sub DescribeAllAnomalyDetectors {
    my $self = shift;

    my $callback = shift @_ if (ref($_[0]) eq 'CODE');
    my $result = $self->DescribeAnomalyDetectors(@_);
    my $next_result = $result;

    if (not defined $callback) {
      while ($next_result->NextToken) {
        $next_result = $self->DescribeAnomalyDetectors(@_, NextToken => $next_result->NextToken);
      ...
      while ($result->NextToken) {
        ...
        $result = $self->DescribeAnomalyDetectors(@_, NextToken => $result->NextToken);
      }
      $callback->($_ => 'AnomalyDetectors') foreach (@{ $result->AnomalyDetectors });
    }
  ...
  sub ListAllMetrics {
    ...
    if (not defined $callback) {
      while ($next_result->NextToken) {
        ...
        push @{ $result->OwningAccounts }, @{ $next_result->OwningAccounts };
      }
      return $result;
    } else {
      while ($result->NextToken) {
        $callback->($_ => 'Metrics') foreach (@{ $result->Metrics });
        $callback->($_ => 'OwningAccounts') foreach (@{ $result->OwningAccounts });
...
in real time. You can use CloudWatch to collect and track metrics,
which are the variables you want to measure for your resources and
applications.
...
Web Services, you can monitor your own custom metrics. With CloudWatch,
you gain system-wide visibility into resource utilization, application
...
operation after using C<DeleteAlarms>.
...
=item [MetricMathAnomalyDetector => L<Paws::CloudWatch::MetricMathAnomalyDetector>]

=item [MetricName => Str]

=item [Namespace => Str]

=item [SingleMetricAnomalyDetector => L<Paws::CloudWatch::SingleMetricAnomalyDetector>]
...
you must be signed on with the C<cloudwatch:DescribeAlarmHistory>
permission that is scoped to C<*>. You can't return information about
composite alarms if your C<cloudwatch:DescribeAlarmHistory> permission
...
you must be signed on with the C<cloudwatch:DescribeAlarms> permission
that is scoped to C<*>. You can't return information about composite
alarms if your C<cloudwatch:DescribeAlarms> permission has a narrower
scope.
...
=item [AnomalyDetectorTypes => ArrayRef[Str|Undef]]
...
C<METRIC_MATH> to the C<AnomalyDetectorTypes> array. This will return
...
You can use the C<GetMetricData> API to retrieve CloudWatch metric
values. The operation can also include a CloudWatch Metrics Insights
query, and one or more metric math functions.

A C<GetMetricData> operation that does not include a query can retrieve
...
If you include a Metrics Insights query, each C<GetMetricData>
operation can include only one query. But the same C<GetMetricData>
...
without a B<GROUP BY> clause returns a single time-series (TS), and can
be used as input for a metric math expression that expects a single
time series. A Metrics Insights query with a B<GROUP BY> clause returns
an array of time-series (TS[]), and can be used as input for a metric
math expression that expects an array of time series.
...
Each argument is described in detail in: L<Paws::CloudWatch::ListManagedInsightRules>

Returns: a L<Paws::CloudWatch::ListManagedInsightRulesOutput> instance
...
=item [MetricCharacteristics => L<Paws::CloudWatch::MetricCharacteristics>]
...
specify in the C<SingleMetricAnomalyDetector> parameter.
...
=item [ActionsSuppressor => Str]

=item [ActionsSuppressorExtensionPeriod => Int]

=item [ActionsSuppressorWaitPeriod => Int]
...
composite alarm can include as many as 100 underlying alarms. Any
...
C<cloudwatch:PutCompositeAlarm> permission that is scoped to C<*>. You
can't create a composite alarms if your C<cloudwatch:PutCompositeAlarm>
...
=head2 PutManagedInsightRules

=over

=item ManagedRules => ArrayRef[L<Paws::CloudWatch::ManagedRule>]


=back

Each argument is described in detail in: L<Paws::CloudWatch::PutManagedInsightRules>

Returns: a L<Paws::CloudWatch::PutManagedInsightRulesOutput> instance
...
services. You cannot edit these rules with C<PutInsightRule>. The rules
can be enabled, disabled, and deleted using C<EnableInsightRules>,
C<DisableInsightRules>, and C<DeleteInsightRules>. If a previously
created managed rule is currently disabled, a subsequent call to this
API will re-enable it. Use C<ListManagedInsightRules> to describe all
available rules.
...
query for an alarm, see Create alarms on Metrics Insights queries
...
The C<iam:CreateServiceLinkedRole> permission for all alarms with EC2
actions

=item *

The C<iam:CreateServiceLinkedRole> permissions to create an alarm with
...
service-linked roles are called C<AWSServiceRoleForCloudWatchEvents>
and C<AWSServiceRoleForCloudWatchAlarms_ActionSSM>. For more
information, see Amazon Web Services service-linked role
(https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_terms-and-concepts.html#iam-term-service-linked-role).

Each C<PutMetricAlarm> action has a maximum uncompressed payload of 120
KB.
...
already have a sharing role named
B<CloudWatch-CrossAccountSharingRole>. If it does not already have this
...
B<AWSServiceRoleForCloudWatchCrossAccount> to allow CloudWatch to
assume the sharing role in the sharing account. If it does not, you
must create it following the directions in B<Set up a monitoring
account> in Cross-account cross-Region CloudWatch console
(https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Cross-Account-Cross-Region.html#enable-cross-account-cross-Region).
...
=item [EntityMetricData => ArrayRef[L<Paws::CloudWatch::EntityMetricData>]]

=item [MetricData => ArrayRef[L<Paws::CloudWatch::MetricDatum>]]

=item [StrictEntityValidation => Bool]
...
C<EntityMetricData> parameter. To send metrics without entity data, use
the C<MetricData> parameter. The C<EntityMetricData> structure includes
C<MetricData> structures for the metric data.

You can publish either individual values in the C<Value> field, or
arrays of values and the number of times each value occurred during the
period by using the C<Values> and C<Counts> fields in the C<MetricData>
structure. Using the C<Values> and C<Counts> method enables you to
publish up to 150 values per metric with one C<PutMetricData> request,
and supports retrieving percentile statistics on this data.

Each C<PutMetricData> request is limited to 1 MB in size for HTTP POST
...
C<MetricData> and C<EntityMetricData> properties).
...
=item [IncludeLinkedAccountsMetrics => Bool]

=item [StatisticsConfigurations => ArrayRef[L<Paws::CloudWatch::MetricStreamStatisticsConfiguration>]]
...
use the C<StatisticsConfigurations> parameter to have the metric stream
...
=head2 DescribeAllAnomalyDetectors(sub { },[AnomalyDetectorTypes => ArrayRef[Str|Undef], Dimensions => ArrayRef[L<Paws::CloudWatch::Dimension>], MaxResults => Int, MetricName => Str, Namespace => Str, NextToken => Str])

=head2 DescribeAllAnomalyDetectors([AnomalyDetectorTypes => ArrayRef[Str|Undef], Dimensions => ArrayRef[L<Paws::CloudWatch::Dimension>], MaxResults => Int, MetricName => Str, Namespace => Str, NextToken => Str])
...
If not, it will return a a L<Paws::CloudWatch::DescribeAnomalyDetectorsOutput> instance with all the C<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
...
=head2 ListAllMetrics(sub { },[Dimensions => ArrayRef[L<Paws::CloudWatch::DimensionFilter>], IncludeLinkedAccounts => Bool, MetricName => Str, Namespace => Str, NextToken => Str, OwningAccount => Str, RecentlyActive => Str])

=head2 ListAllMetrics([Dimensions => ArrayRef[L<Paws::CloudWatch::DimensionFilter>], IncludeLinkedAccounts => Bool, MetricName => Str, Namespace => Str, NextToken => Str, OwningAccount => Str, RecentlyActive => Str])
...
If passed a sub as first parameter, it will call the sub for each element found in :
 ...
 - OwningAccounts, passing the object as the first parameter, and the string 'OwningAccounts' as the second parameter 

If not, it will return a a L<Paws::CloudWatch::ListMetricsOutput> instance with all the C<param>s; andC<param>s;  from all the responses. Please take into account that this mode can potentially consume vasts ammounts of memory.
