...
package Paws::Athena::WorkGroupConfiguration;
  ...
  has IdentityCenterConfiguration => (is => 'ro', isa => 'Paws::Athena::IdentityCenterConfiguration');
...
As an example, if Att1 is expected to be a Paws::Athena::WorkGroupConfiguration object:
  ...
  $service_obj->Method(Att1 => { AdditionalConfiguration => $value, ..., ResultConfiguration => $value  });
...
whether the Amazon CloudWatch Metrics are enabled for the workgroup and
whether workgroup settings override query settings, and the data usage
...
Queries on the C<AmazonAthenaPreviewFunctionality> workgroup run on the
...
sessions and IAM Identity Center enabled workgroups. This property
applies only to Spark enabled workgroups and IAM Identity Center
enabled workgroups. The property is required for IAM Identity Center
enabled workgroups.


=head2 IdentityCenterConfiguration => L<Paws::Athena::IdentityCenterConfiguration>
...
queries (client-side), using ResultConfiguration$OutputLocation. If
