...
package Paws::LexModelsV2::AnalyticsIntentStageFilter;
  use Moose;
...
Paws::LexModelsV2::AnalyticsIntentStageFilter
...
As an example, if Att1 is expected to be a Paws::LexModelsV2::AnalyticsIntentStageFilter object:

  $service_obj->Method(Att1 => { Name => $value, ..., Values => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::LexModelsV2::AnalyticsIntentStageFilter object:
...
stages. The expression may be understood as C<name> C<operator>
...
C<IntentName CO Book> E<ndash> The intent name contains the string
"Book."

=item *

C<BotVersion EQ 2> E<ndash> The bot version is equal to two.
...
C<IntentName> E<ndash> C<EQ>, C<CO>.

=item *

C<IntentStageName> E<ndash> C<EQ>, C<CO>.
...
The category by which to filter the intent stages. The descriptions for
...
C<IntentStageName> E<ndash> The stage in the intent.
