package Paws::CodeCommit;
  ...
  sub ListFileCommitHistory {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodeCommit::ListFileCommitHistory', @_);
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub UpdateRepositoryEncryptionKey {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::CodeCommit::UpdateRepositoryEncryptionKey', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
This is the I<CodeCommit API Reference>. This reference provides
...
ListFileCommitHistory, which retrieves a list of commits and changes to
a specified file.
...
associated with a repository, CodeCommit creates approval rules that
...
that can then be associated with one or more repositories in your
...
Each argument is described in detail in: L<Paws::CodeCommit::ListFileCommitHistory>

Returns: a L<Paws::CodeCommit::ListFileCommitHistoryResponse> instance
...
resources in CodeCommit, see CodeCommit Resources and Operations
(https://docs.aws.amazon.com/codecommit/latest/userguide/auth-and-access-control-iam-access-control-identity-based.html#arn-formats)
...
=head2 UpdateRepositoryEncryptionKey
...
Each argument is described in detail in: L<Paws::CodeCommit::UpdateRepositoryEncryptionKey>

Returns: a L<Paws::CodeCommit::UpdateRepositoryEncryptionKeyOutput> instance
...
about the limits on repository names, see Quotas
