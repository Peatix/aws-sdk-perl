...
package Paws::QBusiness::SnippetExcerpt;
  use Moose;
...
As an example, if Att1 is expected to be a Paws::QBusiness::SnippetExcerpt object:

  $service_obj->Method(Att1 => { Text => $value, ..., Text => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::QBusiness::SnippetExcerpt object:
...
generate a citation text segment in an Amazon Q Business chat response.
