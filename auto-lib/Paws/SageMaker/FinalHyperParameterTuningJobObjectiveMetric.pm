...
package Paws::SageMaker::FinalHyperParameterTuningJobObjectiveMetric;
  use Moose;
...
Paws::SageMaker::FinalHyperParameterTuningJobObjectiveMetric
...
As an example, if Att1 is expected to be a Paws::SageMaker::FinalHyperParameterTuningJobObjectiveMetric object:

  $service_obj->Method(Att1 => { MetricName => $value, ..., Value => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SageMaker::FinalHyperParameterTuningJobObjectiveMetric object:
