...
package Paws::SageMaker::RSessionAppSettings;
  use Moose;
...
Paws::SageMaker::RSessionAppSettings
...
As an example, if Att1 is expected to be a Paws::SageMaker::RSessionAppSettings object:

  $service_obj->Method(Att1 => { CustomImages => $value, ..., DefaultResourceSpec => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SageMaker::RSessionAppSettings object:
...
A collection of settings that apply to an C<RSessionGateway> app.
