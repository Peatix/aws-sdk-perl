package Paws::SES;
  ...
  sub PutConfigurationSetDeliveryOptions {
    my $self = shift;
    my $call_object = $self->new_with_coercions('Paws::SES::PutConfigurationSetDeliveryOptions', @_);
    return $self->caller->do_call($self, $call_object);
  }
...
(https://docs.aws.amazon.com/ses/latest/DeveloperGuide/regions.html) in
...
changes to "Failed." In that case, to verify the domain, you must
...
about using configuration sets, see Monitoring Your Amazon SES Sending
Activity
(https://docs.aws.amazon.com/ses/latest/dg/monitor-sending-activity.html)
in the I<Amazon SES Developer Guide.>
...
operation also returns C<NextToken>. You can then execute the
C<ListConfigurationSets> operation again, passing the C<NextToken>
...
original call, e.g., if you used C<IdentityType=Domain> in the the
original call and received a C<NextToken> in the response, you should
continue providing the C<IdentityType=Domain> parameter for further
C<NextToken> calls; however, if you didn't provide the C<IdentityType>
parameter in the original call, then continue to not provide it for
successive pagination calls. Using this protocol will ensure consistent
results.
...
Each argument is described in detail in: L<Paws::SES::PutConfigurationSetDeliveryOptions>

Returns: a L<Paws::SES::PutConfigurationSetDeliveryOptionsResponse> instance
...
This operation is more flexible than the C<SendEmail> operation. When
you use the C<SendRawEmail> operation, you can specify the headers of
...
C<SendRawEmail> operation, Amazon SES automatically applies its own
...
C<ReturnPathArn>, or you can include the following X-headers in the
...
C<Destination> parameter. A destination is a set of recipients that
receives the same version of the email. The C<Destination> parameter
can include up to 50 recipients, across the To:, CC: and BCC: fields.
...
call this operation with C<RuleSetName> set to null.
...
in the current Amazon Web Services Region. You can use this operation
in conjunction with Amazon CloudWatch alarms to temporarily pause email
...
When you execute the C<VerifyDomainDkim> operation, the domain that you
...
domain with your account by using the C<VerifyDomainIdentity>
...
In the preceding example, replace I<token> with one of the tokens that
are generated when you execute this operation. Replace I<example.com>
