...
B<Authorize application> page, for B<Organization access>, choose
B<Request access> next to each repository you want to allow CodeBuild
to have access to, and then choose B<Authorize application>. (After you
have connected to your GitHub account, you do not need to finish
...
buildspec file. You must connect your Amazon Web Services account to
your GitLab account. Use the CodeBuild console to start creating a
build project. When you use the console to connect (or reconnect) with
GitLab, on the Connections B<Authorize application> page, choose
B<Authorize>. Then on the CodeConnections B<Create GitLab connection>
page, choose B<Connect to GitLab>. (After you have connected to your
GitLab account, you do not need to finish creating the build project.
You can leave the CodeBuild console.) To instruct CodeBuild to override
the default connection and use this connection instead, set the C<auth>
object's C<type> value to C<CODECONNECTIONS> in the C<source> object.
...
GitHub, GitHub Enterprise, GitLab, GitLab Self Managed, GitLab, GitLab
Self Managed, or Bitbucket. If this is set and you use a different
source provider, an C<invalidInputException> is thrown.
...
C<GITLAB>: The source code is in a GitLab repository.

=item *

C<GITLAB_SELF_MANAGED>: The source code is in a self-managed GitLab
repository.
