...
The stack's Amazon Web Services Region, such as C<ap-south-1>. For more
...
In the CLI, this API maps to the C<--stack-region> parameter. If the
C<--stack-region> parameter and the CLI common parameter C<--region>
are set to the same value, the stack uses a I<regional> endpoint. If
the C<--stack-region> parameter is not set, but the CLI C<--region>
parameter is, this also results in a stack with a I<regional> endpoint.
However, if the C<--region> parameter is set to C<us-east-1>, and the
C<--stack-region> parameter is set to one of the following, then the
stack uses a legacy or I<classic> region: C<us-west-1, us-west-2,
...
in C<us-east-1>. Only the preceding regions are supported as classic
regions in the C<us-east-1> API endpoint. Because it is a best practice
...
new stacks. The CLI common C<--region> parameter always specifies a
...
The stack's IAM role, which allows OpsWorks Stacks to work with Amazon
