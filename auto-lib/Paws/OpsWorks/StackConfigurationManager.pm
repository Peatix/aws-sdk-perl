...
As an example, if Att1 is expected to be a Paws::OpsWorks::StackConfigurationManager object:

  $service_obj->Method(Att1 => { Name => $value, ..., Version => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::OpsWorks::StackConfigurationManager object:
...
The name. This parameter must be set to C<Chef>.
