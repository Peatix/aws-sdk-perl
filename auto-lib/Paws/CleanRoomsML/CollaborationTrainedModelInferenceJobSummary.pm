...
package Paws::CleanRoomsML::CollaborationTrainedModelInferenceJobSummary;
  use Moose;
...
Paws::CleanRoomsML::CollaborationTrainedModelInferenceJobSummary
...
As an example, if Att1 is expected to be a Paws::CleanRoomsML::CollaborationTrainedModelInferenceJobSummary object:

  $service_obj->Method(Att1 => { CollaborationIdentifier => $value, ..., UpdateTime => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CleanRoomsML::CollaborationTrainedModelInferenceJobSummary object:
