...
returns C<DryRunOperation>; otherwise, it returns
C<UnauthorizedException>. Defaults to C<false> if not specified.
...
the C<timestamp> value is not before the start of the software usage.



=head2 UsageAllocations => ArrayRef[L<Paws::MarketplaceMetering::UsageAllocation>]

The set of C<UsageAllocations> to submit.

The sum of all C<UsageAllocation> quantities must equal the
C<UsageQuantity> of the C<MeterUsage> request, and each
C<UsageAllocation> must have a unique set of tags (include no tags).
