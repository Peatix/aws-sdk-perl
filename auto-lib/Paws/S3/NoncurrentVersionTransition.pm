...
As an example, if Att1 is expected to be a Paws::S3::NoncurrentVersionTransition object:
  ...
  $service_obj->Method(Att1 => { NewerNoncurrentVersions => $value, ..., StorageClass => $value  });
...
C<INTELLIGENT_TIERING>, C<GLACIER_IR>, C<GLACIER>, or C<DEEP_ARCHIVE>
...
transition noncurrent object versions to the C<STANDARD_IA>,
C<ONEZONE_IA>, C<INTELLIGENT_TIERING>, C<GLACIER_IR>, C<GLACIER>, or
C<DEEP_ARCHIVE> storage class at a specific period in the object's
lifetime.
...
same storage class before transitioning objects. You can specify up to
100 noncurrent versions to retain. Amazon S3 will transition any
additional noncurrent versions beyond the specified number to retain.
For more information about noncurrent versions, see Lifecycle
